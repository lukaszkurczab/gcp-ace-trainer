...
test("review transition preserves due counter, reset, resolution, and same-session protection", () => {
  ...
  const base = (kind: "correct" | "incorrect", id: string, sessionId: string, committedAt: string) => createTrainingAttempt({ occurrenceId: "occurrence-1", id, sessionId, trackId: "algorithms", modeId: "algorithms-weak-area-review", item: catalog.toContentItemRef(item), response: { kind: "choice" as const, selectedOptionIds: [] }, result: { kind, earnedPoints: kind === "correct" ? 1 : 0, maxPoints: 1 }, reviewEvidence: { sourceItem: catalog.toContentItemRef(item), taxonomyOrSkillRefs: [] }, answeredAt: committedAt, committedAt });

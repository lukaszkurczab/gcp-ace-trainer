...
import { ALGORITHM_CONTENT_VERSION, algorithmContentCatalog, createAlgorithmReviewEntry, isAlgorithmChoiceQuestion, scoreAlgorithmQuestion, updateAlgorithmReviewEntry, type AlgorithmQuestion } from "../src/tracks/algorithms";
...
test("Algorithms family responses preserve choice, ordering, and complexity scoring", () => {
  const items = algorithmContentCatalog.getItems();
  const choice = items.find(isAlgorithmChoiceQuestion); assert.ok(choice);
...
test("review transition preserves due counter, reset, resolution, and same-session protection", () => {
  const item = algorithmContentCatalog.getItems().find(isAlgorithmChoiceQuestion); assert.ok(item);
  const base = (kind: "correct" | "incorrect", id: string, sessionId: string, committedAt: string) => createTrainingAttempt({ id, sessionId, trackId: "algorithms", modeId: "algorithms-review", item: algorithmContentCatalog.toContentItemRef(item), response: { kind: "choice" as const, selectedOptionIds: [] }, result: { kind, earnedPoints: kind === "correct" ? 1 : 0, maxPoints: 1 }, reviewEvidence: { sourceItem: algorithmContentCatalog.toContentItemRef(item), taxonomyOrSkillRefs: [] }, answeredAt: committedAt, committedAt });

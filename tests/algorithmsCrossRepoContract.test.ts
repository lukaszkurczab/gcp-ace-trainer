...
import { ALGORITHM_MODE_IDS, AlgorithmContentCatalog, prepareAlgorithmsInterviewSimulation, selectAlgorithmSessionPlan } from "../src/tracks/algorithms";
...
test("round-trips a production-pipeline test artifact through the exact Algorithms consumer and every declared mode", async () => {
  ...
  try {
    ...
    assert.deepEqual(Object.keys(JSON.parse(built.artifact.artifactBytes).bank).sort(), ["compatibilitySets", "contentVersion", "contrastSets", "familyId", "feedbackAssets", "formatVersion", "interleavedScopes", "items", "practiceBlueprints", "recognitionSets", "simulationPools", "simulationProfiles", "trackId"].sort());
    ...
    assert.deepEqual(selected.map((entry) => entry.actualLength), [10, 20, 10, 20, 20, 10, 20, 40]);

...
test("round-trips a production-pipeline test artifact through the exact Algorithms consumer and every declared mode", async () => {
  ...
  try {
    ...
    const selected = [
      selectAlgorithmSessionPlan({ contentCatalog: catalog, mode: "algorithms-learn-approach", sessionLength: 10, scope: { mentalUnitId: "arrays_and_strings" } }),
      selectAlgorithmSessionPlan({ contentCatalog: catalog, mode: "algorithms-guided-practice", sessionLength: 20, scope: { mentalUnitId: "arrays_and_strings" } }),

...
import { ForegroundSimulationTimer } from "../src/application/runtime/ForegroundSimulationTimer";
...
test("foreground timer checkpoints monotonic segments with an explicit drift bound and never counts closed-app wall time", async () => {
  ...
  const timer = ForegroundSimulationTimer.restore({ state: initial, port, monotonicClock: { now: () => monotonic }, wallClock: { now: () => wall } });
  ...
  const restarted = ForegroundSimulationTimer.restore({ state: durable, port, monotonicClock: { now: () => restartedMonotonic }, wallClock: { now: () => wall } });

...
test("preserves journal after verification failure", async () => { const storage = installMemoryStorage(); const record = outcome(); await persistMutationJournal(record); await materializeMutation(record); storage.resetCounters(); storage.setFailurePlan({ kind: "fail_on_key_read", key: STORAGE_KEYS.ACTIVE_SESSION_RUNTIME }); await assert.rejects(() => recoverPendingMutation(), StorageReadError); storage.setFailurePlan(null); assert.notEqual(await getActiveMutationJournal(), null); });

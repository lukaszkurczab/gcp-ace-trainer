...
export function makeSession(questions: readonly CertificationQuestion[], selected: Record<string, string[]> = {}): CertificationExamViewModel {
  ...
  const session = createTrainingSession({ id: "exam-1", trackId: CLOUD_CERTIFICATION_TRACK_ID, modeId: "cloud-exam-simulation", requestedLength: questions.length, actualLength: questions.length, itemOrder: questions.map((question) => ({ trackId: CLOUD_CERTIFICATION_TRACK_ID, itemId: question.id, contentVersion: "test" })), optionOrderByItem: Object.fromEntries(questions.map((question) => [question.id, question.options.map((option) => option.id).reverse()])), activeForegroundMs: 0, contentVersion: "test", status: "active", startedAt });
  return { session, examState: { sessionId: session.id, deadlineAt: "2026-01-01T12:00:00.000Z", responsesByItemId: Object.fromEntries(Object.entries(selected).map(([itemId, selectedOptionIds]) => [itemId, { kind: "option_selection", selectedOptionIds }])), flaggedItemIds: questions[0] ? [questions[0].id] : [], currentItemId: questions[0]?.id } };
}

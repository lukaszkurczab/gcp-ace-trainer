...
export function makeSession(questions: readonly CertificationQuestion[], selected: Record<string, string[]> = {}): CertificationExamViewModel {
  ...
  const session = createTrainingSession({ id: "exam-1", trackId: CLOUD_CERTIFICATION_TRACK_ID, modeId: "cloud-exam-simulation", requestedLength: questions.length, actualLength: questions.length, currentItemIndex: 0, itemOrder: questions.map((question) => ({ trackId: CLOUD_CERTIFICATION_TRACK_ID, itemId: question.id, contentVersion: "test" })), optionOrderByItem: Object.fromEntries(questions.map((question) => [question.id, question.options.map((option) => option.id).reverse()])), activeForegroundMs: 0, contentVersion: "test", status: "active", startedAt });

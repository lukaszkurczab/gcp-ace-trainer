...
test("journal phase advances only one immutable step at a time", async () => {
  const record = await buildMutationJournal({ operation: "submit_training_outcome", sessionId: "s", trackId: "algorithms", identity: attempt.response, writes: [{ kind: "put_attempt", record: attempt }, { kind: "put_session", record: session }], createdAt: session.startedAt });
...
test("journal has a versioned SHA-256 command identity and rejects stale expected revisions", async () => {
  const record = await buildMutationJournal({ operation: "submit_training_outcome", sessionId: "s", trackId: "algorithms", identity: "stale", writes: [{ kind: "put_attempt", record: attempt }, { kind: "put_session", record: session }], createdAt: session.startedAt });

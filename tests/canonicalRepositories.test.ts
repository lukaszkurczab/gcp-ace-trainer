...
test("canonical repositories use individual immutable records and one active session", async () => {
 const ref = { trackId: "algorithms", itemId: "i", contentVersion: "v" }; const session = { id: "s", trackId: "algorithms", modeId: "m", configurationSnapshot: { kind: "practice" }, requestedLength: 1, actualLength: 1, currentItemIndex: 0, itemOrder: [ref], optionOrderByItem: {}, activeForegroundMs: 0, contentVersion: "v", status: "active" as const, startedAt: "2026-01-01T00:00:00.000Z" }; const attempt = { id: "a", sessionId: "s", trackId: "algorithms", modeId: "m", item: ref, response: {}, result: { kind: "correct" as const, earnedPoints: 1, maxPoints: 1 }, reviewEvidence: { sourceItem: ref, taxonomyOrSkillRefs: [] }, answeredAt: session.startedAt, committedAt: session.startedAt };
 await saveActiveTrackId("algorithms"); await saveTrainingSession(session); await addTrainingAttempt(attempt); await addTrainingAttempt(attempt);
...
test("training sessions reject regressing foreground time and missing active records", async () => {
 const ref = { trackId: "algorithms", itemId: "i", contentVersion: "v" };
 const session = { id: "s", trackId: "algorithms", modeId: "m", configurationSnapshot: { kind: "practice" }, requestedLength: 1, actualLength: 1, currentItemIndex: 0, itemOrder: [ref], optionOrderByItem: {}, activeForegroundMs: 500, contentVersion: "v", status: "active" as const, startedAt: "2026-01-01T00:00:00.000Z" };
...
test("terminal sessions are immutable except for identical recovery replay", async () => {
 const ref = { trackId: "algorithms", itemId: "i", contentVersion: "v" };
 const completed = { id: "done", trackId: "algorithms", modeId: "m", configurationSnapshot: { kind: "practice" }, requestedLength: 1, actualLength: 1, currentItemIndex: 0, itemOrder: [ref], optionOrderByItem: {}, activeForegroundMs: 500, contentVersion: "v", status: "completed" as const, startedAt: "2026-01-01T00:00:00.000Z", completedAt: "2026-01-01T00:01:00.000Z" };
...
test("attempt persistence rejects item and review-evidence identity mismatches", async () => {
 const ref = { trackId: "algorithms", itemId: "i", contentVersion: "v" };
 const base = { id: "a-invalid", sessionId: "s", trackId: "algorithms", modeId: "m", item: ref, response: {}, result: { kind: "correct" as const, earnedPoints: 1, maxPoints: 1 }, reviewEvidence: { sourceItem: ref, taxonomyOrSkillRefs: [] }, answeredAt: "2026-01-01T00:00:00.000Z", committedAt: "2026-01-01T00:00:00.000Z" };
 await assert.rejects(() => addTrainingAttempt({ ...base, item: { ...ref, trackId: "cloud-certification" } }), /invalid/);
...
test("review persistence rejects hidden identity replacement", async () => {
 const ref = { trackId: "algorithms", itemId: "i", contentVersion: "v" };
 const first = { id: "review-a", trackId: "algorithms", sourceAttemptId: "a", sourceSessionId: "s", sourceItem: ref, taxonomyOrSkillRefs: [], reasons: ["incorrect" as const], dueAt: "2026-01-02T00:00:00.000Z", createdAt: "2026-01-01T00:00:00.000Z", consecutiveAfterDueSuccesses: 0, persistent: true };

...
test("launch readiness CI is an explicit exact-SHA enforced gate", () => {
  ...
  assert.match(workflow, /RELEASE_REPORT_PATH: \$\{\{ runner\.temp \}\}\/launch-readiness-report\.json/);
  assert.match(workflow, /node scripts\/releaseGate\.mjs --enforce --output "\$RELEASE_REPORT_PATH"/);
  assert.doesNotMatch(workflow, /> launch-readiness-report\.json/);
  assert.match(workflow, /gate_status=\$\?/);
  assert.match(workflow, /JSON\.parse\(fs\.readFileSync\(process\.env\.RELEASE_REPORT_PATH, "utf8"\)\)/);
  ...
  assert.match(workflow, /path: \$\{\{ runner\.temp \}\}\/launch-readiness-report\.json/);

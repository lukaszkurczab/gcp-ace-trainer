...
for (const pattern of [/algorithmContentGroups/, /defaultQuestionBank/, /HttpContentSource/, /ContentCacheRepository/, /loadTrackContent/, /RemoteQuestionAdapter/, /ContentCompatibilityLayer/, /as unknown as/, /@ts-ignore/, /@ts-expect-error/]) if (pattern.test(source)) failures.push(`Forbidden application content architecture remains: ${pattern}`);

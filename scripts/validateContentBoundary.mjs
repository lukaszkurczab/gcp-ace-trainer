...
if (lock.schemaVersion !== 2 || !Array.isArray(lock.artifacts)) failures.push("The application content lock is invalid.");
  ...
  if (!generatedBundle.includes(`BUNDLED_CONTENT_BUNDLE_ID = ${JSON.stringify(lock.bundleId)}`)) failures.push("The generated bundle does not pin the canonical application content bundle.");

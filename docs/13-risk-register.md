...
| Visible family/category leaks implementation model | Medium | UI headings/filters expose family | Track-only surface and density tests across the eight launch descriptors. |
| Placeholder tracks create false launch scope | High | Empty/Coming soon production registry entry or post-launch brief admitted as launch | Shipping admission requires exactly eight real launch tracks, each with a real free vertical and full core loop. |
| Count targets distort content quality | High | Mass bank edits or a global question-count readiness gate | Preserve the accepted eight-track baseline; use targeted evidence for coverage, variation, validity, diagnostic value, provenance and profile behavior. |
| Review tooling becomes a second content authority | High | Console data disagrees with canonical source or automated signal is treated as approval | Keep the Console local/internal, bind review to identity and fingerprint, and require explicit human outcomes. |
| Profile capability is confused with inventory size | High | UI offers unsupported lengths or runtime fills from another unit | Resolve profile-specific subsets before the UI and fail unsupported requests explicitly. |
| Commerce is coupled to storefront SKU shape | High | Fixed access or recurring access cannot project to the same entitlement | Keep fixed-duration and recurring products behind one SKU-neutral account entitlement. |

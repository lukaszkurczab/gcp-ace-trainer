...
| AUD-FIXTURE / E01 | `partial` / CHECK: B go/no-go, potem READY | **Nowe:** A — kontrakt ochrony ownera i izolacji ustalony; B — konta testowe, seed, atestacja zachowania ownera i bezpieczny cleanup w istniejącym środowisku. Nie wymaga ukończenia całego AUD-17. | Powtarzalny dostęp do synthetic owner/guest, sesji, review i ekranów danych; dowód niezmienności chronionych danych ownera. SIM-READY. | [Raport A](active/AUD-FIXTURE/A-REPORT.md); nowa sekcja w pakiecie AUD (`PATTERNLY-AUDIT-TASKS-2026-09-22.md` — brak w workspace); kryteria w 6.1. |
...
| 04 | AUD-FIXTURE/B | Najpierw go/no-go: bezpieczna, lokalna atestacja chronionego markera bez ujawniania danych, przed przejściem guest na iPhonie. Następnie własne konta testowe, kontrolowane dane i cleanup; potwierdzenie, że zachowany owner nie został zmieniony. |

...
**Najbliższe działanie:** `CI-CONTRACT/A3` jest odebrane: historyczny release lock i bieżący builder mają osobne checkouty oraz ścisłą kontrolę pełnego SHA. Następnym dostępnym slice’em kontraktu jest `CI-CONTRACT/B`; wcześniejsze zadania produktowe pozostają WAIT na zapisane zależności. `B1b4c` nadal WAIT na decyzję PO o retry e-maila po niejednoznacznym wyniku SMTP.
...
| --- | --- | --- | --- | --- |
| CI-CONTRACT / E01/E08 | `partial` / A1, A2a, A2b, A3 done; READY: B | A2b wiąże push/PR z exact draft/readiness i acceptance v2, waliduje 9 Free-node packages oraz jawnie blokuje manual release na `not_granted`, bez deployu. A3 rozdziela historyczny release lock od exact SHA bieżącego buildera w app QA; B — włączyć testy kandydata do obowiązkowej bramki; C — uruchomić właściwy etap dla jawnych czterech SHA. | A2b i A3: PASS. B/C pozostają osobnymi slice; ponowny przebieg dla wydania do FREEZE. | [Raport A1](active/CI-CONTRACT/A1-REPORT.md); [A2a](active/CI-CONTRACT/A2a-REPORT.md); [A2b](active/CI-CONTRACT/A2b-REPORT.md); [A3](active/CI-CONTRACT/A3-REPORT.md). |

...
**Najbliższe działanie:** `AUD-04-B` jest odebrane lokalnie przez niezależne QA: backend udostępnia bezpośredni, zweryfikowany stream gzip dopiero po świeżym pozytywnym entitlement, a lokalny filesystem jest jawnie zabroniony w produkcji. Następny slice to `AUD-04-C` — mobile verify i atomowa aktywacja; semantyczny producent Premium oraz cloud storage nadal nie są przyjęte. `CI-CONTRACT/C` pozostaje otwarte bez lokalnego PASS z powodu braków środowiska i istniejącego `recovery:check`. `B1b4c` nadal WAIT na decyzję PO o retry e-maila po niejednoznacznym wyniku SMTP.
...
| ODK-119-GATE / E04 | `partial` / A done; B WAIT: AUD-04-C/D | A — jedna lokalna reguła przed startem nowej sesji: pełny plan, fresh online albo jawne offline, bez cache fallbacku po błędzie. Backendowy AUD-04-B jest gotowy; GATE/B obejmie wszystkie rzeczywiste wejścia sesji/download po AUD-04-C/D. Bez dodatkowych dni offline; realny provider osobno. | A: niezależne QA PASS. Pełne active/grace/odmowy/offline, expiry/refund/hold i ponowny dostęp z jednym właścicielem; provider E2E pozostaje późniejszą bramką. SIM-READY. | [Kontrakt ODK-119](active/ODK-119/ODK-119-WORKING-BRIEF.md), [raport A](active/ODK-119/A-REPORT.md). |
| AUD-04 / E04 | `partial` / B done; READY: C | A pozostaje ukończone kontraktowo. B (`patternly-backend` `11f988e`) dostarcza lokalny admission/download z App Check, świeżym entitlement, niezmiennymi bajtami i pointerem; 233/233 testów backendu oraz niezależne QA PASS. C — mobile verify/atomic activate; D — discovery/preparation. B nie przyjmuje jeszcze semantycznego producenta Premium, cloud storage ani wdrożenia. | Prawdziwy package lifecycle, integralność i atomowość, poprawne błędy i retencja historii. Wspólne testy z GATE/B bez duplikacji entitlementu. SIM-READY. | [Raport AUD-04](active/AUD-04/REPORT.md), [raport B](https://github.com/lukaszkurczab/patternly-backend/blob/11f988e/docs/active/AUD-04/B-REPORT.md). |

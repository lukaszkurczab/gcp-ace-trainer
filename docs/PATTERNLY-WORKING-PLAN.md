...
| --- | --- | --- |
| PROFILE-01 / `partial`, READY: C | `A` zamknął [kontrakt](active/PROFILE-01/CONTRACT.md) i [raport QA](active/PROFILE-01/A-REPORT.md); `B` dostarczył [testowaną granicę storage](active/PROFILE-01/B-REPORT.md), bez runtime PASS. `C` podłącza root startup i Auth przed repozytoriami; `D` dodaje trwałą lokalną blokadę/logout offline i jawne pending revoke. C–D to oddzielne taski z raportem, QA, zmianą planu i push. Obszary: `App`, storage/router, preferencje, bootstrap, `AccountSessionProvider`, nawigacja i kontrakt backendu. | Konto→jego dane tylko po zgodnym Auth, guest→jego postęp, świeża instalacja→welcome, logout/restart→login; brak Auth nie otwiera profilu konta. Offline logout lokalnie możliwy bez deklarowania zdalnego revoke. Odbiór C–D przed PROFILE-02/03. |
| PROFILE-02 / `planned`, WAIT: PROFILE-01 | Przebudować router i przejścia profili do jednego bieżącego gościa na instalację, z izolacją przestrzeni kont według uwierzytelnionego ID. Zachować istniejącego gościa; usunąć wielokrotne tworzenie gości i niepotrzebne legacy gałęzie po sprawdzeniu referencji. Obszary: `profileStorageRouter`, `mmkvClient`, repozytoria guest i testy storage. | Wielokrotne wejście w guest i restart wskazują te same dane; konto A, konto B i guest nie odczytują wzajemnie danych. Testy przejść, izolacji, restartu i awarii zapisu rejestru. |
...
| 03 | PROFILE-01/C → D | Kontrakt A i granica storage B [zamknięte](active/PROFILE-01/B-REPORT.md); kolejno root startup/Auth, lokalna blokada i pending revoke. Każdy slice ma własny raport, QA, zmianę planu i push na `main`. |
| 04 | PROFILE-02–05 | Kolejno jeden gość i izolacja kont, offline logout z zachowaniem danych konta, oba wybory adopcji, usunięcie starego oracle. Każdy slice zamknąć osobnym raportem, QA, zmianą planu i push na `main`. |

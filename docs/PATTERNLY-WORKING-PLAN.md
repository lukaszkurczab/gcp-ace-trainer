...
| AUD-FIXTURE / E01 | `partial` / READY: B2 | **Nowe:** A — kontrakt ochrony ownera i izolacji ustalony; B1 — lokalny oracle i rozdzielenie smoke/release wykonane; korekta niepotwierdzonego readback usuwa tylko własny wpis; B2 — preflight sesji/Auth, UI smoke i Maestro; B3 — własne konta, seed i bezpieczny cleanup. Nie wymaga ukończenia całego AUD-17. | Powtarzalny dostęp do synthetic owner/guest, sesji, review i ekranów danych; dowód niezmienności chronionych danych ownera. SIM-READY. | [Raport A](active/AUD-FIXTURE/A-REPORT.md), [raport B1](active/AUD-FIXTURE/B1-REPORT.md); nowa sekcja w pakiecie AUD (`PATTERNLY-AUDIT-TASKS-2026-09-22.md` — brak w workspace); kryteria w 6.1. |
...
| 04 | AUD-FIXTURE/B2 → B3 | B1 oracle i bundle: [raport](active/AUD-FIXTURE/B1-REPORT.md). B2: preflight przed skutkami sesji/Auth, UI tylko smoke i Maestro bez `clearState` na istniejącym iPhonie 17; przejście guest tylko po pozytywnej kontroli. B3: własne konta testowe, kontrolowane dane i cleanup; dowód zachowania ownera. |

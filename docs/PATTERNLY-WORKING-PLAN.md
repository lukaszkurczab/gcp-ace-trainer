...
**Najbliższe działanie:** `PROFILE-05` zakończone lokalnie; po niezależnym QA, commicie i pushu rozpocząć `PROFILE-06`. Oracle B1/B2 i jego UI/Metro/test harness usunięto, zachowując produkcyjny lock przejścia gościa, migrację istniejącego `legacy_owner` i backendowy B3a. `B1b4c` pozostaje WAIT na decyzję PO o retry e-maila po niejednoznacznym wyniku SMTP.

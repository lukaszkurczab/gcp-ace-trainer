...
**Najbliższe działanie:** `CI-CONTRACT/A2b`, następnie `AUD-FIXTURE/A`. `CI-CONTRACT/A2a` naprawił bramki źródłowe contentu; cały workflow pozostaje otwarty. `PLAN-SYNC/B2` udokumentował bieżące dowody, unknowns i właścicieli dalszych kontraktów; całe PLAN-SYNC pozostaje `partial` do usunięcia rozbieżności AWS-02 i ustalenia dowodu SIMP-05. Nie wracać do kolejnych prób tych samych niedostępnych flow bez usunięcia ich wspólnej blokady. Brak danych prawnych, fizycznego iPhone’a lub ręcznego testu Androida nie zatrzymuje prac lokalnych i symulatorowych.
...
| CI-CONTRACT / E01/E08 | `partial` / READY: A2b | A1 — jawny testowy profil smoke dla clean prebuild wykonany; A2a — źródłowe build/scoring 9 tracków zastąpiły brakujące komendy; A2b — wdrożyć rzeczywiste bramki candidate readiness, review packets i manual release, bez pozorowania admission; A3 — rozdzielić historyczny release lock od exact SHA bieżącego buildera w app QA; B — włączyć testy kandydata do obowiązkowej bramki; C — uruchomić właściwy etap dla jawnych czterech SHA. A3 wymaga rozstrzygnięcia źródła SHA. Nie mylić naprawy A1 z zielonym całym CI. | Po A2b nieistniejące komendy nie występują w workflow; celowo uszkodzony manifest daje FAIL; końcowa agregacja `continue-on-error` egzekwuje FAIL. C do SIM-READY, ponowny przebieg dla wydania do FREEZE. | [Przegląd, §3](PATTERNLY-REPO-PLAN-ADDITIONS.md#add-ci-contract--zgodność-pipelineu-z-rzeczywistymi-komendami-repo); [raport A1](active/CI-CONTRACT/A1-REPORT.md); [raport A2a](active/CI-CONTRACT/A2a-REPORT.md); raport i QA kolejnych slice. |
...
Jeśli izolacja wymaga niewielkiej poprawki istniejącego profile routing/storage, wykonać ją jako jawny prerequisite z testami, przed pełnym E2E AUD-17. Nie tworzyć cyklu „fixture czeka na pełny AUD-17, a AUD-17 czeka na fixture”. Błąd uprawnień narzędzia rozwiązywać w dozwolonym środowisku; nie obchodzimy zabezpieczeń ani nie przyznajemy narzędziom niepotrzebnego dostępu.
...
| 02 | CI-CONTRACT/A2b | Wdrożyć rzeczywiste bramki readiness/review i kontrakt manual release. Źródłowe bramki A2a: [raport](active/CI-CONTRACT/A2a-REPORT.md). A1 clean prebuild: [raport](active/CI-CONTRACT/A1-REPORT.md). |

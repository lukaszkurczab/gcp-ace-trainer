...
| AUD-08 / E05 | `partial` / READY: A0; A WAIT: A0 | [Diagnoza](active/AUD-08/A-DIAGNOSIS.md) potwierdza brak `operationId`/ACK i stratę kodu po błędzie. A0 — mapa backendu, bezpośrednich reguł Firebase i klientów dla monotonicznej generacji autoryzacji; następnie A — Codex uzgadnia wspólny kontrakt recovery/reissue, B — implementacja po briefingu i QA. Odpowiedź PO o UX może informować projekt, ale nie jest bramką techniczną. | Retry po awarii, restart klienta, utracona odpowiedź, provider failure, reissue/deletion i ochrona tokenu; pełny kontrakt, nie backend-only patch. SIM-READY. | [Diagnoza](active/AUD-08/A-DIAGNOSIS.md); [historyczny raport backendu](https://github.com/lukaszkurczab/patternly-backend/blob/0ea62f7/docs/active/AUD-08/REPORT.md). |
...
| 09 | AUD-08/A0 — READY | [Diagnoza](active/AUD-08/A-DIAGNOSIS.md) kieruje do mapy wszystkich granic autoryzacji i testu wykonalności generacji; po nim Codex kończy protokół A. Bez kierowania technicznej decyzji do PO. |
| 10 | AUD-08/B — WAIT: A0/A | Implementacja mobile/backend i testy awarii dopiero po kompletnym, zwalidowanym kontrakcie A. |

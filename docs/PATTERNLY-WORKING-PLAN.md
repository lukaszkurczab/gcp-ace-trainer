...
**Najbliższe działanie:** `AUD-08/B1c` jest lokalnie odebrane (QA PASS WITH ISSUES), a `SIMP-05` ponownie zweryfikowano jako `done`; następnym dostępnym krokiem jest `AWS-02/CANDIDATE`. `B1b4c` WAIT na decyzję PO o retry e-maila po niejednoznacznym wyniku SMTP. `CI-CONTRACT/A2b` czeka na aktualny kontrakt kandydata i nie może być naprawiany przez odtworzenie legacy publishera. Pozostałe statusy i zależności obowiązują według rejestru i kolejki poniżej; brak danych prawnych, fizycznego iPhone’a lub ręcznego testu Androida nie zatrzymuje prac lokalnych i symulatorowych.

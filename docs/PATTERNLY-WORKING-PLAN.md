...
| 04 | AUD-FIXTURE/B3a — done; B3b/B2b-device — WAIT | B1 oracle i bundle: [raport](active/AUD-FIXTURE/B1-REPORT.md). B2a strzeżone przejście: [raport](active/AUD-FIXTURE/B2a-REPORT.md). B2b kod: [raport](active/AUD-FIXTURE/B2b-REPORT.md), w tym restartowy wynik, cleanup, UI smoke i test konkurencji; urządzenie Guest, brak atestacji ownera. B3a: własne konto i dane w izolowanym projekcie emulatora, cleanup po ID: [raport](active/AUD-FIXTURE/B3a-REPORT.md), [rozpoznanie](active/AUD-FIXTURE/B3-FEASIBILITY.md). B3b/B2b-device: WAIT na wspieraną ścieżkę syntetycznego `legacy_owner` na tym samym iPhonie; dopiero wtedy wznowić Maestro bez `clearState` i zebrać dowód. |

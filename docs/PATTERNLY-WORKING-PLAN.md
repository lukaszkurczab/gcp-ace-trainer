...
**Najbliższe działanie:** `CI-CONTRACT/B` jest odebrane: testy kandydata są obowiązkowe w exact-SHA gate, a aplikacyjny test A3 ma poprawne historyczne i bieżące wejścia. Następnym dostępnym slice’em kontraktu jest `CI-CONTRACT/C`; wcześniejsze zadania produktowe pozostają WAIT na zapisane zależności. `B1b4c` nadal WAIT na decyzję PO o retry e-maila po niejednoznacznym wyniku SMTP.

...
**Najbliższe działanie:** `AWS-02/CANDIDATE` jest lokalnie odebrane i wypchnięte jako `b0341ad`; publishing/runtime admission oraz app lock pozostają osobnym krokiem. Następne dostępne zadanie to `CI-CONTRACT/A2b`, które ma podłączyć rzeczywiste readiness/review/manual-release bez odtwarzania legacy publishera. `B1b4c` nadal WAIT na decyzję PO o retry e-maila po niejednoznacznym wyniku SMTP. Pozostałe statusy i zależności obowiązują według rejestru i kolejki poniżej.
...
| --- | --- | --- | --- | --- |
| CI-CONTRACT / E01/E08 | `partial` / READY: A2b | A1 — jawny testowy profil smoke dla clean prebuild wykonany; A2a — źródłowe build/scoring 9 tracków zastąpiły brakujące komendy; A2b — podłączyć readiness v2 z `b0341ad`, review packets i manual release bez pozorowania admission; A3 — rozdzielić historyczny release lock od exact SHA bieżącego buildera w app QA; B — włączyć testy kandydata do obowiązkowej bramki; C — uruchomić właściwy etap dla jawnych czterech SHA. | Po A2b nieistniejące komendy nie występują w workflow; celowo uszkodzony manifest daje FAIL; końcowa agregacja `continue-on-error` egzekwuje FAIL. C do SIM-READY, ponowny przebieg dla wydania do FREEZE. | [Przegląd, §3](PATTERNLY-REPO-PLAN-ADDITIONS.md#add-ci-contract--zgodność-pipelineu-z-rzeczywistymi-komendami-repo); [raport A1](active/CI-CONTRACT/A1-REPORT.md); [raport A2a](active/CI-CONTRACT/A2a-REPORT.md); [diagnoza A2b](active/CI-CONTRACT/A2b-DIAGNOSIS.md); raport i QA kolejnych slice. |
...
| --- | --- | --- |
| 02 | CI-CONTRACT/A2b — READY | Wdrożyć rzeczywiste bramki readiness/review i kontrakt manual release dla kandydata `b0341ad`; nie nadawać admission. [Diagnoza zależności](active/CI-CONTRACT/A2b-DIAGNOSIS.md); źródłowe bramki A2a: [raport](active/CI-CONTRACT/A2a-REPORT.md). |

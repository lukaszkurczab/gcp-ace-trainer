...
| AUD-FIXTURE / E01 | `partial` / READY: B2b | **Nowe:** A — kontrakt ochrony ownera i izolacji ustalony; B1 — lokalny oracle i rozdzielenie smoke/release wykonane; B2a — strzeżone przejście guest po preflight sesji/Auth, wynik `pending`; B2b — weryfikacja po restarcie, cleanup, UI smoke i Maestro; B3 — własne konta, seed i bezpieczny cleanup. Nie wymaga ukończenia całego AUD-17. | Powtarzalny dostęp do synthetic owner/guest, sesji, review i ekranów danych; dowód niezmienności chronionych danych ownera. SIM-READY. | [Raport A](active/AUD-FIXTURE/A-REPORT.md), [raport B1](active/AUD-FIXTURE/B1-REPORT.md), [raport B2a](active/AUD-FIXTURE/B2a-REPORT.md); nowa sekcja w pakiecie AUD (`PATTERNLY-AUDIT-TASKS-2026-09-22.md` — brak w workspace); kryteria w 6.1. |
...
| 04 | AUD-FIXTURE/B2b → B3 | B1 oracle i bundle: [raport](active/AUD-FIXTURE/B1-REPORT.md). B2a strzeżone przejście: [raport](active/AUD-FIXTURE/B2a-REPORT.md). B2b: weryfikacja po restarcie, cleanup tylko własnego wpisu, UI tylko smoke i Maestro bez `clearState` na istniejącym iPhonie 17; test konkurencji zwykłego i strzeżonego polecenia. B3: własne konta testowe, kontrolowane dane i cleanup; dowód zachowania ownera. |

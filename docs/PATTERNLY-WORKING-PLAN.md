...
| --- | --- | --- |
| PROFILE-01 / `planned`, READY | Spisać i zaimplementować jedną maszynę stanów startu, dostępu oraz trwały znacznik pierwszego uruchomienia/wylogowania. Oddzielić lokalne odcięcie dostępu od zdalnego unieważnienia; ustalić bezpieczne ponowienie tego ostatniego dla dokładnie tego samego konta. Obszary: `AccountSessionProvider`, `RootNavigator`, `AccountEntryScreen`, `profileStorageRouter`, lifecycle oraz kontrakt backendu. | Testy stanów: sesja konta→jego dane, guest→jego postęp, świeża instalacja→welcome, logout/restart→login. Brak sesji nie otwiera profilu konta. Lokalny logout offline jest możliwy; brak sieci nie jest przedstawiany jako udane zdalne unieważnienie. Raport kontraktu i QA przed PROFILE-02/03. |
...
| 03 | PROFILE-01 | Kontrakt stanów, trwałego dostępu i zdalnego revoke zgodnie z §6.1; osobny briefing i QA przed implementacją. |

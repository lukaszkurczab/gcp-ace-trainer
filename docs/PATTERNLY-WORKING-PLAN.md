...
**Najbliższe działanie:** `PROFILE-06/F` zakończone lokalnie i przyjęte przez niezależne QA jako PASS WITH ISSUES; `PROFILE-06/E` pozostaje WAIT na dokładną decyzję PO o UX częściowego odzyskania, a pozostały odzyskany zakres PROFILE-06 wymaga konsolidacji końcowej macierzy. Po pushu F przejść do następnego niezależnego zadania. `B1b4c` pozostaje WAIT na decyzję PO o retry e-maila po niejednoznacznym wyniku SMTP.
...
| PROFILE-06 / `partial`, active; F done, E WAIT/PO | Odbiór integracyjny iOS na istniejącym iPhonie 17. F wykonało pełną reautoryzację i hold-to-delete na izolowanym koncie; naprawiono utratę `authorizationGeneration` po password reauth, a podpisany fixture potwierdził exact Auth/Firestore deletion przed cleanupem. E czeka na decyzję PO o uczciwym stanie częściowego odzyskania; pozostałą macierz skonsolidować z odzyskanych dowodów A–D bez ich fikcyjnego zaliczania. | F: backend fixture 10/10, app account/session/Firebase 92/92 i Maestro terminal 3/3 PASS; QA PASS WITH ISSUES. Pełny app typecheck nadal FAIL na 20 równoległych błędach locale; terminalny marker revoke pozostaje jawny. [Kontrakt](active/PROFILE-06/CONTRACT.md), [raport F](active/PROFILE-06/F-REPORT.md). SIM-READY. |

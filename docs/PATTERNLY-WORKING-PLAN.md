...
**Rewizja:** 26 września 2026; decyzje PO z 23–26.09 oraz uzgodnienie [przeglądu repozytoriów](PATTERNLY-REPO-PLAN-ADDITIONS.md).<br>
...
**Najbliższe działanie:** `PROFILE-06/B`, C, D i F zakończone lokalnie; następny jest PROFILE-06/E. Decyzja PO dla E: przy nieodzyskanym planie otworzyć Home z zachowanym stanem i komunikatem, że planu nie odzyskano i trzeba utworzyć go ponownie; nie przedstawiać braku jako pustego planu ani nie nadpisywać nim danych zdalnych. `B1b4c` pozostaje WAIT na decyzję PO o retry e-maila po niejednoznacznym wyniku SMTP.
...
| Niepełne odzyskanie danych — decyzje PO 24–26.09 | Opcja 1: dla potwierdzonego konta nieodzyskany plan nie blokuje poprawnie odzyskanego Home. Aplikacja zachowuje track/cel/postęp i pokazuje zamykalny komunikat, że planu nie odzyskano i trzeba utworzyć go ponownie. Brak planu nie jest pustym planem, tombstone'em ani zgodą na nadpisanie zdalnych danych. Zamknięcie komunikatu przeżywa restart dla tego samego incydentu; nowy plan kończy incydent. Brak pewnego właściciela albo brak możliwości odseparowania danych nadal prowadzi do blokady/loginu. |
...
| Lokalizacja | `partial` | EN/PL istnieją; DE/FR/ES/IT/ET i pochodzenie tekstów wymagają pracy. Wstępna inwentaryzacja: 1334 wpisy top-level, 1521 liści, 8 namespace i bezpośrednie literały. |
...
| PROFILE-REDESIGN / E01 | `partial` / READY: PROFILE-06 | PROFILE-01–05 done: kontrakt stanów, jeden gość i izolacja kont, wylogowanie offline, oba trwałe wybory adopcji oraz usunięta syntetyczna ścieżka oracle B1/B2. PROFILE-06 — odbiór na jednym iPhonie 17. Zależności i kryteria w §6.1. | Start, restart, zmiana konta i wylogowanie nie ujawniają cudzych danych; pending zapis wznawia się tylko po logowaniu na to samo konto; oba wybory gościa przeżywają przerwanie. SIM-READY. | Decyzja PO 24.09 w §2.3; aktualny kod `AccountSessionProvider`, `profileStorageRouter`, `accountDataService`, `RootNavigator` i testy adopcji; raport i QA każdego slice. |
...
| AUD-17 / E01 | `blocking` / active through PROFILE-06 | PROFILE-02–04 są done; historyczny owner-mismatch zastępuje bieżąca macierz rzeczywistych stanów PROFILE-06. Skonsolidować guest write/restart, adopcję transfer/reset, logout offline, restart i powrót tego samego konta oraz odmowę innemu kontu. F deletion jest odebrane, ale samo nie zamyka AUD-17. | Dowód braku mieszania profili i utraty pending postępu; nie zamykać na Account Entry, samym buildzie ani samym F. SIM-READY. | [Raport AUD-17](active/AUD-17/REPORT.md) jest historyczny; nowy raport i QA zgodny z PROFILE-06. |
...
| AUD-11 / E05 | `done` / QA PASS | Fixture `__DEV__` + smoke umożliwił bezpieczny odbiór bez mutacji storage. Przed/po PL/EN, duży tekst, neutralna hierarchia usuwania, stan limitu, anulowanie hold i semantyka przeszły na istniejącym iPhonie 17. | Lokalny odbiór zakończony. Fixture jest dowodem prezentacji, nie rzeczywistej awarii storage; VoiceOver nie został fikcyjnie zaliczony i pozostaje dokładnym przypadkiem ODK-088, jeśli będzie wymagany fizycznie. SIM-READY. | [Raport AUD-11](active/AUD-11/REPORT.md). |
...
| AWS-02 / E03 | `done` / PASS WITH ISSUES | Exact candidate `11d56baa…`, decyzja/readiness v2, osobny admission v3, exact app lock i runtime evidence są gotowe lokalnie. Publishing oznacza wyłącznie zweryfikowane lokalne bajty; brak deployu/storage. | CANDIDATE i ADMISSION: PASS WITH ISSUES. Pierwszy FAIL admission zamknięto dokładnym per-track bindingiem; re-QA PASS WITH ISSUES. | [Raport candidate](https://github.com/lukaszkurczab/patternly-content/blob/b0341ad/reports/candidate-reconciliation/AWS-02-REPORT.md), [raport admission](active/AWS-02/ADMISSION-REPORT.md). |
...
| ODK-117 / E06 | `blocking` / A0, A1 i B-CONTRACT done; A2 WAIT: linguistic review | A2 obejmuje 7 605 nowych wartości w pięciu pakietach UI. Brak app draftów, procesu i kompetentnych recenzentów; modelowy self-review nie spełnia kontraktu. Nie generować ani nie promować niezweryfikowanych tłumaczeń. A3 pozostaje za A2; niezależnie kontynuować PROFILE-03. | Pięć kompletnych pakietów `de/fr/es/it/et`, pełny parytet oraz audytowalny, kompetentny przegląd per język. Potem A3–A5. B-VALUES nadal osobno do FREEZE. | [Pakiet odblokowania A2](active/ODK-117/A2-REVIEW-BLOCKER.md), [mapa A0](active/ODK-117/A0-SOURCE-MAP.md), [raport A1](active/ODK-117/A1-REPORT.md), [raport B-CONTRACT](active/ODK-117/B-CONTRACT-APP-REPORT.md), [raport](ODK-117-REPORT.md). |

### 5.3 Konfiguracja, web i wydanie

| ID / epik | Status / dostępność | Najbliższy wynik i zależności | Warunek zamknięcia / bramka | Kontrakt i raport |
| --- | --- | --- | --- | --- |
| OPS-PRODUCTION / E05/E07 | `partial` / A done QA PASS; B1 done QA PASS WITH ISSUES; READY: B2 | A potwierdziło, że odczytane repozytoria nie zawierają kanału produkcyjnego. B1 (`patternly-backend` `e25c28a`) daje fail-closed OIDC/JWKS, HMAC pseudonim i exact-action allowlistę bez fallbacku do Firebase. B2 — guard i endpointy nad istniejącymi store’ami; B3 — lokalne CLI; B4 — syntetyczny odbiór. Bez hosted admina i bez bezpośredniego Firestore. C — kontrolowany dostęp w docelowym środowisku. | A/B: synthetic request → autoryzowana obsługa → właściwy dla rodziny wynik i audyt, z odmową dostępu oraz bezpiecznym read-after-uncertain; efekty zewnętrzne bez dowodu są `AMBIGUOUS`, bez automatycznego retry. C: kontrolowany dowód produkcyjny przed GO. | [Raport A](active/OPS-PRODUCTION/A-REPORT.md), [raport B1](https://github.com/lukaszkurczab/patternly-backend/blob/e25c28a/docs/active/OPS-PRODUCTION/B1-REPORT.md), [przegląd, §3](PATTERNLY-REPO-PLAN-ADDITIONS.md#add-ops-production--droga-od-zgłoszenia-użytkownika-do-działania-operatora). |
...
| PROFILE-02 / `done` — A i C done, B cancelled by owner | [A: router ponownie używa jednego znanego Gościa](active/PROFILE-02/A-REPORT.md). B nie tworzy selektora, odzyskiwania ani migracji wielu historycznych Gości. [C](active/PROFILE-02/C-REPORT.md) potwierdza jeden kanoniczny Guest i izolację Guest/A/B na wielu kluczach, restartach i operacjach storage; runtime nie wymagał zmiany. | Testy dowodzą ciągłości ID/danych i izolacji Guest/A/B. Maestro potwierdza zachowanego Gościa po restarcie bez `clearState`; pełne A/B E2E pozostaje w PROFILE-06. |
| PROFILE-03 / `done`, QA PASS WITH ISSUES | Logout/revoke/resume używa exact completed receipt w jedynym kanonicznym formacie v2 i scoped cleanupu; format v1 jest odrzucany, nie migrowany; kolejne wylogowanie rotuje operationId. | Backend po integracji 244/244; app targeted 90/90 i 121/121; typecheck/OpenAPI/diff PASS. Świeży Maestro na istniejącym iPhonie 17: pending po logout i restarcie, same-account resume do Home, ponowny restart zachowuje Home/track bez markerów pending. Nieblokujące: pięć wcześniejszych błędów pełnego suite app, brak produkcyjnego provider E2E i osobnego urządzeniowego outbox/journal. [Kontrakt](active/PROFILE-03/CONTRACT.md), [raport](active/PROFILE-03/REPORT.md). |
| PROFILE-04 / `done` | Wybór adopcji przy **zakładaniu** konta ma jeden trwały stan: transfer do konta i sync albo porzucenie guest i start od zera. Dokładna promocja zakończonego profilu pozwala później utworzyć jednego nowego gościa bez dostępu do danych konta. | Oba wybory przeżyły natywny restart na iPhonie 17; transfer zachował cel i plan bez duplikacji, discard nie wysłał porzuconego postępu, a testy pokrywają konflikt, przerwania i offline retry. [Raport](active/PROFILE-04/REPORT.md). |
| PROFILE-05 / `done` | Usunięto kod oracle B1/B2 wraz z UI smoke, kopiami, hookami storage, martwymi testami i aliasami Metro. Zachowano produkcyjny lock równoczesnych przejść, migrację istniejącego `legacy_owner` oraz backendowy B3a. | Brak aktywnych symboli oracle; lock ma osobny test konkurencji i zwolnienia. Targetowane 71/71, pełny suite 1268/1274 i smoke iOS export PASS dla zakresu; 6 wcześniejszych błędów pełnego suite i typecheck rozszerzonych locale są niezależne. Release selection Metro 2/2 PASS. [Raport](active/PROFILE-05/REPORT.md). |
| PROFILE-06 / `partial`, active; B/C/D/F done, E READY | B: fresh Guest zachowuje Coding Interview, cel i plan przez natywny restart. C: transfer zachował dokładnie track/cel/plan raz, a discard pozostawił konto bez rekordów Gościa. D: logout offline zachował pending A, B pozostało izolowane, a A po wznowieniu API wysłało własną parę goal+plan i zachowało Home przez restart. F: pełna reautoryzacja i exact Auth/Firestore deletion. E ma decyzję opcji 1: Home z zachowanym stanem i komunikatem o nieodzyskanym planie wymagającym ponownego utworzenia. | D: Maestro offline/restart + B isolation + A recovery/restart PASS; exact-account A 5→5 podczas B i 5→7 dopiero po powrocie A, B stale revision 1; targeted 50/50 PASS; QA w raporcie. Pełny app typecheck nadal FAIL na równoległych błędach locale. [Kontrakt](active/PROFILE-06/CONTRACT.md), [B](active/PROFILE-06/B-REPORT.md), [C](active/PROFILE-06/C-REPORT.md), [D](active/PROFILE-06/D-REPORT.md), [F](active/PROFILE-06/F-REPORT.md). SIM-READY. |
...
| 04 | PROFILE-02–05 — PROFILE-02/03/04 done; następny PROFILE-05 | PROFILE-04 ma dowód Maestro obu trwałych wyborów, restartów, zakończonego transferu/discard oraz jednego nowego gościa po adopcji. Każdy slice zamknąć osobnym raportem, QA, zmianą planu i push na `main`. |
...
| 18 | ODK-119-GATE/B — done / PASS WITH ISSUES | Wszystkie rzeczywiste wejścia do nowej sesji i downloadu mają jednego właściciela; usunięto nieużywane `prepareSession` i ogólny installer, dodano wykonywalny kontrakt wejść oraz macierz active/grace/negative/unavailable/offline. Backendowy test emulatora potwierdza świeżą kontrolę przed odczytem bajtów. QA PASS WITH ISSUES wyłącznie przez brak realnego RevenueCat/store E2E. Bez wdrożenia. [Raport](active/ODK-119/B-REPORT.md). |
| 19 | AWS-02/ADMISSION — done | Aktualne lokalne publishing/runtime evidence oraz app lock odebrane QA PASS WITH ISSUES; bez fałszywego historycznego ACC-02 i bez twierdzenia o wdrożeniu. [Raport](active/AWS-02/ADMISSION-REPORT.md). |
...
| 25 | ODK-117-A1 done → A2 WAIT: linguistic review → A3 → A4 → A5 | A1 odebrane. A2 wymaga pięciu kompetentnych przeglądów językowych dla 7 605 wartości; brak procesu/reviewerów, a modelowy self-review nie jest dowodem. Nie rozpoczynać A3 przed zweryfikowaniem pakietów A2. Kontynuować niezależny PROFILE-03. |
...
| 28c | RELEASE-CONTRACT/C — done | Release jest `embedded-only`; manifest wiąże politykę, a physical receipt musi odpowiadać dokładnemu manifestowi/buildowi/runtime przed GO. Bez publikacji OTA. [Raport](active/RELEASE-CONTRACT/C-REPORT.md). |
| 28d | OPS-PRODUCTION/A — done | Repozytoria nie wykazują kanału produkcyjnego; przyjęto kontrakt lokalnego CLI, OIDC/allowlisty, istniejących state machines i read-after-uncertain bez fałszywej idempotencji. Niezależne QA PASS. [Raport](active/OPS-PRODUCTION/A-REPORT.md). Następny slice: B1. |
| 28e | OPS-PRODUCTION/B1 — done | Backend ma oddzielny opcjonalny profil OIDC operatora, exact-action allowlistę, pseudonim HMAC i przypięty publiczny JWKS z limitami, deadline’em i ochroną refreshu. Bez tras B2 i bez wdrożenia. QA PASS WITH ISSUES. [Raport](https://github.com/lukaszkurczab/patternly-backend/blob/e25c28a/docs/active/OPS-PRODUCTION/B1-REPORT.md). Następny slice: B2. |

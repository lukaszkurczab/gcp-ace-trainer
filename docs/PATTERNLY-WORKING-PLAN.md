...
| --- | --- | --- |
| PROFILE-01 / `partial`, READY: B | `A` zamknął [kontrakt](active/PROFILE-01/CONTRACT.md) i [raport QA](active/PROFILE-01/A-REPORT.md) po potwierdzeniu odczytu scope przed Auth. `B` oddziela metadane od aktywacji profilu; `C` podłącza root startup i Auth przed repozytoriami; `D` dodaje trwałą lokalną blokadę/logout offline i jawne pending revoke. B–D to oddzielne małe taski z raportem, QA, zmianą planu i push. Obszary: `App`, storage/router, preferencje, bootstrap, `AccountSessionProvider`, nawigacja i kontrakt backendu. | Konto→jego dane tylko po zgodnym Auth, guest→jego postęp, świeża instalacja→welcome, logout/restart→login; brak Auth nie otwiera profilu konta. Offline logout lokalnie możliwy bez deklarowania zdalnego revoke. Odbiór B–D przed PROFILE-02/03. |
...
| 03 | PROFILE-01/B → C → D | Kontrakt A [zamknięty](active/PROFILE-01/CONTRACT.md); kolejno granica odczytu storage, root startup/Auth, lokalna blokada i pending revoke. Każdy slice ma własny raport, QA, zmianę planu i push na `main`. |

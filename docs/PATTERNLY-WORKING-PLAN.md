...
**Najbliższe działanie:** `CI-CONTRACT/A2b` jest odebrane i wypchnięte jako content `21707b6`; manual release poprawnie zatrzymuje się na braku admission i nie wdraża. Następnym dostępnym slice’em tego kontraktu jest `CI-CONTRACT/A3`; wcześniejsze zadania produktowe pozostają WAIT na zapisane zależności. `B1b4c` nadal WAIT na decyzję PO o retry e-maila po niejednoznacznym wyniku SMTP.
...
| --- | --- | --- | --- | --- |
| CI-CONTRACT / E01/E08 | `partial` / A1, A2a, A2b done; READY: A3 | A2b wiąże push/PR z exact draft/readiness i acceptance v2, waliduje 9 Free-node packages oraz jawnie blokuje manual release na `not_granted`, bez deployu. A3 — rozdzielić historyczny release lock od exact SHA bieżącego buildera w app QA; B — włączyć testy kandydata do obowiązkowej bramki; C — uruchomić właściwy etap dla jawnych czterech SHA. | A2b: PASS. A3/B/C pozostają osobnymi slice; ponowny przebieg dla wydania do FREEZE. | [Raport A1](active/CI-CONTRACT/A1-REPORT.md); [A2a](active/CI-CONTRACT/A2a-REPORT.md); [A2b](active/CI-CONTRACT/A2b-REPORT.md). |

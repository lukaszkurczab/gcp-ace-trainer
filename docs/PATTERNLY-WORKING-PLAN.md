...
**Najbliższe działanie:** `CI-CONTRACT/A2`, następnie `AUD-FIXTURE/A`. `PLAN-SYNC/B2` udokumentował bieżące dowody, unknowns i właścicieli dalszych kontraktów; całe PLAN-SYNC pozostaje `partial` do usunięcia rozbieżności AWS-02 i ustalenia dowodu SIMP-05. Nie wracać do kolejnych prób tych samych niedostępnych flow bez usunięcia ich wspólnej blokady. Brak danych prawnych, fizycznego iPhone’a lub ręcznego testu Androida nie zatrzymuje prac lokalnych i symulatorowych.
...
| CI-CONTRACT / E01/E08 | `partial` / READY: A2 | A1 — jawny testowy profil smoke dla clean prebuild wykonany; A2 — uzgodnić komendy content workflow z aktualnymi właścicielskimi bramkami; A3 — rozdzielić historyczny release lock od exact SHA bieżącego buildera w app QA; B — włączyć testy kandydata do obowiązkowej bramki; C — uruchomić właściwy etap dla jawnych czterech SHA. A3 wymaga rozstrzygnięcia źródła SHA. Nie mylić naprawy A1 z zielonym całym CI. | Nieistniejące komendy nie występują w workflow; celowo uszkodzony manifest daje FAIL; końcowa agregacja `continue-on-error` egzekwuje FAIL. C do SIM-READY, ponowny przebieg dla wydania do FREEZE. | [Przegląd, §3](PATTERNLY-REPO-PLAN-ADDITIONS.md#add-ci-contract--zgodność-pipelineu-z-rzeczywistymi-komendami-repo); [raport A1](active/CI-CONTRACT/A1-REPORT.md); raport i QA kolejnych slice. |
...
| 02 | CI-CONTRACT/A2 | Aktualne komendy workflow i scripts contentu, bez przywracania legacy tylko dla CI. A1 clean prebuild: [raport](active/CI-CONTRACT/A1-REPORT.md). |

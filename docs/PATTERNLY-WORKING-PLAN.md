...
**Najbliższe działanie:** `AUD-04-D` ma naprawioną i testowaną ścieżkę offer → exact install/preparation → Gate A, ale pozostaje `WAIT/DEVICE`: zachowany `account-remote-revoke-pending` na jedynym iPhonie 17 blokuje wymagany dowód Maestro success/denial, a stanu nie wolno usuwać ani przejmować poza bezpiecznym protokołem recovery. Kod nie jest przyjęty jako ukończony. W czasie tej blokady należy wykonać pierwszy niezależny dostępny slice z kolejki. Semantyczny producent Premium, cloud storage, admission i wdrożenie nadal nie są przyjęte. `CI-CONTRACT/C` ma lokalny exact-SHA PASS WITH ISSUES bez hosted runu i wdrożenia, a `B1b4c` nadal WAIT na decyzję PO o retry e-maila po niejednoznacznym wyniku SMTP.
...
| RELEASE-CONTRACT / E08 | `partial` / A–B done; READY: C | A rozdziela local, FREEZE i GO. B wiąże w jednym manifeście v2 cztery SHA, locki, dokładny iOS build, fingerprint publicznej konfiguracji i integralną kopertę signing evidence. C — określić i wdrożyć politykę OTA. | A: QA PASS; B: QA PASS WITH ISSUES — rzeczywisty build i pochodzenie evidence pozostają do potwierdzenia przed FREEZE. Brak provider/device evidence nie obala FREEZE; brak fizycznego ODK-088 obala GO. C pozostaje wymagane przed FREEZE. | [Raport A](active/RELEASE-CONTRACT/A-REPORT.md); [raport B](active/RELEASE-CONTRACT/B-REPORT.md); [przegląd, §3](PATTERNLY-REPO-PLAN-ADDITIONS.md#add-release-contract--bramki-etapowe-i-tożsamość-przebadanego-wydania). |

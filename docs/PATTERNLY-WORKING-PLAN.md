...
**Najbliższe działanie:** `PROFILE-06/B` i F zakończone lokalnie z niezależnym QA PASS WITH ISSUES; następny jest PROFILE-06/C — osobne transfer i discard z exact Firestore assertions. `PROFILE-06/E` pozostaje WAIT na dokładną decyzję PO o UX częściowego odzyskania. `B1b4c` pozostaje WAIT na decyzję PO o retry e-maila po niejednoznacznym wyniku SMTP.
...
| PROFILE-06 / `partial`, active; B/F done, E WAIT/PO | B: fresh Guest zachowuje Coding Interview, cel i plan przez natywny restart bez Account Entry/onboardingu. F: pełna reautoryzacja i hold-to-delete na izolowanym koncie; naprawiono utratę `authorizationGeneration`, a podpisany fixture potwierdził exact Auth/Firestore deletion. Następny C: osobne transfer/discard z exact backend assertions. E czeka na decyzję PO. | B: Maestro runtime + targeted 35/35, QA PASS WITH ISSUES. F: backend fixture 10/10, app 92/92 i terminal 3/3, QA PASS WITH ISSUES. Pełny app typecheck nadal FAIL na równoległych błędach locale. [Kontrakt](active/PROFILE-06/CONTRACT.md), [B](active/PROFILE-06/B-REPORT.md), [F](active/PROFILE-06/F-REPORT.md). SIM-READY. |

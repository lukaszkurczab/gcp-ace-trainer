...
| ODK-082–087 / E08 | `planned` / WAIT: FREEZE + dostęp do danej bramki | Odczytać właściwy kontrakt i wykonywać jedną bramkę naraz. Konta testowe przygotowuje Codex. Rzeczywistą niedostępność właścicielską kierować do PO-ACCESS. | Realne, przenośne dowody dla tych samych SHA/buildu/konfiguracji. Nie odgadywać przypisania ID providerów; znany z planu ODK-085 obejmuje wspólny test z ODK-119. PROVIDER. | Istniejące pakiety znaleźć w PLAN-SYNC; brakujący pakiet sporządzić przed wykonaniem. |
...
`blocking` dla zakończenia B: brak pięciu historycznych pakietów wskazanych w tej rewizji (`PATTERNLY-AUDIT-TASKS-2026-09-22.md`, `PATTERNLY-AUDIT-2026-09-22.md`, `ODK-116-INPUTS-PACKET.md`, `AUD-01-REPORT.md`, `IDENTITY-CUTOVER-2026-09-22.md`); wymagają odnalezienia/odtworzenia z dowodów, a nie pozornego linku. Walidator candidate readiness nadal wymaga `humanApproval`; B ustala właściwy kontrakt delegowanej decyzji, a implementację kieruje do AWS-02. Niespójne komendy CI są wejściem do `CI-CONTRACT/A`.
...
- `PLAN-SYNC` mapuje `ODK-082–087` na istniejące kontrakty, wejścia i dowody. W szczególności uzgadnia obsługę operatorską z `OPS-PRODUCTION`, bez dodatkowej równoległej bramki.

...
**Najbliższe działanie:** `CI-CONTRACT/A`, następnie `AUD-FIXTURE/A`. `PLAN-SYNC/B2` udokumentował bieżące dowody, unknowns i właścicieli dalszych kontraktów; całe PLAN-SYNC pozostaje `partial` do usunięcia rozbieżności AWS-02 i ustalenia dowodu SIMP-05. Nie wracać do kolejnych prób tych samych niedostępnych flow bez usunięcia ich wspólnej blokady. Brak danych prawnych, fizycznego iPhone’a lub ręcznego testu Androida nie zatrzymuje prac lokalnych i symulatorowych.
...
| CI-CONTRACT / E01/E08 | `planned` / READY | A — porównać workflow, komendy, skrypty i ścieżki checkout; B — włączyć testy kandydata do obowiązkowej bramki; C — uruchomić właściwy etap dla jawnych czterech SHA. Wejścia A: content CI zgłasza brak `authoring:validate`; app QA używa historycznego SHA contentu bez `scripts/build.mjs` w teście bieżącego buildera, a recovery prebuild nie ma `PATTERNLY_RUNTIME_MODE`. To błędy do uzgodnienia, nie zielony dowód CI. | Nieistniejące komendy nie występują w workflow; celowo uszkodzony manifest daje FAIL; końcowa agregacja `continue-on-error` egzekwuje FAIL. C do SIM-READY, ponowny przebieg dla wydania do FREEZE. | [Przegląd, §3](PATTERNLY-REPO-PLAN-ADDITIONS.md#add-ci-contract--zgodność-pipelineu-z-rzeczywistymi-komendami-repo); raport i QA każdego slice. |

...
**Najbliższe działanie:** `AUD-FIXTURE/B3b` — po zgodzie użytkownika przygotować jawną ścieżkę syntetycznego legacy ownera tylko dla builda smoke, a następnie osobny świeży iPhone 17 do urządzeniowego B2b; zachować istniejący Guest iPhone bez zmian. [Plan B3b](active/AUD-FIXTURE/B3b-PLAN.md). B3a przygotował izolowany fixture backendowy. `AUD-FIXTURE/A` ustalił granice izolacji i bezpiecznego cleanupu; B zaczyna się od go/no-go dla atestacji zachowanego ownera przed przejściem guest na urządzeniu. `CI-CONTRACT/A2b` czeka na aktualny kontrakt `AWS-02/CANDIDATE` dla readiness i review; kontrola samego draftu nie spełnia A2b. `CI-CONTRACT/A2a` naprawił bramki źródłowe contentu; cały workflow pozostaje otwarty. `PLAN-SYNC/B2` udokumentował bieżące dowody, unknowns i właścicieli dalszych kontraktów; całe PLAN-SYNC pozostaje `partial` do usunięcia rozbieżności AWS-02 i ustalenia dowodu SIMP-05. Nie wracać do kolejnych prób tych samych niedostępnych flow bez usunięcia ich wspólnej blokady. Brak danych prawnych, fizycznego iPhone’a lub ręcznego testu Androida nie zatrzymuje prac lokalnych i symulatorowych.
...
| --- | --- | --- | --- | --- |
| WP-E01 — środowisko i bezpieczne testowanie | `partial` | Powtarzalna droga do wszystkich wymaganych testowych stanów iOS bez uszkodzenia profilu właściciela. | PLAN-SYNC/B, CI-CONTRACT, AUD-FIXTURE, AUD-02, AUD-17. | Fixture z kontami testowymi, bezpiecznym cleanup i dowodem zachowania ownera; pełny owner/guest E2E; lokalne środowisko odebrane. |
...
| AUD-FIXTURE / E01 | `partial` / B3a done; IN PROGRESS: B3b/B2b-device | **Nowe:** A — kontrakt ochrony ownera i izolacji ustalony; B1 — lokalny oracle i rozdzielenie smoke/release wykonane; B2a — strzeżone przejście guest po preflight sesji/Auth, wynik `pending`; B2b — kod weryfikacji po restarcie, cleanup i UI smoke wykonany; urządzeniowy owner→guest czeka na syntetyczny owner baseline; B3a — własne konto i dane postępu/review w izolowanym projekcie emulatora oraz cleanup po identyfikatorach wykonane; B3b/B2b-device — zatwierdzona ścieżka smoke na odrębnym świeżym iPhonie 17; istniejący Guest iPhone pozostaje nienaruszony. Nie wymaga ukończenia całego AUD-17. | Powtarzalny dostęp do synthetic owner/guest, sesji, review i ekranów danych; dowód niezmienności chronionych danych ownera. SIM-READY. | [Raport A](active/AUD-FIXTURE/A-REPORT.md), [raport B1](active/AUD-FIXTURE/B1-REPORT.md), [raport B2a](active/AUD-FIXTURE/B2a-REPORT.md), [raport B2b](active/AUD-FIXTURE/B2b-REPORT.md), [rozpoznanie B3](active/AUD-FIXTURE/B3-FEASIBILITY.md), [raport B3a](active/AUD-FIXTURE/B3a-REPORT.md), [plan B3b](active/AUD-FIXTURE/B3b-PLAN.md); nowa sekcja w pakiecie AUD (`PATTERNLY-AUDIT-TASKS-2026-09-22.md` — brak w workspace); kryteria w 6.1. |
| AUD-02 / E01 | `partial` / READY: pozostały zakres lokalny | Raportowany local baseline obejmuje build i lokalne bramki, ale driver-form values i RC session flow nie mają odbioru; kontynuować na izolowanym fixture i przypiąć aktualny build do source SHA. Aktualny exact candidate/admission kierować do AWS-02. Nie wymaga FREEZE ani realnych sklepów. | Odbiór lokalnego flow i manifest/preflight dla 9 tracków. Historyczny test 8 tracków nie pokrywa 9. Wszystkie release-only dowody wydzielone do ODK-082–087. SIM-READY. | Pakiet AUD (`PATTERNLY-AUDIT-TASKS-2026-09-22.md` — brak w workspace), [raport AUD-02](active/AUD-02/REPORT.md). |
| AUD-17 / E01 | `blocking` / WAIT: AUD-FIXTURE | Wykonać owner-mismatch→guest, guest write/restart, powrót ownera i adopcję na syntetycznych danych. Istniejącą poprawkę kluczy przyjąć jako lokalny baseline. | Pełne E2E bez uszkodzenia chronionego stanu. Nie zamykać na Account Entry ani samym buildzie. SIM-READY. | Pakiet AUD (`PATTERNLY-AUDIT-TASKS-2026-09-22.md` — brak w workspace), [raport AUD-17](active/AUD-17/REPORT.md). |
...
### 6.1 AUD-FIXTURE: testy bez niszczenia zachowanego profilu

Celem nie jest nowy framework testowy, lecz odblokowanie konkretnych AUD-17/13/05/11/14/15. Tworzenie kont jest uprawnieniem do przygotowania środowiska, nie dowodem rozwiązania izolacji lokalnego storage.

| Slice | Zakres | Kryteria akceptacji |
| --- | --- | --- |
| AUD-FIXTURE/A | Odczytać istniejące narzędzia, scope storage i ownership. Zidentyfikować chroniony stan ownera oraz istniejące synthetic fixture. Zaprojektować najmniejszy bezpieczny mechanizm. | Znane granice zapisu i cleanup; możliwy dowód zachowania ownera bez ujawniania danych/sekretów. Brak bezpiecznego odczytu jest konkretnym zadaniem diagnostycznym, nie powodem do resetu. |
| AUD-FIXTURE/B | Utworzyć własne konta testowe i kontrolowane stany sesji/odpowiedzi/review/danych. Użyć rzeczywistych ścieżek produktu oraz jawnego seedowania wyłącznie testowych danych. | Powtarzalne wejście w stan, zapis i restart; dowód niezmienności chronionych bajtów/markerów ownera; cleanup tylko zasobów utworzonych do testu. Mechanizm nie trafia jako aktywny bypass do produkcji. |

Nie wolno używać `clearState`, globalnego resetu zachowanych danych ani dodatkowego symulatora jako obejścia. Nie dopisywać testowych odpowiedzi do chronionego profilu. Tworzenie i przełączanie **odizolowanych kont/scope testowych** jest dozwolone; zakaz dotyczy naruszenia zachowanego ownera, nie samego faktu utworzenia konta.

Po A brak gotowej atestacji chronionego ownera na urządzeniu. B zaczyna od sprawdzenia wykonalności lokalnego, wyłącznie testowego wyniku `unchanged/changed` dla markera przed/po guest i restart; brak bezpiecznej metody zatrzymuje przejście guest na urządzeniu, a test pamięciowy nie jest dowodem natywnym. Po pozytywnej kontroli dopuszczalne są tylko minimalne zapisy rejestru przejścia i nowego scope testowego, nigdy chronionego profilu ownera. Jeśli izolacja wymaga niewielkiej poprawki istniejącego profile routing/storage, wykonać ją jako jawny prerequisite z testami, przed pełnym E2E AUD-17. Nie tworzyć cyklu „fixture czeka na pełny AUD-17, a AUD-17 czeka na fixture”. Błąd uprawnień narzędzia rozwiązywać w dozwolonym środowisku; nie obchodzimy zabezpieczeń ani nie przyznajemy narzędziom niepotrzebnego dostępu.
...
| 03 | AUD-FIXTURE/A — done | Bezpieczny kontrakt chronionego ownera i izolacji: [raport](active/AUD-FIXTURE/A-REPORT.md). |
| 04 | AUD-FIXTURE/B3b/B2b-device — active | B1 oracle i bundle: [raport](active/AUD-FIXTURE/B1-REPORT.md). B2a strzeżone przejście: [raport](active/AUD-FIXTURE/B2a-REPORT.md). B2b kod: [raport](active/AUD-FIXTURE/B2b-REPORT.md), w tym restartowy wynik, cleanup, UI smoke i test konkurencji; urządzenie Guest, brak atestacji ownera. B3a: własne konto i dane w izolowanym projekcie emulatora, cleanup po ID: [raport](active/AUD-FIXTURE/B3a-REPORT.md), [rozpoznanie](active/AUD-FIXTURE/B3-FEASIBILITY.md). B3b/B2b-device: [plan](active/AUD-FIXTURE/B3b-PLAN.md) smoke-only seed przed migracją na nowej instancji iPhone 17, potem Maestro bez `clearState` z wynikiem po natywnym restarcie; istniejący iPhone bez zmian. |
| 05 | AUD-17 | Pełny owner→guest→owner, zapis, restart, adopcja. |
...
| 07 | AUD-02 — pozostały zakres lokalny | Formularze sterownika i lokalne RC. Części wymagające aktualnego contentu zakończyć po AWS-02; nie tworzyć zależności od freeze. |
...
| 11 | ODK-116-A | Schema zmiennych, platformowa walidacja iOS/Android i środowisko testowe. Gotowe wejście dla tłumaczeń dokumentów oraz web. |
...
| --- | --- | --- |
| Konto testowe nie rozwiązuje izolacji lokalnych danych | Jawny fixture contract, kontrola niezmienności ownera, brak globalnego resetu. | Codex, AUD-FIXTURE. |

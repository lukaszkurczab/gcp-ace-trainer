...
**Najbliższe działanie:** `PROFILE-06/B`, C i F zakończone lokalnie; następny jest PROFILE-06/D — logout offline oraz izolacja i odzyskanie kont A/B z exact-account assertions. `PROFILE-06/E` pozostaje WAIT na dokładną decyzję PO o UX częściowego odzyskania. `B1b4c` pozostaje WAIT na decyzję PO o retry e-maila po niejednoznacznym wyniku SMTP.
...
| PROFILE-06 / `partial`, active; B/C/F done, E WAIT/PO | B: fresh Guest zachowuje Coding Interview, cel i plan przez natywny restart. C: transfer zachował dokładnie track/cel/plan raz, a discard pozostawił konto bez rekordów Gościa; oba wybory przetrwały restart przed i po zatwierdzeniu. F: pełna reautoryzacja i exact Auth/Firestore deletion. Następny D: logout offline, izolacja B i odzyskanie A. E czeka na decyzję PO. | B: Maestro + targeted 35/35, QA PASS WITH ISSUES. C: dwa pełne flow Maestro + exact-account Auth/Firestore inspection, QA PASS WITH ISSUES; prywatny inspector nie jest odtwarzalnym artefaktem repo. F: fixture 10/10, app 92/92 i terminal 3/3, QA PASS WITH ISSUES. Pełny app typecheck nadal FAIL na równoległych błędach locale. [Kontrakt](active/PROFILE-06/CONTRACT.md), [B](active/PROFILE-06/B-REPORT.md), [C](active/PROFILE-06/C-REPORT.md), [F](active/PROFILE-06/F-REPORT.md). SIM-READY. |

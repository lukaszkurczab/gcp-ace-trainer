...
16. `bc09d63` zamknął bezpieczną geometrię Practice Hub, `65aeccd` zamknął bezpieczną geometrię Practice Setup, a `256717e` domknął source-level geometrię Practice Summary względem `750:6235`. Te slice'y nie zmieniają trybów, route'ów, komend ani danych; w podsumowaniu usunięto wyłącznie nadmiarową prezentację `missed/points`, której nie ma w Figma. Następny task to owner-bound `DES-005-C`; dopiero po tej decyzji można zmieniać semantic contract Custom Practice.

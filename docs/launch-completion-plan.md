...
16. `bc09d63` zamknął bezpieczną geometrię Practice Hub, `65aeccd` zamknął bezpieczną geometrię Practice Setup, `256717e` domknął source-level geometrię Practice Summary względem `750:6235`, `cc7cdf5` dopasował kolejną bezpieczną warstwę Progress względem `842:9563`, a `ee7dde1` dopasował metadata i separatory zagnieżdżonej Activity względem `842:11192`. Te slice'y nie zmieniają trybów, route'ów, komend ani danych; w podsumowaniu usunięto wyłącznie nadmiarową prezentację `missed/points`, której nie ma w Figma, a w Progress/Activity nie dodano brakujących celów, effectiveness ani paginacji. Następny task semantyczny to owner-bound `DES-005-C`; dopiero po tej decyzji można zmieniać semantic contract Custom Practice.

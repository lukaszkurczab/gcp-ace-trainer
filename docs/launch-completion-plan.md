...
16. `bc09d63` zamknął bezpieczną geometrię Practice Hub, `65aeccd` zamknął bezpieczną geometrię Practice Setup, `256717e` domknął source-level geometrię Practice Summary względem `750:6235`, a `cc7cdf5` dopasował kolejną bezpieczną warstwę Progress względem `842:9563`. Te slice'y nie zmieniają trybów, route'ów, komend ani danych; w podsumowaniu usunięto wyłącznie nadmiarową prezentację `missed/points`, której nie ma w Figma, a w Progress nie dodano brakujących celów ani effectiveness. Następny task semantyczny to owner-bound `DES-005-C`; dopiero po tej decyzji można zmieniać semantic contract Custom Practice.

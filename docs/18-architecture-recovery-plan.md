...
# Etap 0 — Recovery baseline, CI i removal inventory

## Cel

Zbudować dokładny, automatycznie sprawdzalny obraz repozytorium przed pierwszym usunięciem kodu.

## Warunki wejścia

- dokumenty `00–17` są zsynchronizowane;
- ten plan jest zaakceptowany;
- repozytorium jest dostępne na aktualnym `main`.
...
- inventory screenów;
- inventory services;
- inventory storage APIs;
- inventory routes;
- inventory tests;
- inventory keys;
- dependency graph;
- test i CI baseline;
- lista design references;
- lista gates G-01–G-05.

## Wymagane zmiany

1. Zarejestrować wszystkie importy i użycia:
   - AsyncStorage;
   - `localStorage`;
   - starych keys;
   - `Question`;
...
2. Uruchomić:
   ...
   - `npm run qa:static`.
3. Zweryfikować GitHub Actions:
   - jeden workflow;
   - PR i `main`;
   - `qa:static`;
   - czytelny failure output.
4. Dodać target contract fixtures niezależne od Reacta:
   - canonical result classification;
   - adjacent ordering;
   - complexity dimensions;
   - review success rules;
   - journal idempotency.
5. Utworzyć machine-readable removal inventory.

## Wymagane usunięcia

- martwe testy, które nie uruchamiają aktywnego kodu;
- puste skrypty QA;
- duplikaty CI.

Nie usuwać jeszcze aktywnych runtime paths.

## Testy

- cały istniejący test suite;
- CI smoke;
- inventory check;
- documentation consistency smoke;
- baseline report z exit codes.

## Kryteria zakończenia

- baseline jest zielony albo każda istniejąca awaria ma opis i ownera;
- istnieje jedna automatyczna bramka PR;
- inventory obejmuje wszystkie stare pathy;
- każdy kolejny prompt może wskazać exact files;
- brakujące designs są blockerami, nie otwartymi zadaniami Codexa.

## Zakazane interpretacje

- nie poprawiać runtime w tym etapie;
- nie dodawać nowego architecture scaffoldingu;
- nie oznaczać nieznanych ścieżek jako „do późniejszej migracji” bez wpisania ich do inventory;
- nie wyciszać istniejących test failures.

## Stan repo po etapie

Zachowanie produktu pozostaje bez zmian. Repo ma wiarygodną bramkę i pełny removal inventory.

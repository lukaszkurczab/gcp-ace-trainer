...
| `SimulationNavigator` | Exactly 40 numbered occurrence controls. It exposes current, durably answered, unanswered, and frozen states only.                                                                 |
...
| ID                                          | Runtime state                                            | Visible composition and user meaning                                                                                                                                                          | Actions and semantics                                                        |
| ------------------------------------------- | -------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ---------------------------------------------------------------------------- |
| S-01 · Preparing                            | `preparing`                                              | Stable shell with reserved timer and counter slots, `Preparing Interview Simulation`, and `Checking the required 40 unique items and creating your draft.` Navigator and controls are absent. | No semantic `1 of 40` or active countdown before verification.               |
| S-02 · Insufficient content                 | preparation failure                                      | Stable shell with `The required 40 simulation items could not be prepared.` It states that no shorter session was created.                                                                    | Only explicit return or retry supplied by the application.                   |
| S-03 · Editable unanswered                  | `active.simulation.editable` with no response            | Prompt, neutral controls, navigator with exactly 40 positions, durability row `No saved response`.                                                                                            | No correctness, score, Reason, Details, or review state.                     |
...
| S-09 · Navigator inventory                  | `active.simulation.editable`                             | All 40 numbered controls in a compact grid. Current = violet outline and inset treatment.                                                                                                     | Grid semantics expose number and state.                                      |
...
| Timer                | Elapsed active foreground time.                              | 45-minute active-foreground countdown. Background time does not reduce it.           |
...
7. The navigator always contains exactly 40 positions and never exposes correctness or flags.

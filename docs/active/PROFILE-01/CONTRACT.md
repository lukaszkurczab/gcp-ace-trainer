...
2. Start otwiera wyłącznie zaszyfrowaną bazę i metadane kontrolne. Nie udostępnia profilu ani nie montuje preferencji, repozytoriów, content bootstrap lub lifecycle przed decyzją dostępu. Odtworzenie Firebase Auth i kontrola trwałej blokady następują przed aktywacją scope.
...
| `PROFILE-01/B` | Rozdzielenie kontrolnego otwarcia storage od aktywacji profilu i minimalny ponowny wybór istniejącego guest ID. | Test: po restarcie z wybranym kontem odczyty scope nie występują przed zgodnym UID; zachowany guest wraca do tych samych danych bez utworzenia nowego profilu; awaria metadanych kończy się jawną odmową. |
| `PROFILE-01/C` | Root startup coordinator, Auth i stan welcome/login/guest/account przed repozytoriami. | Testy przejść, UID mismatch, restartu, błędu Auth i kolejności montowania; tylko właściwy scope uruchamia content bootstrap. |

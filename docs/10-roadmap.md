...
5. **Commercial entitlement** — SKU-neutral fixed-duration and recurring Premium access, RevenueCat/backend authority, restore/conflict, downgrade and seven-day offline verification grace.
...
13. **Eight-track content quality and targeted correction** — preserve the accepted launch banks, use evidence-driven corrections, maintain profile-specific session capability, and publish immutable changes only when a real defect requires them.
14. **Content Review Console** — local/internal source review, advisory risk and coverage signals, exact identity/fingerprint tracking, explicit human outcomes and no second content authority.
15. **Platform/public/store hardening** — Expo SDK 57, target mobile matrix, links/domain/sender/legal/support surfaces, perceived quality, accessibility, performance, store assets, signing and release-compatible evidence; physical-device testing is optional and non-blocking.
16. **Release closure** — clean signed artifacts, production provider/store state, complete gates, whole-product audit and explicit GO/NO-GO.

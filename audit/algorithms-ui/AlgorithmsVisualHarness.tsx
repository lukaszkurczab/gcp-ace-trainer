...
function practiceProjection(fixture: AlgorithmsVisualFixture): PracticeSessionSurfaceProps {
  ...
  const feedback = fixture.id === "P-06" || fixture.id === "P-07" || fixture.id === "P-08" || fixture.id === "P-10" ? { reason: item.feedback.reason, details: item.feedback.details, result: "incorrect" as const } : undefined;
  return {
    ...
    question: phase === "preparing" ? undefined : { itemId: item.id, prompt: item.prompt, constraints: item.constraints, responseControl: practiceControl(item, fixture.id) },
...
function simulationProjection(fixture: AlgorithmsVisualFixture): SimulationSurfaceProjection {
  ...
  const timer = fixture.operation === "expired" ? "00:00" : "31:42";
  const base = { state, title: unavailable ? "Interview Simulation unavailable" : "Interview Simulation", modeLabel: "Interview Simulation", position: unavailable || fixture.operation === "preparing" ? undefined : { accessibilityLabel: "Question 1 of 40", label: "1 of 40" }, progress: unavailable || fixture.operation === "preparing" ? undefined : 0.025, timer: fixture.operation === "preparing" ? undefined : { accessibilityLabel: `Time remaining ${timer}`, label: timer }, notice: { tone: unavailable || fixture.operation.includes("failed") || fixture.operation === "stale_revision" ? "error" as const : "neutral" as const, message: unavailable ? "The canonical simulation state is unavailable; no substitute session was created." : fixture.operation === "editable_unsaved" ? "Not saved yet" : fixture.operation === "editable_saved" ? "Saved" : fixture.operation === "saving" ? "Saving…" : fixture.operation === "expired" ? "Time expired. Freezing your latest saved draft." : "Immutable application projection fixture." }, navigator: unavailable || fixture.operation === "preparing" ? undefined : positions };

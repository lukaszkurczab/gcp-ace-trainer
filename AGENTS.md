...
- For PROFILE simulator acceptance, the owner has authorized clearing Patternly app data on the existing iPhone 17 now and in future runs when needed for an isolated test. Do not ask again for that exact cleanup. This does not authorize deleting repository files, production data, external user accounts, emulator backend data outside the task fixture, or creating/reinstalling a duplicate app/device without a concrete need.

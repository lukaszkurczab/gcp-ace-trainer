...
export type AlgorithmSelectionScope = Readonly<{ mentalUnitId?: string; recognitionSetId?: string; contrastSetId?: string; interleavedScopeId?: string; simulationProfileId?: string }>;
...
export function selectAlgorithmSessionPlan(input: SelectAlgorithmSessionItemsInput): AlgorithmSessionSelection {
  ...
  const blueprint = catalog.bank.practiceBlueprints.find((entry) => entry.modeId === input.mode); if (!blueprint) throw new Error(`Algorithms bank has no blueprint for ${input.mode}.`);
...
function applyExplicitScope(mode: AlgorithmModeId, scope: AlgorithmSelectionScope | undefined, catalog: AlgorithmContentCatalog, items: readonly AlgorithmQuestion[]): readonly AlgorithmQuestion[] {
  if (mode === ALGORITHM_MODE_IDS.learnApproach || mode === ALGORITHM_MODE_IDS.guidedPractice) { const mentalUnitId = scope?.mentalUnitId; if (!mentalUnitId) throw new Error(`Algorithms ${mode} requires an explicit mental unit.`); return items.filter((item) => item.taxonomy.primaryMentalUnitId === mentalUnitId); }

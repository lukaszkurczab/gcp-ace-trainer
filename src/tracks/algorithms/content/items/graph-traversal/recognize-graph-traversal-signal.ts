...
export const recognizeGraphTraversalSignalQuestions = [
  {
    ...
    "id": "alg-graph-traversal-naming-001",
    "learningStage": "pattern_mechanics",
    "primarySkillAtomId": "track_graph_visited_state",
    "prompt": "A task gives roads between cities and asks which cities are reachable from a start city. Which signal matters first?",
    "roadmapNodeId": "graph_traversal",
    "staticMicroChecks": [
      {
        "correctAnswer": "adjacency_and_visited",
        "feedback": "Reachability requires neighbor representation and visited state.",
        "id": "alg-graph-traversal-naming-001-check",
        "mistakeTypes": ["data_structure_mismatch", "cannot_trace_algorithm"],
        "options": [
          { "id": "adjacency_and_visited", "text": "Represent neighbors and track visited nodes." },
          { "id": "sort_values", "text": "Sort city names alphabetically." },
          { "id": "ignore_visited", "text": "Traverse without marking visited nodes." },
          { "id": "single_index", "text": "Use only one numeric index as the state." }
        ],
        "prompt": "Choose the graph traversal signal.",
        "status": "active",
        "testedSkillAtomIds": ["track_graph_visited_state"],
        "type": "single_choice"
      }
    ],
    "status": "active",
    "taxonomyRefs": [
      { "axisId": "pattern_family", "nodeId": "graph_traversal", "role": "primary" },
      { "axisId": "skill_atom", "nodeId": "track_graph_visited_state", "role": "primary" },
      { "axisId": "pattern_variant", "nodeId": "visited_state", "role": "secondary" },

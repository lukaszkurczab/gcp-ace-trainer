export const streamingOnlineAndDynamicUpdatesQuestions = [
  {
    ...
    type: "single_choice",
    prompt:
      "Values arrive one at a time. After every arrival starting with the kth value, you must immediately output the sum of the latest k values. Which strategy best matches the contract?",
    options: [
      {
        id: "rolling_window",
    ...
    feedbackModel: {
      decisionSignal:
        "The required answer concerns one evolving suffix of fixed length and must be produced online.",
      mentalModelCorrection:
        "A rolling window directly models incoming and expiring contributions. Delaying work until the stream ends violates the output contract.",
      mistakeTypes: ["online_contract_mismatch"],
      nextAction:
        "Check whether each answer must be available before the complete input exists.",
      result: "diagnostic",
    },
  },
  {
    ...
    type: "single_choice",
    prompt:
      "An input stream may continue indefinitely, and the system only needs the average of the latest 100 values. What is the main drawback of retaining one prefix value for every arrival?",
    options: [
      {
    ...
    correctOptionId: "history_grows_without_bound",
    feedbackModel: {
      decisionSignal:
        "The output depends only on a bounded recent suffix, while cumulative history grows with all arrivals.",
      mentalModelCorrection:
        "Prefix history can represent the answer, but it retains more state than the contract requires for an unbounded stream.",
      mistakeTypes: ["unnecessary_history_storage"],
      nextAction:
        "Compare required retention with total input history, not only whether a representation is mathematically sufficient.",
      result: "diagnostic",
    },
  },
  {
    ...
    type: "single_choice",
    prompt:
      "A prefix-sum array is built for a static array. Later, values[3] changes before another range query is answered. Why may the old prefix array be incorrect?",
    options: [
      {
    ...
    correctOptionId: "later_prefixes_are_stale",
    feedbackModel: {
      decisionSignal:
        "A point update changes every cumulative state whose covered prefix includes the updated position.",
      mentalModelCorrection:
        "Plain prefix preprocessing represents one version of the array. Without maintenance or rebuilding, later queries use stale cumulative values.",
      mistakeTypes: ["stale_preprocessed_state"],
      nextAction:
        "Identify which stored cumulative states include the updated index.",
      result: "diagnostic",
    },
  },
  {
    ...
    prompt:
      "How do append-only changes differ from point updates for a stored prefix-sum sequence?",
    options: [
      {
    ...
    correctOptionId: "append_extends_update_invalidates_suffix",
    feedbackModel: {
      decisionSignal:
        "Appending does not alter any previously covered prefix, while editing the past changes cumulative states after that position.",
      mentalModelCorrection:
        "Prefix sums can be extended incrementally for append-only data. Their static weakness concerns modifications to already represented positions.",
      mistakeTypes: ["dynamic_update_misclassification"],
      nextAction:
        "Ask whether the change extends the data or modifies history already included in stored cumulative states.",
      result: "diagnostic",
    },
  },
  {
    ...
    type: "single_choice",
    prompt:
      "A stream processor reports the count of errors among the latest 1,000 events. No historical query is ever allowed. Which retention policy best matches the contract?",
    options: [
      {
    ...
    correctOptionId: "retain_active_window",
    feedbackModel: {
      decisionSignal:
        "Only the active bounded suffix can affect current or future outputs.",
      mentalModelCorrection:
        "Once an event expires and no historical query can request it, keeping its full history is unnecessary.",
      mistakeTypes: ["retention_contract_mismatch"],
      nextAction:
        "Determine when past data can no longer influence any permitted output.",
      result: "diagnostic",
    },
  },
  {
    ...
    type: "solution_comparison",
    prompt:
      "A system retains only the latest k values in a rolling window. A new requirement asks for the sum of an arbitrary historical range that expired long ago. Can the current window state answer it?",
    options: [
      {
    ...
    correctOptionId: "no_history_discarded",
    feedbackModel: {
      decisionSignal:
        "Rolling state intentionally forgets contributions outside the active range.",
      mentalModelCorrection:
        "A bounded window supports current rolling outputs, not arbitrary historical reconstruction after the source values have been discarded.",
      mistakeTypes: ["state_capability_overclaim"],
      nextAction:
        "Verify that the retained state contains all information required by every permitted query.",
      result: "diagnostic",
    },
  },
  {
    ...
    type: "single_choice",
    prompt:
      "Which statement correctly contrasts changing one active-window boundary with editing an earlier element represented by prefix sums?",
    options: [
      {
    ...
    correctOptionId: "local_vs_global_effect",
    feedbackModel: {
      decisionSignal:
        "The representations encode different scopes: one current range versus cumulative history at many boundaries.",
      mentalModelCorrection:
        "Local rolling state changes with the active boundary. Prefix state after an edited position contains globally stale cumulative information.",
      mistakeTypes: ["state_update_scope_mismatch"],
      nextAction:
        "Identify how many stored states include the changed contribution.",
      result: "diagnostic",
    },
  },
  {
    ...
    type: "solution_comparison",
    prompt:
      "Values are appended over time, and the system must support arbitrary range-sum queries over all values received so far. No existing value is ever modified. Which assessment is most accurate?",
    options: [
      {
    ...
    correctOptionId: "extend_prefix_history",
    feedbackModel: {
      decisionSignal:
        "Appends preserve all earlier cumulative boundaries, and historical queries require retaining those boundaries.",
      mentalModelCorrection:
        "Prefix sums are not inherently offline. They can grow incrementally for append-only data, but their memory usage grows with retained history.",
      mistakeTypes: ["online_prefix_misconception"],
      nextAction:
        "Separate whether input is complete from whether earlier cumulative states remain valid.",
      result: "diagnostic",
    },
  },
  {
    ...
    type: "solution_comparison",
    prompt:
      "A developer builds prefix sums once, then supports point updates to the array while continuing to answer queries from the unchanged prefix array. The query operation is still O(1). Which review is correct?",
    options: [
      {
    ...
    feedbackModel: {
      decisionSignal:
        "Complexity says nothing about whether stored state reflects the current data version.",
      mentalModelCorrection:
        "An O(1) query over stale preprocessing is still wrong. Correctness must be restored through maintenance, rebuilding, or a different representation.",
      mistakeTypes: [
        "correctness_before_complexity",
        "stale_preprocessed_state",
      ],
      nextAction:
        "Validate preprocessed-state freshness before evaluating operation complexity.",
      result: "diagnostic",
    },
  },
  {
    ...
    type: "solution_comparison",
    prompt:
      "Which explanation best distinguishes rolling-window state from prefix state under streaming and changing-data constraints?",
    options: [
      {
    ...
    feedbackModel: {
      decisionSignal:
        "The correct comparison covers retained history, update scope, append behavior, expiration, and supported query contracts.",
      mentalModelCorrection:
        "The strategies are not separated simply into online and offline. Their key difference is what historical state they preserve and how changes affect that state.",
      mistakeTypes: ["weak_strategy_justification"],
      nextAction:
        "Justify the strategy through retention, update behavior, output timing, and historical-query requirements.",
      result: "diagnostic",
    },
  },
];

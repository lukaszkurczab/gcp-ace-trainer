export const fixedSizeWindowContractQuestions = [
  {
    ...
    type: "single_choice",
    prompt:
      "In a fixed-size sliding window with size k, what do left and right represent?",
    options: [
      {
    ...
    feedbackModel: {
      decisionSignal:
        "The problem asks about every contiguous range containing exactly k elements.",
      mentalModelCorrection:
        "The two indexes are not independent candidates. Together they describe one window with a fixed length.",
      mistakeTypes: ["independent_pointer_interpretation"],
      nextAction:
        "Write the invariant right - left + 1 === k for every window that contributes a result.",
      result: "diagnostic",
    },
  },
  {
...
Which sequence correctly processes each complete window?`,
    ...
    feedbackModel: {
      decisionSignal:
        "The current complete window must be measured before its outgoing element is removed.",
      mentalModelCorrection:
        "The incoming contribution completes the new window. Record that window first, then prepare the state for the next one.",
      mistakeTypes: ["fixed_window_update_order_mismatch"],
      nextAction:
        "Separate processing the current window from preparing the next window.",
      result: "diagnostic",
    },
  },
  {
...
What sums should be recorded, in order?`,
    ...
    feedbackModel: {
      decisionSignal:
        "There are two complete contiguous windows: [5, 2, 4] and [2, 4, 1].",
      mentalModelCorrection:
        "Moving to the second window adds 1 but also removes the outgoing 5.",
      mistakeTypes: ["rolling_sum_trace_mismatch"],
      nextAction:
        "For every shift, explicitly name the incoming and outgoing elements.",
      result: "diagnostic",
    },
  },
  {
...
What is the main bug?`,
    ...
    feedbackModel: {
      decisionSignal:
        "The boundary moves, but the maintained aggregate is not updated to match the new boundary.",
      mentalModelCorrection:
        "Window state and window indexes must describe the same range. Moving left requires removing the contribution that left leaves behind.",
      mistakeTypes: ["outgoing_contribution_not_removed"],
      nextAction:
        "Pair every left increment with removal of the old values[left] contribution.",
      result: "diagnostic",
    },
  },
  {
...
Which movement strategy matches that contract?`,
    ...
    feedbackModel: {
      decisionSignal:
        "Every candidate range must contain exactly k elements, regardless of its values.",
      mentalModelCorrection:
        "A fixed-size contract determines when left moves. The aggregate does not decide whether the window should shrink.",
      mistakeTypes: ["conditional_shrinking_in_fixed_window"],
      nextAction:
        "Derive pointer movement from the required size, not from a validity condition.",
      result: "diagnostic",
    },
  },
  {
...
Why is it incorrect for finding the best sum of exactly k elements?`,
    ...
    feedbackModel: {
      decisionSignal:
        "The result is updated for ranges of sizes 1, 2, and so on before the first size-k range exists.",
      mentalModelCorrection:
        "Only a complete window satisfying the exact size contract may contribute to the answer.",
      mistakeTypes: ["incomplete_window_recorded"],
      nextAction:
        "Place result recording inside the branch that proves the window size is exactly k.",
      result: "diagnostic",
    },
  },
  {
    ...
    complexityExplanation:
      "The first window is built once, and every subsequent window is derived with one incoming addition and one outgoing subtraction.",
...
Which comparison is correct?`,
    ...
    feedbackModel: {
      decisionSignal:
        "The outgoing contribution can be reversed, so most work from the previous window can be reused.",
      mentalModelCorrection:
        "Conceptually containing k values does not require processing all k values again after every one-position shift.",
      mistakeTypes: ["fixed_window_recomputed_from_scratch"],
      nextAction:
        "Identify which contribution enters and which leaves when adjacent windows overlap.",
      result: "diagnostic",
    },
  },
  {
    ...
    prompt:
      "For an input containing n elements and k = 1, how should a correct fixed-size window behave?",
    options: [
      {
    ...
    feedbackModel: {
      decisionSignal:
        "A contiguous range of exactly one element is valid at every array position.",
      mentalModelCorrection:
        "The first window is already complete when the first element is added.",
      mistakeTypes: ["fixed_window_k_one_edge_case"],
      nextAction:
        "Test the invariant right - left + 1 === k with k equal to one.",
      result: "diagnostic",
    },
  },
  {
    ...
    type: "edge_case_drill",
    prompt:
      "An array has n elements and the required fixed window size is also n. How many complete windows exist?",
    options: [
      {
        id: "exactly_one",
    ...
    feedbackModel: {
      decisionSignal:
        "A complete window does not need to have a later neighboring window.",
      mentalModelCorrection:
        "When k equals n, the initial completed range is also the only valid range.",
      mistakeTypes: ["full_length_window_count_mismatch"],
      nextAction:
        "Use the complete-window count n - k + 1 for valid values of k.",
      result: "diagnostic",
    },
  },
  {
...
The first window [1, 2, 5] contains 2 odd values. What count should the next window [2, 5, 4] contain, and how is it obtained?`,
    ...
    feedbackModel: {
      decisionSignal:
        "The maintained state is a sum of per-element contributions: one for odd and zero for even.",
      mentalModelCorrection:
        "Rolling-window state need not be a numeric sum of the original values. It only needs removable incoming and outgoing contributions.",
      mistakeTypes: ["removable_state_update_mismatch"],
      nextAction:
        "Define each element's contribution, then update the aggregate when that element enters or leaves.",
      result: "diagnostic",
    },
  },
  {
...
What is the best correction?`,
    ...
    feedbackModel: {
      decisionSignal:
        "The answer depends on the complete contiguous range rather than only on two endpoint values.",
      mentalModelCorrection:
        "The indexes define the boundaries of a collection of k elements. The maintained state summarizes that whole collection.",
      mistakeTypes: ["window_treated_as_endpoint_pair"],
      nextAction:
        "Translate [left, right] into the explicit set of indexes left through right.",
      result: "diagnostic",
    },
  },
  {
...
A correct implementation records a result whenever it has formed another complete size-3 window. How many results should it record, and what are the starting indexes?`,
    ...
    feedbackModel: {
      decisionSignal:
        "A starting index is valid when k elements still fit between it and the end of the array.",
      mentalModelCorrection:
        "The number of complete fixed-size windows is n - k + 1, not n, k, or n - k.",
      mistakeTypes: ["fixed_window_count_off_by_one"],
      nextAction:
        "Check both the first valid start, zero, and the final valid start, n - k.",
      result: "diagnostic",
    },
  },
];

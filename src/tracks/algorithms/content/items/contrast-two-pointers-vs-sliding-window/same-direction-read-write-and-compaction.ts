...
What do the two pointers represent?`,
    ...
    feedbackModel: {
      decisionSignal:
        "One pointer observes input while the other identifies the next output slot.",
      mentalModelCorrection:
        "The pointers do not bound one candidate range. Their distance reflects how many values were rejected, not a window contract.",
      mistakeTypes: ["read_write_roles_treated_as_window"],
      nextAction:
        "Name the operational responsibility of each pointer before interpreting the range between them.",
      result: "diagnostic",
    },
  },
  {
...
Why does write advance only inside the condition?`,
    ...
    feedbackModel: {
      ...
      mentalModelCorrection:
        "A rejected value consumes no output position. Advancing write would leave a gap or preserve rejected data in the logical result.",
      mistakeTypes: ["write_advanced_for_rejected_value"],
      nextAction:
        "Tie write movement to creation of one new valid output element.",
      result: "diagnostic",
    },
  },
  {
...
What does the distance between them most directly indicate?`,
    ...
    feedbackModel: {
      decisionSignal:
        "read counts observations while write counts accepted output positions.",
      mentalModelCorrection:
        "Their separation records filtering progress. It is not automatically the size of a semantically meaningful range.",
      mistakeTypes: ["write_read_distance_called_window_length"],
      nextAction:
        "Ask whether the algorithm maintains any property of every element between write and read.",
      result: "diagnostic",
    },
  },
  {
    ...
    prompt:
      "Which sequence correctly processes one observation in a stable read/write compaction algorithm?",
    options: [
      {
    ...
    feedbackModel: {
      decisionSignal:
        "An output slot should be consumed only after the observation has been accepted.",
      mentalModelCorrection:
        "The decision precedes write advancement. There is no window-validity phase or outgoing contribution.",
      mistakeTypes: ["compaction_subgoal_order_mismatch"],
      nextAction:
        "Structure each iteration as observe, decide, optionally emit.",
      result: "diagnostic",
    },
  },
  {
...
What should the retained prefix contain after processing all elements?`,
    ...
    feedbackModel: {
      decisionSignal:
        "Accepted values are emitted in the same order in which read observes them.",
      mentalModelCorrection:
        "Stable compaction removes rejected values without rearranging the retained ones.",
      mistakeTypes: ["stable_compaction_trace_mismatch"],
      nextAction:
        "Append each accepted observation to a conceptual output list before mapping that list back onto the prefix.",
      result: "diagnostic",
    },
  },
  {
...
Which implementation strategy satisfies that contract?`,
    ...
    feedbackModel: {
      ...
      mentalModelCorrection:
        "Filling holes from the end may compact the array, but it can reorder retained values and violate stability.",
      mistakeTypes: ["stable_order_lost_during_compaction"],
      nextAction:
        "Check whether accepted elements appear in output in the same relative order as in input.",
      result: "diagnostic",
    },
  },
  {
...
Which read/write interpretation is most useful?`,
    ...
    feedbackModel: {
      decisionSignal:
        "Sorted order makes duplicates adjacent, and the retained prefix summarizes the distinct values already accepted.",
      mentalModelCorrection:
        "The algorithm compares an observation with retained output state; it does not maintain one candidate window between the pointers.",
      mistakeTypes: ["duplicate_collapse_classified_as_window"],
      nextAction:
        "Describe write in terms of the logical unique output rather than pointer distance.",
      result: "diagnostic",
    },
  },
  {
...
Which movement is correct?`,
    ...
    feedbackModel: {
      decisionSignal:
        "The duplicate has been inspected but should not extend the logical result.",
      mentalModelCorrection:
        "read advances for consumed input; write advances only when output grows.",
      mistakeTypes: ["write_advanced_for_duplicate"],
      nextAction:
        "Separate consumption of an observation from emission of a retained value.",
      result: "diagnostic",
    },
  },
  {
...
What should the logical retained prefix contain?`,
    ...
    feedbackModel: {
      decisionSignal:
        "Only the first observation from each adjacent duplicate group extends the unique output prefix.",
      mentalModelCorrection:
        "The resulting prefix contains retained values, not one representative for every pointer movement.",
      mistakeTypes: ["duplicate_collapse_trace_mismatch"],
      nextAction:
        "Track the last retained value and emit only when the observation differs from it.",
      result: "diagnostic",
    },
  },
  {
...
Why is that reasoning incorrect?`,
    ...
    feedbackModel: {
      decisionSignal:
        "No maintained state corresponds to all elements inside the pointer interval.",
      mentalModelCorrection:
        "Window updates are meaningful only when the pointers are boundaries of one represented range. Compaction pointers have different semantic roles.",
      mistakeTypes: ["outgoing_element_removed_without_window_state"],
      nextAction:
        "Identify the exact state that an alleged outgoing update would modify before adding such logic.",
      result: "diagnostic",
    },
  },
  {
...
What does the half-open range [0, write) represent?`,
    ...
    feedbackModel: {
      decisionSignal:
        "write is the logical output length after processing the current input prefix.",
      mentalModelCorrection:
        "The prefix before write is finalized retained output, not a movable candidate range.",
      mistakeTypes: ["processed_prefix_state_misinterpreted"],
      nextAction:
        "Partition the array conceptually into retained output, observed-but-not-retained space, and unprocessed input.",
      result: "diagnostic",
    },
  },
  {
...
Which design better matches the contract?`,
    ...
    feedbackModel: {
      decisionSignal:
        "The output is a stable sequence of independently accepted observations, not one valid contiguous subarray of the original input.",
      mentalModelCorrection:
        "A shrink loop solves a range-validity problem. Compaction instead builds a logical output prefix.",
      mistakeTypes: ["variable_window_used_for_compaction"],
      nextAction:
        "Classify whether the task asks for a subarray boundary or a rewritten retained sequence.",
      result: "diagnostic",
    },
  },
  {
    ...
    prompt:
      "A stable in-place filter rejects every element in a non-empty array. Where should write finish if it starts at zero?",
    options: [
      {
        id: "zero",
    ...
    feedbackModel: {
      ...
      mentalModelCorrection:
        "Processing an observation advances read, but rejecting it does not increase logical output length.",
      mistakeTypes: ["all_rejected_write_position_mismatch"],
      nextAction:
        "Derive the final write value from the number of accepted elements.",
      result: "diagnostic",
    },
  },
  {
    ...
    type: "edge_case_drill",
    prompt:
      "A stable in-place filter accepts every element. How should read and write behave?",
    options: [
      {
        id: "advance_together",
    ...
    feedbackModel: {
      decisionSignal:
        "Every observation extends the retained output by one value.",
      mentalModelCorrection:
        "Even when the write is effectively a self-assignment, it still represents growth of the logical retained prefix.",
      mistakeTypes: ["all_accepted_write_position_mismatch"],
      nextAction:
        "Check that write equals the number of accepted observations in every edge case.",
      result: "diagnostic",
    },
  },
];

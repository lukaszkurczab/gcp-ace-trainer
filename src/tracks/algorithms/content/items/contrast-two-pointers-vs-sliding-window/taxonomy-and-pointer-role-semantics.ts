export const taxonomyAndPointerRoleSemanticsQuestions = [
  {
    ...
    type: "single_choice",
    prompt:
      "Which statement best describes the relationship between two pointers and sliding window?",
    options: [
      {
    ...
    feedbackModel: {
      decisionSignal:
        "Sliding window has the additional semantic requirement that two boundaries describe one maintained contiguous range.",
      mentalModelCorrection:
        "Two pointers is a broader family. Sliding window is one possible interpretation of two moving boundaries, not a separate implementation shape.",
      mistakeTypes: ["two_pointers_and_window_treated_as_disjoint"],
      nextAction:
        "Classify the represented state before deciding whether the narrower sliding-window label applies.",
      result: "diagnostic",
    },
  },
  {
...
What is the most precise correction?`,
    ...
    feedbackModel: {
      decisionSignal:
        "The same number of indexes can support several different invariants and algorithmic roles.",
      mentalModelCorrection:
        "Pattern identity comes from what the indexes mean, not from counting how many variables move.",
      mistakeTypes: ["every_two_pointer_loop_called_window"],
      nextAction:
        "Explain the responsibility of each pointer and the meaning of any region between them.",
      result: "diagnostic",
    },
  },
  {
...
What roles do left and right actually have?`,
    ...
    feedbackModel: {
      decisionSignal:
        "right inspects each input element, while left advances only when an output element is produced.",
      mentalModelCorrection:
        "Names such as left and right do not force boundary semantics. Operations reveal that these are read/write roles.",
      mistakeTypes: ["pointer_names_assumed_to_determine_role"],
      nextAction:
        "Describe which pointer consumes input and which pointer grows the output.",
      result: "diagnostic",
    },
  },
  {
    ...
    learningStage: "foundations",
    primarySkillAtomId:
      "distinguish_same_direction_roles_from_window_boundaries",
    secondarySkillAtomIds: [
    ...
    type: "single_choice",
    prompt:
      "Two pointers move from left to right through the same array. What is required before classifying them as sliding-window boundaries?",
    options: [
      {
    ...
    feedbackModel: {
      decisionSignal:
        "Sliding-window boundaries jointly represent a range whose complete contents matter.",
      mentalModelCorrection:
        "Movement direction and pointer distance are insufficient without a range-level invariant.",
      mistakeTypes: ["same_direction_pair_automatically_called_window"],
      nextAction:
        "Identify whether any maintained state corresponds exactly to the elements inside the pointer interval.",
      result: "diagnostic",
    },
  },
  {
...
Which interpretation is correct?`,
    ...
    feedbackModel: {
      decisionSignal:
        "The decisive difference is whether the interior contributes to the represented candidate state.",
      mentalModelCorrection:
        "An interval can be merely a search region in one algorithm and the actual candidate range in another.",
      mistakeTypes: ["candidate_pair_and_window_boundaries_conflated"],
      nextAction:
        "List every input position that contributes to the current candidate evaluation.",
      result: "diagnostic",
    },
  },
  {
...
Which semantic description is strongest?`,
    ...
    feedbackModel: {
      decisionSignal:
        "The pointers differ by advancement policy and operational responsibility.",
      mentalModelCorrection:
        "Names such as slow and fast describe traversal behavior, but do not imply that the region between them is an active candidate.",
      mistakeTypes: ["slow_fast_pair_treated_as_window"],
      nextAction:
        "State what event causes each pointer to advance and what progress that movement represents.",
      result: "diagnostic",
    },
  },
  {
...
How should the two indexes be interpreted?`,
    ...
    feedbackModel: {
      decisionSignal:
        "Each index belongs to a different side of a data transfer.",
      mentalModelCorrection:
        "Pointer roles may describe data flow rather than boundaries within one shared candidate range.",
      mistakeTypes: ["source_destination_roles_called_window"],
      nextAction:
        "Name which data structure each pointer indexes and whether any shared interval exists at all.",
      result: "diagnostic",
    },
  },
  {
    ...
    prompt:
      "Which invariant most clearly identifies left and right as sliding-window boundaries?",
    options: [
      {
    ...
    feedbackModel: {
      decisionSignal:
        "A window invariant binds both the represented range and the state maintained for that range.",
      mentalModelCorrection:
        "Ordering and monotonic movement are common mechanics, but they do not define window semantics.",
      mistakeTypes: ["window_invariant_not_stated"],
      nextAction:
        "Name both the current range and the state guaranteed to correspond to it.",
      result: "diagnostic",
    },
  },
  {
...
Which invariant best describes [0, write)?`,
    ...
    feedbackModel: {
      decisionSignal:
        "write represents produced output length rather than a movable range boundary.",
      mentalModelCorrection:
        "A finalized processed prefix is structurally different from an active window that may continue changing.",
      mistakeTypes: ["processed_prefix_called_active_window"],
      nextAction:
        "State whether the region is finalized output, a current candidate, or unprocessed input.",
      result: "diagnostic",
    },
  },
  {
...
A reviewer calls [write, read) the current active window. What is missing from that claim?`,
    ...
    feedbackModel: {
      decisionSignal:
        "A numerical interval exists, but no range-level candidate meaning has been established.",
      mentalModelCorrection:
        "Not every area between two indexes is algorithmically meaningful as a window.",
      mistakeTypes: ["pointer_gap_assumed_to_be_active_window"],
      nextAction:
        "Describe what the region contains and whether the algorithm evaluates or maintains it as one unit.",
      result: "diagnostic",
    },
  },
  {
...
How should start and cursor be classified?`,
    ...
    feedbackModel: {
      decisionSignal:
        "The maintained sum corresponds to the complete contiguous region between the two indexes.",
      mentalModelCorrection:
        "Semantic invariants remain the same even when pointer variables use different names.",
      mistakeTypes: ["nonstandard_names_hide_window_roles"],
      nextAction:
        "Replace variable names mentally with descriptions of the state they delimit.",
      result: "diagnostic",
    },
  },
  {
...
Which mapping is correct?`,
    ...
    feedbackModel: {
      decisionSignal:
        "Each algorithm assigns a different invariant and responsibility to its indexes.",
      mentalModelCorrection:
        "Identical surface structures can encode different algorithmic abstractions.",
      mistakeTypes: ["pointer_role_taxonomy_conflated"],
      nextAction:
        "Classify each pointer pair by current candidate meaning, maintained state, and movement responsibility.",
      result: "diagnostic",
    },
  },
  {
    ...
    type: "single_choice",
    prompt:
      "Which explanation gives the most useful semantic account of a two-pointer algorithm?",
    options: [
      {
    ...
    feedbackModel: {
      decisionSignal:
        "A useful explanation identifies responsibilities, represented regions, and correspondence between state and indexes.",
      mentalModelCorrection:
        "Mechanical observations do not explain why the pointers exist or what correctness invariant they maintain.",
      mistakeTypes: ["pointer_roles_described_only_syntactically"],
      nextAction:
        "State what each pointer controls and what every relevant array region currently means.",
      result: "diagnostic",
    },
  },
  {
...
What is the best response?`,
    ...
    feedbackModel: {
      decisionSignal:
        "One label describes the broad mechanism, while the other describes a more specific represented state.",
      mentalModelCorrection:
        "Algorithmic pattern categories may be hierarchical rather than mutually exclusive.",
      mistakeTypes: ["pattern_categories_assumed_mutually_exclusive"],
      nextAction:
        "Use the broadest valid category and then add the more specific semantic classification.",
      result: "diagnostic",
    },
  },
];

export const sortingContiguityAndOutputContractQuestions = [
  {
    ...
    type: "single_choice",
    prompt:
      "An unsorted array must be checked for whether any two distinct values sum to a target. The result only needs to be true or false. What effect does sorting have on the contract?",
    options: [
      {
    ...
    feedbackModel: {
      decisionSignal:
        "The output concerns existence of two values, not their adjacency or original positions.",
      mentalModelCorrection:
        "Sorting may reorder candidates without changing which numeric value pairs exist.",
      mistakeTypes: ["sorting_rejected_despite_value_only_pair_contract"],
      nextAction:
        "Identify which properties of the input must remain observable in the returned result.",
      result: "diagnostic",
    },
  },
  {
...
A candidate sorts the values first and then applies a sliding window. What is the central mistake?`,
    ...
    feedbackModel: {
      decisionSignal:
        "The required answer is defined by adjacency in the original sequence.",
      mentalModelCorrection:
        "Sorting preserves values but replaces the original neighborhood structure on which a subarray contract depends.",
      mistakeTypes: ["sorting_before_original_subarray_search"],
      nextAction:
        "Before sorting, ask whether the output must correspond to consecutive original indexes.",
      result: "diagnostic",
    },
  },
  {
    ...
    prompt:
      "A task asks for any two array elements whose values satisfy a relationship. The elements may occur anywhere and do not need to be adjacent. Which interpretation is correct?",
    options: [
      {
    ...
    feedbackModel: {
      decisionSignal:
        "The contract selects two elements independently rather than one complete contiguous range.",
      mentalModelCorrection:
        "Two answer indexes do not imply that the interior belongs to the answer.",
      mistakeTypes: ["sliding_window_used_for_non_adjacent_pair"],
      nextAction:
        "State whether interior positions contribute to the candidate result.",
      result: "diagnostic",
    },
  },
  {
...
Which review is correct?`,
    ...
    feedbackModel: {
      decisionSignal:
        "The result must refer to positions in the original input, not the transformed search representation.",
      mentalModelCorrection:
        "Sorting is still possible, but positional identity must be carried explicitly through the transformation.",
      mistakeTypes: ["original_indexes_lost_after_sorting"],
      nextAction:
        "Track whether each transformed value still retains a mapping to its source position.",
      result: "diagnostic",
    },
  },
  {
    ...
    type: "single_choice",
    prompt:
      "A task asks to return one pair of numeric values that sums to a target. It does not require their original positions or original order. Which statement is precise?",
    options: [
      {
    ...
    feedbackModel: {
      decisionSignal:
        "The output contract retains value identity but not positional identity.",
      mentalModelCorrection:
        "A transformation is legal when it preserves all information the result must expose.",
      mistakeTypes: ["value_output_confused_with_index_output"],
      nextAction:
        "List the exact fields the caller expects from the returned answer.",
      result: "diagnostic",
    },
  },
  {
    ...
    type: "single_choice",
    prompt:
      "A function must return the start and end indexes of one qualifying contiguous range in the original array. Which strategy property is required?",
    options: [
      {
    ...
    feedbackModel: {
      decisionSignal:
        "The returned indexes define an inclusive original subarray.",
      mentalModelCorrection:
        "For a range output, the relationship between all indexes from start through end is part of the contract.",
      mistakeTypes: ["independent_pair_returned_as_contiguous_range"],
      nextAction:
        "Verify that every index between the returned boundaries belongs to the candidate answer.",
      result: "diagnostic",
    },
  },
  {
    ...
    type: "strategy_choice",
    prompt:
      "A task asks only for the length of the longest contiguous substring satisfying a maintainable condition. Which observation is most relevant?",
    options: [
      {
    ...
    feedbackModel: {
      decisionSignal:
        "Although the output is only a number, that number is defined by a contiguous range in the original input.",
      mentalModelCorrection:
        "Returning less information does not remove structural constraints from the answer being measured.",
      mistakeTypes: ["length_only_output_treated_as_order_independent"],
      nextAction:
        "Separate the form of the returned value from the structure of the candidate it summarizes.",
      result: "diagnostic",
    },
  },
  {
    ...
    type: "strategy_choice",
    prompt:
      "A task requires modifying an array in place so that all retained values occupy a stable prefix, then returning the new logical length. Which strategy matches the output contract?",
    options: [
      {
    ...
    feedbackModel: {
      decisionSignal:
        "The required result is a rewritten prefix rather than a selected original range.",
      mentalModelCorrection:
        "Compaction constructs output positions; it does not preserve one active candidate interval.",
      mistakeTypes: ["range_strategy_used_for_transformed_prefix_output"],
      nextAction:
        "Determine whether the function selects from the original input or rewrites a logical output sequence.",
      result: "diagnostic",
    },
  },
  {
...
A learner says that indexes 2 through 8 form the algorithm's current output range. What is the best correction?`,
    ...
    feedbackModel: {
      decisionSignal:
        "The algorithm evaluates a pair and uses the surrounding interval to describe candidates not yet eliminated.",
      mentalModelCorrection:
        "A search region and a returned contiguous range are different abstractions, even when both are written with two boundaries.",
      mistakeTypes: ["search_interval_assumed_to_be_output_range"],
      nextAction:
        "Identify which positions directly participate in the current result calculation.",
      result: "diagnostic",
    },
  },
  {
...
A candidate sorts the array before searching. What is wrong?`,
    ...
    feedbackModel: {
      decisionSignal:
        "A subsequence preserves original left-to-right order even though it may skip positions.",
      mentalModelCorrection: "Non-contiguous does not mean order-independent.",
      mistakeTypes: ["sorting_before_original_subsequence_search"],
      nextAction:
        "Check separately whether the answer requires adjacency and whether it requires relative order.",
      result: "diagnostic",
    },
  },
  {
    ...
    type: "single_choice",
    prompt:
      "Which distinction between a subarray, a subsequence, and a pair is precise?",
    options: [
      {
    ...
    feedbackModel: {
      decisionSignal:
        "The three contracts preserve different amounts of the input's positional structure.",
      mentalModelCorrection:
        "Strategy choice depends on exactly which positional relationships the answer must retain.",
      mistakeTypes: ["output_structure_terms_conflated"],
      nextAction:
        "Translate each contract into explicit constraints on indexes.",
      result: "diagnostic",
    },
  },
  {
...
How should they be compared first?`,
    ...
    feedbackModel: {
      ...
      mentalModelCorrection:
        "Complexity cannot compensate for returning an object that does not satisfy the problem's required result form.",
      mistakeTypes: ["big_o_chosen_before_output_contract"],
      nextAction:
        "Reject contract-incompatible strategies before ranking the remaining candidates.",
      result: "diagnostic",
    },
  },
  {
...
What must a sorting-based representation preserve?`,
    ...
    feedbackModel: {
      decisionSignal:
        "Equal values may correspond to distinct valid source elements.",
      mentalModelCorrection:
        "Value equality does not erase index identity when the output contract requires original positions.",
      mistakeTypes: ["duplicate_source_indexes_collapsed_after_sorting"],
...
Which strategy mapping is most appropriate at a high level?`,
    ...
    feedbackModel: {
      decisionSignal:
        "The tasks require three different result structures: selected values, an original contiguous range, and a rewritten prefix.",
      mentalModelCorrection:
        "Strategy follows the semantic contract and preserved relationships, not a preference for one pointer shape.",
      mistakeTypes: ["strategy_selected_without_output_contract_analysis"],
      nextAction:
        "Classify the required output before deciding whether input reordering or range maintenance is legal.",
      result: "diagnostic",
    },
  },
];

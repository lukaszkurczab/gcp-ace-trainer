...
Which answer-state mapping is correct?`,
    ...
    feedbackModel: {
      decisionSignal:
        "The same boundary movement can support different observable outputs.",
      mentalModelCorrection:
        "Pointer mechanics do not determine the answer rule; the result contract determines how valid windows are recorded.",
      mistakeTypes: ["window_answer_contracts_conflated"],
      nextAction:
        "Identify whether the requested output is an optimum, a boolean, or a number of candidates.",
      result: "diagnostic",
    },
  },
  {
    ...
    type: "single_choice",
    prompt:
      "In a longest-window problem, the algorithm expands right and shrinks left while the window is invalid. When should the best length be updated?",
    options: [
      {
    ...
    feedbackModel: {
      decisionSignal:
        "The longest answer must be chosen from windows that satisfy the contract.",
      mentalModelCorrection:
        "An invalid expanded window cannot become the recorded optimum merely because it is large.",
      mistakeTypes: ["longest_window_recorded_before_validity_restored"],
      nextAction:
        "Normalize the window first, then compare its length with the current maximum.",
      result: "diagnostic",
    },
  },
  {
...
What is the main problem?`,
    ...
    feedbackModel: {
      decisionSignal:
        "An optimum selects one best candidate, while counting includes every qualifying candidate.",
      mentalModelCorrection:
        "A maximum-length update cannot substitute for candidate enumeration or combinatorial counting.",
      mistakeTypes: ["longest_answer_rule_reused_for_count"],
      nextAction:
        "Determine how many valid windows are represented by each pointer state.",
      result: "diagnostic",
    },
  },
  {
    ...
    secondarySkillAtomIds: ["minimize_window_length", "shrink_while_valid"],
    type: "single_choice",
    prompt:
      "A shortest-window problem has a condition that remains satisfied while redundant values are removed from the left. Which inner-loop structure is appropriate?",
    options: [
      {
    ...
    feedbackModel: {
      decisionSignal:
        "Once a satisfying window exists, removing left-side values may produce a shorter satisfying candidate.",
      mentalModelCorrection:
        "Shortest-window searches often continue shrinking while valid rather than stopping at the first valid boundary.",
      mistakeTypes: ["shortest_window_uses_longest_shrink_rule"],
      nextAction:
        "Record every valid contraction candidate before removing its current left endpoint.",
      result: "diagnostic",
    },
  },
  {
...
What can go wrong?`,
    ...
    feedbackModel: {
      decisionSignal:
        "Removing the current left value may destroy the condition that made the current interval a valid candidate.",
      mentalModelCorrection:
        "Record a valid candidate before performing the update that may invalidate it.",
      mistakeTypes: ["shortest_window_recorded_after_validity_destroyed"],
      nextAction:
        "Move the minimum update to the start of the valid-shrinking loop.",
      result: "diagnostic",
    },
  },
  {
    ...
    type: "single_choice",
    prompt:
      "A problem asks only whether any non-empty valid window exists. What may the algorithm do after proving that the current non-empty window is valid?",
    options: [
      {
        id: "return_true",
    ...
    feedbackModel: {
      ...
      mentalModelCorrection:
        "Existence does not require optimization or complete enumeration.",
      mistakeTypes: ["existence_problem_overprocessed_as_optimization"],
      nextAction:
        "Return as soon as one candidate satisfies every required condition.",
      result: "diagnostic",
    },
  },
  {
...
What is wrong?`,
    ...
    feedbackModel: {
      decisionSignal:
        "Constraint satisfaction and candidate eligibility are separate requirements.",
      mentalModelCorrection:
        "An empty internal state must not be returned as a solution when the contract requires a non-empty window.",
      mistakeTypes: ["empty_window_accepted_as_nonempty_solution"],
      nextAction:
        "Check both the window condition and the minimum permitted candidate length.",
      result: "diagnostic",
    },
  },
  {
    ...
    type: "single_choice",
    prompt:
      "A window must contain at most k distinct values. When should left advance?",
    options: [
      {
    ...
    feedbackModel: {
      decisionSignal:
        "The at-most contract permits every count from 0 through k.",
      mentalModelCorrection:
        "Equality with k is valid; shrinking is required only after the upper bound is exceeded.",
      mistakeTypes: ["at_most_constraint_treated_as_strictly_less_than"],
      nextAction:
        "Shrink only while the maintained quantity violates the stated upper bound.",
      result: "diagnostic",
    },
  },
  {
...
to the answer?`,
    ...
    feedbackModel: {
      decisionSignal:
        "Removing a prefix cannot introduce a new distinct value.",
      mentalModelCorrection:
        "The addition counts different valid starting positions for the current right endpoint.",
      mistakeTypes: ["at_most_window_length_count_not_justified"],
      nextAction:
        "Prove that every suffix of the normalized window preserves the at-most property.",
      result: "diagnostic",
    },
  },
  {
...
What does it miss?`,
    ...
    feedbackModel: {
      decisionSignal:
        "A single normalized window compactly represents several valid subarrays sharing the same right endpoint.",
      mentalModelCorrection:
        "Counting existence once per right endpoint undercounts when multiple starts remain valid.",
      mistakeTypes: ["only_one_at_most_window_counted_per_right"],
      nextAction:
        "Add the number of eligible start positions rather than one boolean witness.",
      result: "diagnostic",
    },
  },
  {
    ...
    type: "single_choice",
    prompt:
      "Which identity correctly counts subarrays containing exactly k distinct values?",
    options: [
      {
    ...
    feedbackModel: {
      decisionSignal:
        "The at-most-k set consists of windows with 0 through k distinct values, while at-most-(k - 1) removes all windows below k.",
      mentalModelCorrection:
        "The subtraction works because these are counts of nested candidate sets.",
      mistakeTypes: ["exact_k_count_identity_not_recognized"],
      nextAction:
        "Express exact equality as the difference between two nested upper-bound sets.",
      result: "diagnostic",
    },
  },
  {
...
Why is this invalid?`,
    ...
    feedbackModel: {
      decisionSignal:
        "Set cardinalities support subtraction over nested sets; selected maximum values generally do not.",
      mentalModelCorrection:
        "Do not transfer an identity for counts to lengths, booleans, or other answer representations.",
      mistakeTypes: ["exact_count_subtraction_reused_for_longest_length"],
      nextAction:
        "Solve the exactly-k optimization contract directly with an invariant that preserves equality.",
      result: "diagnostic",
    },
  },
  {
...
Why can this overcount?`,
    ...
    feedbackModel: {
      decisionSignal:
        "The at-most property is inherited by every suffix, but exact equality may be lost when a distinguishing value is removed.",
      mentalModelCorrection:
        "A width-based count is valid only when every represented start position satisfies the same requested contract.",
      mistakeTypes: ["at_most_suffix_count_reused_for_exact_k"],
      nextAction:
        "Use two at-most counts or maintain an additional boundary that separates fewer-than-k suffixes.",
      result: "diagnostic",
    },
  },
  {
    ...
    type: "single_choice",
    prompt:
      "A problem asks whether any non-empty window contains exactly k distinct values. Which condition proves success?",
    options: [
      {
    ...
    feedbackModel: {
      decisionSignal:
        "The requested relationship is equality of a maintained quantity.",
      mentalModelCorrection:
        "An upper-bound condition includes windows below the target and therefore does not prove exactness.",
      mistakeTypes: ["at_most_window_accepted_for_exact_k_existence"],
      nextAction:
        "Check equality after restoring the relevant upper-bound invariant.",
      result: "diagnostic",
    },
  },
  {
...
What result is it computing instead?`,
    ...
    feedbackModel: {
      decisionSignal:
        "A length describes one candidate; repeated addition aggregates contributions from many pointer states.",
      mentalModelCorrection:
        "The expression `right - left + 1` may represent either a candidate length or a number of valid starts, depending on how it is used.",
      mistakeTypes: ["count_and_length_aggregation_confused"],
      nextAction:
        "Use `Math.max` for a longest contract and addition only after proving a counting interpretation.",
      result: "diagnostic",
    },
  },
  {
    ...
    type: "single_choice",
    prompt:
      "Which statement most completely explains correctness across longest, shortest, existence, count, at-most, and exactly-k window problems?",
    options: [
      {
    ...
    feedbackModel: {
      decisionSignal:
        "Correct boundary state is necessary but not sufficient; the recording rule must preserve the requested output semantics.",
      mentalModelCorrection:
        "Sliding-window families often share mechanics while differing critically in validity, contraction, and answer aggregation.",
      mistakeTypes: ["window_objective_and_answer_invariant_incomplete"],
      nextAction:
        "For each problem, define the candidate, validity relation, contraction rule, and exact meaning of one answer update.",
      result: "diagnostic",
    },
  },
];

export const monotonicDequeWindowExtremesQuestions = [
  {
    ...
    type: "single_choice",
    prompt:
      "What should a standard monotonic deque store while computing fixed-window maxima or minima?",
    options: [
      {
    ...
    feedbackModel: {
      decisionSignal:
        "Each stored entry needs both a comparable value through its index and an age for stale-window eviction.",
      mentalModelCorrection:
        "The deque stores surviving extreme candidates, not the complete window.",
      mistakeTypes: ["monotonic_deque_role_misidentified"],
      nextAction:
        "For each deque entry, explain why it may still become the answer before it expires.",
      result: "diagnostic",
    },
  },
  {
    ...
    type: "single_choice",
    prompt:
      "Which value ordering should indexes in a deque maintain for fixed-window maximum queries?",
    options: [
      {
    ...
    feedbackModel: {
      decisionSignal:
        "Every candidate behind the front has a value no larger than the front candidate.",
      mentalModelCorrection:
        "For maxima, the deque is monotonic by referenced values while indexes remain ordered by arrival time.",
      mistakeTypes: ["maximum_deque_order_reversed"],
      nextAction:
        "Read candidate values from front to back and verify that none increases.",
      result: "diagnostic",
    },
  },
  {
    ...
    type: "single_choice",
    prompt:
      "How does the monotonic ordering change when computing fixed-window minima instead of maxima?",
    options: [
      {
        id: "nondecreasing_values",
    ...
    feedbackModel: {
      decisionSignal:
        "Minimum candidates eliminate larger values behind them rather than smaller values.",
      mentalModelCorrection:
        "Maximum and minimum deques use symmetric dominance rules with opposite value ordering.",
      mistakeTypes: ["minimum_deque_uses_maximum_order"],
      nextAction:
        "Reverse each value comparison while preserving the same stale-index logic.",
      result: "diagnostic",
    },
  },
  {
...
Which deque entries are stale?`,
    ...
    feedbackModel: {
      decisionSignal:
        "A fixed window contains the inclusive index range [right - k + 1, right].",
      mentalModelCorrection:
        "An index expires only after it falls strictly before the current inclusive left boundary.",
      mistakeTypes: ["stale_deque_boundary_off_by_one"],
      nextAction:
        "Write the current inclusive window interval before choosing the eviction comparison.",
      result: "diagnostic",
    },
  },
  {
...
What is wrong?`,
    ...
    feedbackModel: {
      decisionSignal:
        "Indexes are appended as right moves forward, so age increases toward the front.",
      mentalModelCorrection:
        "The front handles expiration; the back handles value dominance.",
      mistakeTypes: ["stale_index_evicted_from_wrong_deque_end"],
      nextAction:
        "Associate each deque end with one responsibility: age at the front and dominance at the back.",
      result: "diagnostic",
    },
  },
  {
...
Before appending index right, which cleanup is correct?`,
    ...
    feedbackModel: {
      decisionSignal:
        "A newer value that is at least as large will outlive and dominate every smaller-or-equal candidate behind it.",
      mentalModelCorrection:
        "Dominance cleanup proceeds repeatedly from the back until monotonic order is restored.",
      mistakeTypes: ["maximum_dominance_rule_incorrect"],
      nextAction:
        "Remove every trailing candidate that can no longer beat or outlive the incoming value.",
      result: "diagnostic",
    },
  },
  {
...
Why is one conditional removal insufficient?`,
    ...
    feedbackModel: {
      decisionSignal:
        "The incoming value may dominate an entire decreasing suffix of candidates.",
      mentalModelCorrection:
        "Monotonicity is a global deque property and may require repeated back removal.",
      mistakeTypes: ["dominance_cleanup_removes_only_one_candidate"],
      nextAction:
        "Replace the single conditional with a bounds-safe while loop.",
      result: "diagnostic",
    },
  },
  {
...
Which advantage does A provide directly?`,
    ...
    feedbackModel: {
      decisionSignal:
        "Window membership is positional, and some contracts also require the extreme's source index.",
      mentalModelCorrection:
        "Indexes preserve identity and age without sacrificing access to values.",
      mistakeTypes: ["value_only_deque_preferred_without_position_reasoning"],
      nextAction:
        "List every operation that requires an element's value, index, or age.",
      result: "diagnostic",
    },
  },
  {
...
Why can stale eviction become incorrect?`,
    ...
    feedbackModel: {
      decisionSignal:
        "Expiration depends on source position rather than numeric equality.",
      mentalModelCorrection:
        "Collapsing equal values without indexes or multiplicity information loses candidate identity.",
      mistakeTypes: ["value_only_deque_loses_duplicate_expiration"],
      nextAction:
        "Store indexes, or explicitly preserve enough occurrence information to distinguish expiration times.",
      result: "diagnostic",
    },
  },
  {
...
What is wrong?`,
    ...
    feedbackModel: {
      ...
      mentalModelCorrection:
        "Maintaining candidates for a partial prefix does not make that prefix an eligible fixed-size output window.",
      mistakeTypes: ["window_extreme_emitted_before_window_complete"],
  ...
  },
  {
    ...
    prompt:
      "Which sequence correctly processes an incoming index right for a fixed-window maximum?",
    options: [
      {
    ...
    feedbackModel: {
      decisionSignal:
        "Before output, every deque entry must belong to the window and the front must be the best surviving candidate.",
      mentalModelCorrection:
        "Output is safe only after both membership and monotonic-candidate invariants have been restored.",
      mistakeTypes: ["monotonic_deque_update_order_invalid"],
      nextAction:
        "Before reading the front, verify freshness, dominance order, and window completeness.",
      result: "diagnostic",
    },
  },
  {
...
Which output is correct?`,
    ...
    feedbackModel: {
      decisionSignal:
        "Each output corresponds to one complete consecutive window of three values.",
      mentalModelCorrection:
        "The deque front changes only when the current maximum expires or a larger incoming value dominates it.",
      mistakeTypes: ["fixed_window_maximum_trace_mismatch"],
      nextAction:
        "Track the deque indexes after stale eviction and dominance cleanup at every right position.",
      result: "diagnostic",
    },
  },
  {
...
When inserting a new value, how should equal values at the deque back be handled?`,
    ...
    feedbackModel: {
      decisionSignal:
        "An older equal candidate has the same value and satisfies the requested earliest-index tie rule while it remains inside the window.",
      mentalModelCorrection:
        "Strict versus non-strict dominance cleanup encodes observable tie behavior.",
      mistakeTypes: ["earliest_extreme_tie_policy_uses_latest_candidate"],
      nextAction:
        "Use a strict comparison when older equal candidates must retain precedence.",
      result: "diagnostic",
    },
  },
  {
...
What should happen when the incoming value equals values[deque.back]?`,
    ...
    feedbackModel: {
      decisionSignal:
        "The newer equal candidate has the same value, expires later, and satisfies the latest-index tie rule.",
      mentalModelCorrection:
        "Removing equal back candidates is correct only when the tie contract permits or requires newer-index precedence.",
      mistakeTypes: ["latest_extreme_tie_policy_keeps_older_candidate"],
      nextAction:
        "Use a non-strict dominance comparison when newer equal candidates should replace older ones.",
      result: "diagnostic",
    },
  },
  {
...
C. Read the current extreme.`,
    ...
    feedbackModel: {
      decisionSignal:
        "Arrival order places the oldest candidate at the front, while monotonic cleanup removes trailing dominated candidates.",
      mentalModelCorrection:
        "The deque's two ends support different invariants rather than interchangeable removal operations.",
      mistakeTypes: ["deque_end_responsibilities_conflated"],
      nextAction:
        "Classify each operation as age-based, value-dominance-based, or answer-reading.",
      result: "diagnostic",
    },
  },
  {
    ...
    prompt:
      "Which invariant most completely explains a correct index-based deque for fixed-window maxima?",
    options: [
      {
    ...
    feedbackModel: {
      decisionSignal:
        "The front is a valid maximum only when candidate age, monotonic value order, and dominance elimination are all correct.",
      mentalModelCorrection:
        "A monotonic deque is not merely sorted storage; it is a filtered sequence of fresh, undominated candidates.",
      mistakeTypes: ["monotonic_deque_correctness_invariant_incomplete"],
      nextAction:
        "Before every output, verify index order, freshness, value monotonicity, and the tie policy.",
      result: "diagnostic",
    },
  },
];

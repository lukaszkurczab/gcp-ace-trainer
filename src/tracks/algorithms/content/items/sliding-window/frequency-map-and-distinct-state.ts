export const frequencyMapAndDistinctStateQuestions = [
  {
    ...
    type: "single_choice",
    prompt:
      "A sliding-window problem compares the current window with a target frequency requirement. What should the two frequency maps represent?",
    options: [
      {
    ...
    feedbackModel: {
      decisionSignal:
        "One structure defines the immutable contract; the other mirrors the changing contents of the active window.",
      mentalModelCorrection:
        "Target requirements and current-window observations have different lifecycles and should not be conflated.",
      mistakeTypes: ["target_and_window_frequency_roles_conflated"],
      nextAction:
        "State which map changes when an element enters or leaves the window.",
      result: "diagnostic",
    },
  },
  {
    ...
    prompt:
      "A window must contain two occurrences of `a` and one occurrence of `b`. Why is a Set insufficient for checking the requirement?",
    options: [
      {
    ...
    feedbackModel: {
      decisionSignal:
        "The contract depends on how many copies of each value occur.",
      mentalModelCorrection:
        "Membership state answers whether a key exists; frequency state answers how many occurrences exist.",
      mistakeTypes: ["set_used_for_duplicate_sensitive_window"],
      nextAction:
        "Use a key-to-count structure whenever multiplicity affects validity.",
      result: "diagnostic",
    },
  },
  {
...
What is wrong?`,
    ...
    feedbackModel: {
      decisionSignal:
        "The two sequences have equal lengths and equal distinct keys but different per-key counts.",
      mentalModelCorrection:
        "Duplicate-sensitive equality requires matching frequency distributions, not merely matching key sets.",
      mistakeTypes: ["equal_distinct_keys_treated_as_equal_frequencies"],
  ...
  },
  {
    ...
    type: "single_choice",
    prompt:
      "When values[right] enters the active window, how should its window frequency be updated?",
    options: [
      {
    ...
    feedbackModel: {
      decisionSignal:
        "The active window gains exactly one occurrence of the incoming value.",
      mentalModelCorrection:
        "Boundary movement must be reflected by an equivalent change in the state representing the window.",
      mistakeTypes: ["incoming_window_frequency_not_incremented"],
      nextAction:
        "Pair every right-boundary expansion with one increment for the entering key.",
      result: "diagnostic",
    },
  },
  {
...
When should distinctCount increase after an incoming value is added?`,
    ...
    feedbackModel: {
      decisionSignal:
        "A key becomes newly distinct only when its first active occurrence enters.",
      mentalModelCorrection:
        "Distinct count measures positive-frequency keys, not total observations.",
      mistakeTypes: ["distinct_count_incremented_for_duplicate_occurrence"],
      nextAction:
        "Inspect the key's previous count before updating distinct state.",
      result: "diagnostic",
    },
  },
  {
...
What should happen when distinctCount represents values currently present in the window?`,
    ...
    feedbackModel: {
      decisionSignal:
        "The final active occurrence of that key has left the window.",
      mentalModelCorrection:
        "Distinct state must describe the current interval, not historical membership.",
      mistakeTypes: ["distinct_count_not_decremented_at_zero"],
      nextAction:
        "Update distinctCount only on transitions between absent and present states.",
      result: "diagnostic",
    },
  },
  {
...
What is the problem?`,
    ...
    feedbackModel: {
      decisionSignal:
        "Map.size counts stored keys regardless of whether their associated counts are positive.",
      mentalModelCorrection:
        "If map membership represents active presence, zero-count entries must be removed.",
      mistakeTypes: ["zero_count_key_inflates_distinct_map_size"],
      nextAction:
        "Delete a key when its active count reaches zero, or maintain a separate positive-count variable.",
      result: "diagnostic",
    },
  },
  {
...
What is wrong?`,
    ...
    feedbackModel: {
      decisionSignal:
        "A one-position slide removes the old left endpoint and preserves the overlap beginning at left + 1.",
      mentalModelCorrection:
        "Outgoing updates must use the value that actually leaves the old interval.",
      mistakeTypes: ["wrong_outgoing_frequency_key_updated"],
      nextAction:
        "Capture or process values[left] before advancing the left boundary.",
      result: "diagnostic",
    },
  },
  {
    ...
    type: "mistake_review",
    prompt:
      "A sliding-window implementation increments frequencies for incoming values but never decrements frequencies for outgoing values. What does the map eventually represent?",
    options: [
      {
    ...
    feedbackModel: {
      decisionSignal:
        "Changing pointer boundaries does not automatically remove contributions from stored state.",
      mentalModelCorrection:
        "Window state must forget expired observations as well as incorporate new observations.",
      mistakeTypes: ["global_or_prefix_counts_used_as_window_counts"],
      nextAction:
        "Pair every left-boundary advance with the corresponding frequency decrement.",
      result: "diagnostic",
    },
  },
  {
...
What are its frequencies and distinct count?`,
    ...
    feedbackModel: {
      decisionSignal:
        "The key a occurs twice, while three different keys have positive counts.",
      mentalModelCorrection:
        "Frequency totals and distinct-key totals describe different properties of the same window.",
      mistakeTypes: ["frequency_and_distinct_count_conflated"],
      nextAction:
        "Count occurrences per key first, then count keys with positive frequency.",
      result: "diagnostic",
    },
  },
  {
...
In a required/formed design, what should required usually equal?`,
    ...
    feedbackModel: {
      decisionSignal:
        "formed tracks satisfied key-level requirements, so required must use the same unit.",
      mentalModelCorrection:
        "The target length and the number of distinct frequency constraints are different quantities.",
      mistakeTypes: ["required_uses_total_occurrences_instead_of_keys"],
      nextAction:
        "Define the unit counted by formed before initializing required.",
      result: "diagnostic",
    },
  },
  {
...
What should happen to formed?`,
    ...
    feedbackModel: {
      decisionSignal:
        "The key crosses the threshold from below its requirement to meeting it.",
      mentalModelCorrection:
        "formed counts satisfied distinct constraints, not individual matching occurrences.",
      mistakeTypes: ["formed_not_incremented_at_requirement_threshold"],
      nextAction:
        "Update formed only when a key crosses between unsatisfied and satisfied states.",
      result: "diagnostic",
    },
  },
  {
...
Why is this wrong?`,
    ...
    feedbackModel: {
      decisionSignal:
        "A key contributes at most one unit to a key-level formed counter.",
      mentalModelCorrection:
        "Increment formed on the transition to satisfaction, not throughout the entire satisfied range.",
      mistakeTypes: ["formed_incremented_repeatedly_for_overcount"],
      nextAction:
        "Check for equality with the required count immediately after incrementing the window count.",
      result: "diagnostic",
    },
  },
  {
...
What should happen to formed?`,
    ...
    feedbackModel: {
      ...
      mentalModelCorrection:
        "Presence alone is insufficient when the target requires multiple occurrences.",
      mistakeTypes: ["formed_not_decremented_below_requirement"],
      nextAction:
        "After decrementing an outgoing required key, test whether its count crossed below the target count.",
      result: "diagnostic",
    },
  },
  {
...
What is the correction?`,
    ...
    feedbackModel: {
      decisionSignal:
        "The key remains at or above its required frequency after the removal.",
      mentalModelCorrection:
        "formed changes only when satisfaction status changes, not after every incoming or outgoing occurrence.",
      mistakeTypes: ["formed_decremented_while_requirement_still_satisfied"],
      nextAction:
        "Compare the post-removal window count with the immutable required count.",
      result: "diagnostic",
    },
  },
  {
...
The current window also contains several x values. How should x affect formed?`,
    ...
    feedbackModel: {
      decisionSignal:
        "formed measures satisfied target constraints, not all distinct values in the current window.",
      mentalModelCorrection:
        "A window map may track irrelevant keys, but target-satisfaction state must remain scoped to required keys.",
      mistakeTypes: ["irrelevant_window_key_changes_formed"],
      nextAction:
        "Update formed only inside a check that the changed key exists in the target map.",
      result: "diagnostic",
    },
  },
  {
...
What are required and formed?`,
    ...
    feedbackModel: {
      decisionSignal:
        "There are three distinct target constraints, and exactly two currently meet their required counts.",
      mentalModelCorrection:
        "A required key is formed only when its complete multiplicity requirement is met.",
      mistakeTypes: ["required_formed_trace_mismatch"],
      nextAction:
        "Evaluate each target key independently against its required frequency.",
      result: "diagnostic",
    },
  },
  {
    ...
    type: "single_choice",
    prompt:
      "Which invariant most completely supports a sliding window using frequency maps, distinct counts, and required/formed state?",
    options: [
      {
    ...
    feedbackModel: {
      decisionSignal:
        "Every maintained variable must describe the same current interval and use a clearly defined counting unit.",
      mentalModelCorrection:
        "Frequency, distinctness, and requirement satisfaction are related but separate state dimensions that must remain synchronized.",
      mistakeTypes: ["frequency_window_state_invariant_incomplete"],
      nextAction:
        "After every incoming or outgoing update, verify map counts, zero-key deletion, distinct state, and threshold transitions.",
      result: "diagnostic",
    },
  },
];

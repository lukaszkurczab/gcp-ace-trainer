...
What result is correct under a contract that returns null when no valid window exists?`,
    ...
    feedbackModel: {
      decisionSignal:
        "A width-k result requires exactly k source elements, and the contract explicitly defines the no-result representation.",
      mentalModelCorrection:
        "Do not confuse an empty aggregate identity with the existence of a valid fixed-width window.",
      mistakeTypes: ["empty_input_treated_as_valid_window"],
      nextAction:
        "Check whether a valid candidate exists before returning an aggregate value.",
      result: "diagnostic",
    },
  },
  {
...
k = 0`,
    ...
    feedbackModel: {
      decisionSignal:
        "The parameter violates the stated positive-width precondition.",
      mentalModelCorrection:
        "Invalid input and a valid search with no result are separate contract states.",
      mistakeTypes: ["zero_window_width_not_rejected"],
...
What should it return?`,
    ...
    feedbackModel: {
      decisionSignal:
        "The requested width is valid as a parameter but cannot be realized by this input.",
      mentalModelCorrection:
        "Do not silently weaken an exact-width output contract to accept partial windows.",
      mistakeTypes: ["partial_window_returned_for_k_greater_than_n"],
...
Both windows [4, 1] have sum 5. Which best-update rule preserves the required result?`,
    ...
    feedbackModel: {
      decisionSignal:
        "The first optimum must remain stored when a later equal optimum appears.",
      mentalModelCorrection:
        "The comparison operator used for best-state updates encodes the tie contract.",
      mistakeTypes: ["latest_window_returned_under_earliest_tie_contract"],
      nextAction:
        "Use a strict improvement check when encounter order already visits candidates from earliest to latest.",
      result: "diagnostic",
    },
  },
  {
...
Which best-update rule matches that contract during a left-to-right scan?`,
    ...
    feedbackModel: {
      decisionSignal:
        "Later equal optima must replace earlier stored candidates.",
      mentalModelCorrection:
        "Earliest and latest optimum contracts require different equality handling.",
      mistakeTypes: ["earliest_window_returned_under_latest_tie_contract"],
      nextAction:
        "Define explicitly whether equality preserves or replaces the stored best candidate.",
      result: "diagnostic",
    },
  },
  {
...
What is its length?`,
    ...
    feedbackModel: {
      ...
      mentalModelCorrection:
        "Boundary formulas must match the declared inclusive or half-open representation.",
      mistakeTypes: ["inclusive_window_length_off_by_one"],
      nextAction:
        "Enumerate the concrete included indexes before selecting a formula.",
      result: "diagnostic",
    },
  },
  {
...
What is wrong?`,
    ...
    feedbackModel: {
      decisionSignal:
        "Current traversal state continues changing after the optimum may have been observed.",
      mentalModelCorrection:
        "Optimization algorithms need separate state for the current candidate and the best candidate.",
      mistakeTypes: ["current_window_boundaries_returned_as_best"],
...
What is the bug?`,
    ...
    feedbackModel: {
      decisionSignal:
        "No successful candidate ever established valid best boundaries.",
      mentalModelCorrection:
        "Default indexes are not evidence that a result exists.",
      mistakeTypes: ["missing_no_result_state"],
      nextAction:
        "Track whether any qualifying candidate has been found or use a sentinel that cannot be mistaken for a valid range.",
      result: "diagnostic",
    },
  },
  {
...
What is the problem?`,
    ...
    feedbackModel: {
      decisionSignal:
        "The public result shape is defined as coordinates in the original input.",
      mentalModelCorrection:
        "Finding the correct window does not compensate for returning the wrong representation.",
      mistakeTypes: ["window_values_returned_instead_of_boundaries"],
      nextAction:
        "Match the returned data structure to the exact contract noun.",
      result: "diagnostic",
    },
  },
  {
...
Its contract requires a new array containing the optimal window values. Which return statement satisfies the contract?`,
    ...
    feedbackModel: {
      decisionSignal:
        "Array.prototype.slice excludes its second argument, while bestRight is part of the inclusive window.",
      mentalModelCorrection:
        "Boundary convention and output representation must both be translated correctly.",
      mistakeTypes: ["inclusive_window_copy_drops_last_value"],
      nextAction:
        "Convert the inclusive end into the exclusive end required by slice.",
      result: "diagnostic",
    },
  },
  {
...
What is the main issue?`,
    ...
    feedbackModel: {
      decisionSignal:
        "Sliding windows depend on original adjacency, and in-place sorting is observable mutation.",
      mentalModelCorrection:
        "Preprocessing must preserve every input property and side-effect constraint required by the contract.",
      mistakeTypes: [
        "window_algorithm_mutates_read_only_input",
        "sorting_destroys_contiguous_window_identity",
      ],
      nextAction:
        "Avoid reordering preprocessing when the problem is defined over original contiguous positions.",
      result: "diagnostic",
    },
  },
  {
    ...
    prompt:
      "Which invariant most completely supports a sliding-window function with explicit no-result behavior and a defined tie rule?",
    options: [
      {
    ...
    feedbackModel: {
      decisionSignal:
        "Correctness includes candidate validity, synchronized state, best-result identity, tie behavior, existence, and output representation.",
      mentalModelCorrection:
        "Window mechanics are only one part of the observable function contract.",
      mistakeTypes: ["window_edge_output_contract_invariant_incomplete"],
      nextAction:
        "Verify existence, identity, tie-breaking, boundary convention, representation, and mutation behavior separately.",
      result: "diagnostic",
    },
  },
];

export const recognizeSlidingWindowSignalQuestions = [
  {
    ...
    type: "single_choice",
    prompt:
      "Which problem property most strongly suggests a sliding-window strategy?",
    options: [
      {
    ...
    feedbackModel: {
      decisionSignal:
        "A window represents every element in one active contiguous interval whose boundaries move through the input.",
      mentalModelCorrection:
        "Two boundaries alone do not define a sliding window; the complete interval and its maintained state must form the current candidate.",
      mistakeTypes: ["sliding_window_signal_not_recognized"],
      nextAction:
        "Identify whether every position between the boundaries belongs to the current candidate.",
      result: "diagnostic",
    },
  },
  {
    ...
    type: "single_choice",
    prompt:
      "A task asks for the maximum sum among every contiguous block of exactly k elements. Which signal is present?",
    options: [
      {
    ...
    feedbackModel: {
      decisionSignal:
        "The candidate length is fixed, and adjacent candidates overlap in k - 1 positions.",
      mentalModelCorrection:
        "Fixed-size windows should usually reuse state rather than recompute each complete block.",
      mistakeTypes: ["fixed_size_window_signal_missed"],
      nextAction:
        "Name the value leaving and the value entering when the interval shifts once.",
      result: "diagnostic",
    },
  },
  {
...
Which classification is correct?`,
    ...
    feedbackModel: {
      decisionSignal:
        "A evolves one active candidate interval; B evaluates unrelated requested intervals from precomputed cumulative state.",
      mentalModelCorrection:
        "Sliding windows traverse overlapping candidates, while prefix sums provide direct aggregate access for arbitrary ranges.",
      mistakeTypes: ["sliding_window_and_prefix_sums_conflated"],
      nextAction:
        "Ask whether the boundaries evolve as one scan or arrive as independent queries.",
      result: "diagnostic",
    },
  },
  {
...
Which mapping is correct?`,
    ...
    feedbackModel: {
      decisionSignal:
        "A evaluates the complete contiguous interior, while B evaluates two selected positions.",
      mentalModelCorrection:
        "Classify the semantic candidate rather than the names or number of pointer variables.",
      mistakeTypes: ["sliding_window_and_pair_two_pointers_conflated"],
      nextAction:
        "List which input elements participate in evaluating one current candidate.",
      result: "diagnostic",
    },
  },
  {
    ...
    type: "single_choice",
    prompt:
      "A property must be checked for every contiguous subarray, but a failed interval gives no information about whether moving left or right preserves possible answers. What is the best assessment?",
    options: [
      {
    ...
    feedbackModel: {
      decisionSignal:
        "No comparison supplies a safe rule for permanently discarding intervals associated with one boundary.",
      mentalModelCorrection:
        "Contiguity is necessary for a window candidate but does not by itself justify variable-size boundary movement.",
      mistakeTypes: ["sliding_window_forced_without_movement_rule"],
      nextAction:
        "State what becomes impossible after each proposed boundary movement.",
      result: "diagnostic",
    },
  },
  {
...
What is wrong?`,
    ...
    feedbackModel: {
      decisionSignal:
        "Interior membership is optional for the requested selection.",
      mentalModelCorrection:
        "A pair of outer indexes does not turn a non-contiguous selection into a window.",
      mistakeTypes: ["noncontiguous_selection_called_sliding_window"],
      nextAction:
        "Check whether every index between the candidate boundaries must be selected.",
      result: "diagnostic",
    },
  },
  {
...
What is the central flaw?`,
    ...
    feedbackModel: {
      decisionSignal:
        "The maintained sum is not monotonic under expansion or contraction.",
      mentalModelCorrection:
        "An ordinary target-sum window relies on value-domain constraints; mixed-sign exact sums commonly require prefix-sum state instead.",
      mistakeTypes: ["mixed_sign_exact_sum_uses_invalid_window"],
      nextAction:
        "Test whether adding or removing one permitted value always moves the sum in a predictable direction.",
      result: "diagnostic",
    },
  },
  {
...
Why is the ordinary window rule unsafe?`,
    ...
    feedbackModel: {
      decisionSignal:
        "The effect of removing the outgoing value is not directionally predictable.",
      mentalModelCorrection:
        "Shortest-window contraction is valid only when shrinking has a known effect on the maintained constraint.",
      mistakeTypes: ["mixed_sign_shortest_sum_uses_invalid_window"],
      nextAction:
        "Prove that removing the leftmost permitted value changes the state monotonically.",
      result: "diagnostic",
    },
  },
  {
    ...
    type: "single_choice",
    prompt:
      "Which constraint makes a variable-size sum window especially plausible for finding the longest contiguous subarray with sum at most target?",
    options: [
      {
    ...
    feedbackModel: {
      decisionSignal:
        "The value domain makes the sum respond predictably to adding and removing elements.",
      mentalModelCorrection:
        "Sliding-window validity often depends on input constraints that create monotonic boundary effects.",
      mistakeTypes: ["valid_nonnegative_sum_window_signal_missed"],
      nextAction:
        "Describe how the maintained sum changes under each possible boundary movement.",
      result: "diagnostic",
    },
  },
  {
...
What sliding-window opportunity was missed?`,
    ...
    feedbackModel: {
      decisionSignal:
        "Consecutive fixed-size windows overlap except for one departing and one arriving value.",
      mentalModelCorrection:
        "A rolling window carries forward the aggregate instead of rebuilding it from the overlapping contents.",
      mistakeTypes: ["fixed_size_windows_repeatedly_rescanned"],
      nextAction:
        "Write the next state as previous state minus outgoing plus incoming.",
      result: "diagnostic",
    },
  },
  {
...
What is the best correction?`,
    ...
    feedbackModel: {
      decisionSignal:
        "Variable names reveal no invariant about the interval between them.",
      mentalModelCorrection:
        "A window is defined by candidate membership and maintained state, not surface syntax.",
      mistakeTypes: ["sliding_window_classified_by_variable_names"],
      nextAction:
        "State what the interval contains and whether its complete contents form the candidate.",
      result: "diagnostic",
    },
  },
  {
    ...
    type: "single_choice",
    prompt:
      "Which property makes a contiguous-range condition suitable for efficient sliding-window maintenance?",
    options: [
      {
    ...
    feedbackModel: {
      decisionSignal:
        "Window movement changes membership locally at its boundaries.",
      mentalModelCorrection:
        "The pattern is strongest when candidate state can be synchronized with those local membership changes.",
      mistakeTypes: ["incremental_window_state_signal_missed"],
      nextAction:
        "Define explicit add-incoming and remove-outgoing operations for the maintained state.",
      result: "diagnostic",
    },
  },
  {
    ...
    learningStage: "foundations",
    primarySkillAtomId:
      "distinguish_fixed_window_from_monotonic_variable_window",
    secondarySkillAtomIds: [
...
Which assessment is correct?`,
    ...
    feedbackModel: {
      decisionSignal:
        "A has a predetermined one-step shift, while B needs the sum comparison to justify an irreversible boundary choice.",
      mentalModelCorrection:
        "Mixed signs break many variable-size sum rules but do not prevent rolling state across fixed-size windows.",
      mistakeTypes: ["fixed_and_variable_sum_window_preconditions_conflated"],
      nextAction:
        "Separate scheduled fixed-size movement from comparison-driven variable-size movement.",
      result: "diagnostic",
    },
  },
  {
...
Which mapping is correct?`,
    ...
    feedbackModel: {
      decisionSignal:
        "Each task differs in candidate membership, maintained state, query workload, or legality of boundary movement.",
      mentalModelCorrection:
        "Sliding-window recognition requires both a contiguous active candidate and a correct boundary-update model.",
      mistakeTypes: ["sliding_window_contrast_signals_conflated"],
      nextAction:
        "For each task, identify candidate membership, state ownership, and the proof supporting its next boundary movement.",
      result: "diagnostic",
    },
  },
];

...
Which classification is most appropriate?`,
    ...
    feedbackModel: {
      decisionSignal:
        "A maintains and changes one active contiguous candidate; B computes independent range aggregates from preprocessed cumulative state.",
      mentalModelCorrection:
        "Sliding windows optimize a moving interval, while prefix sums answer range queries without simulating interval movement.",
      mistakeTypes: ["sliding_window_and_prefix_sum_conflated"],
      nextAction:
        "Ask whether boundaries evolve as one candidate or identify independent query ranges.",
      result: "diagnostic",
    },
  },
  {
    ...
    type: "single_choice",
    prompt:
      "An array contains positive and negative values. The task asks for the sum of thousands of arbitrary contiguous ranges without modifying the array. Which technique is the strongest default?",
    options: [
      {
        id: "prefix_sums",
    ...
    feedbackModel: {
      decisionSignal:
        "The input is static and the workload consists of many independent contiguous range-sum queries.",
      mentalModelCorrection:
        "Prefix sums work with mixed signs because they compute exact aggregates rather than relying on monotonic window movement.",
      mistakeTypes: ["range_queries_assigned_to_sliding_window"],
      nextAction:
        "Separate preprocessing for arbitrary queries from online optimization of one moving interval.",
      result: "diagnostic",
    },
  },
  {
...
Which distinction is correct?`,
    ...
    feedbackModel: {
      ...
      mentalModelCorrection:
        "The same pointer names can represent either an active interval or independent endpoint candidates.",
      mistakeTypes: ["window_and_pair_two_pointers_classified_by_shape"],
      nextAction:
        "List exactly which elements contribute to evaluating the current candidate.",
      result: "diagnostic",
    },
  },
  {
...
Which mapping is correct?`,
    ...
    feedbackModel: {
      decisionSignal:
        "Difference arrays encode boundary changes for updates; prefix sums encode cumulative state for queries.",
      mentalModelCorrection:
        "Both use cumulative reconstruction ideas, but they solve opposite workload directions.",
      mistakeTypes: ["difference_array_and_prefix_sum_conflated"],
...
Why is this ordinary sliding-window rule unsafe?`,
    ...
    feedbackModel: {
      decisionSignal:
        "The maintained aggregate lacks the monotonic behavior required by the proposed expand-and-shrink rule.",
      mentalModelCorrection:
        "Contiguity alone does not justify a sliding window; boundary movement must preserve all possible answers.",
      mistakeTypes: ["mixed_sign_sum_uses_nonmonotonic_window"],
      nextAction:
        "Test how adding or removing one negative value changes the maintained sum.",
      result: "diagnostic",
    },
  },
  {
    ...
    type: "mistake_review",
    prompt:
      "A task asks for the longest subsequence satisfying a condition, and selected elements may skip arbitrary indexes. A reviewer recommends a sliding window because the result has a beginning and an end. What is wrong?",
    options: [
      {
    ...
    feedbackModel: {
      decisionSignal:
        "Interior membership is optional for a subsequence but mandatory for a window.",
      mentalModelCorrection:
        "A first and last selected position do not turn a non-contiguous selection into a contiguous candidate.",
      mistakeTypes: ["noncontiguous_subsequence_called_sliding_window"],
      nextAction:
        "Check whether every index between the candidate boundaries must belong to the candidate.",
      result: "diagnostic",
    },
  },
  {
...
What is the best correction?`,
    ...
    feedbackModel: {
      decisionSignal:
        "The task description does not yet establish a monotonic or otherwise safe boundary-update invariant.",
      mentalModelCorrection:
        "Pattern recognition should use candidate semantics and movement proofs rather than isolated keywords.",
      mistakeTypes: ["sliding_window_selected_by_problem_label"],
      nextAction:
        "Explain what state changes when each boundary moves and why no valid answer is lost.",
      result: "diagnostic",
    },
  },
  {
...
What is the central mistake?`,
    ...
    feedbackModel: {
      decisionSignal:
        "The proposed window lacks a correctness argument even though its loop may be linear.",
      mentalModelCorrection:
        "Complexity compares valid algorithms; it cannot repair an invalid invariant.",
      mistakeTypes: ["strategy_selected_by_big_o_alone"],
...
B. Find the longest contiguous subarray with sum at most target when every value is nonnegative.`,
    ...
    feedbackModel: {
      decisionSignal:
        "Mixed signs break ordinary sum-window monotonicity in A, while nonnegative values make exceeding and restoring the bound monotonic in B.",
      mentalModelCorrection:
        "The same sum vocabulary can require different techniques depending on input constraints and output contract.",
      mistakeTypes: ["sum_problem_constraints_ignored_in_strategy_choice"],
      nextAction:
        "Check how adding or removing one element affects the constraint under the stated value domain.",
      result: "diagnostic",
    },
  },
  {
...
Which mapping is correct?`,
    ...
    feedbackModel: {
      decisionSignal:
        "Each technique is defined by a different state model, workload, or candidate-elimination invariant.",
      mentalModelCorrection:
        "Range boundaries and similar complexity do not make these techniques equivalent.",
      mistakeTypes: ["range_techniques_and_invalid_window_conflated"],
      nextAction:
        "For each task, identify whether it maintains, queries, updates, compares, or invalidly discards a range.",
      result: "diagnostic",
    },
  },
];

...
If left = 2 and right = 5, which indexes belong to the window?`,
    ...
    feedbackModel: {
      decisionSignal:
        "Both endpoints are included under the inclusive convention.",
      mentalModelCorrection:
        "An inclusive window contains every index from left through right.",
      mistakeTypes: ["inclusive_right_boundary_treated_as_exclusive"],
      nextAction:
        "Enumerate the concrete indexes represented by the interval before deriving formulas.",
      result: "diagnostic",
    },
  },
  {
...
Which formula gives its length?`,
    ...
    feedbackModel: {
      ...
      mentalModelCorrection:
        "The difference measures gaps between indexes; the additional one counts the starting position.",
      mistakeTypes: ["inclusive_window_length_missing_one"],
      nextAction:
        "Test the formula on a one-element window where left === right.",
      result: "diagnostic",
    },
  },
  {
...
If left = 2 and right = 5, which indexes belong to the window?`,
    ...
    feedbackModel: {
      decisionSignal:
        "left is included, while right is the first position outside the window.",
      mentalModelCorrection:
        "A half-open interval stops immediately before its right boundary.",
      mistakeTypes: ["half_open_right_boundary_treated_as_inclusive"],
      nextAction:
        "Translate [left, right) into indexes from left through right - 1.",
      result: "diagnostic",
    },
  },
  {
...
Which formula gives its length?`,
    ...
    feedbackModel: {
      decisionSignal:
        "The exclusive right boundary is also the position immediately after the final contained element.",
      mentalModelCorrection:
        "Half-open intervals encode their length directly as end minus start.",
      mistakeTypes: ["half_open_window_length_uses_inclusive_formula"],
...
Which representations are correct?`,
    ...
    feedbackModel: {
      decisionSignal:
        "An inclusive singleton has equal endpoints, while a half-open singleton has boundaries one position apart.",
      mentalModelCorrection:
        "The same logical window has different endpoint values under different boundary conventions.",
      mistakeTypes: ["singleton_window_boundary_conventions_conflated"],
      nextAction:
        "Apply each convention's length formula and confirm that the result is one.",
      result: "diagnostic",
    },
  },
  {
    ...
    type: "solution_comparison",
    prompt:
      "Which statement correctly compares a canonical empty window under inclusive and half-open conventions?",
    options: [
      {
    ...
    feedbackModel: {
      decisionSignal:
        "Equal inclusive endpoints contain one element, while equal half-open boundaries contain none.",
      mentalModelCorrection:
        "Emptiness must be derived from the interval convention rather than from pointer equality alone.",
      mistakeTypes: ["empty_window_representation_conflated"],
      nextAction:
        "Compute the length implied by each convention when left === right.",
      result: "diagnostic",
    },
  },
  {
...
What is wrong?`,
    ...
    feedbackModel: {
      decisionSignal:
        "Both boundary positions belong to the represented window.",
      mentalModelCorrection: "An inclusive interval requires right - left + 1.",
      mistakeTypes: ["inclusive_window_length_off_by_one"],
      nextAction:
        "Test the formula when both boundaries point to the same element.",
      result: "diagnostic",
    },
  },
  {
...
What is wrong?`,
    ...
    feedbackModel: {
      decisionSignal:
        "right identifies the first position outside the logical window.",
      mentalModelCorrection: "The half-open length is exactly right - left.",
      mistakeTypes: ["half_open_window_length_off_by_one"],
...
The algorithm expands by incrementing right. Which update sequence keeps a running sum synchronized?`,
    ...
    feedbackModel: {
      decisionSignal:
        "After the increment, the new right index becomes part of the inclusive window.",
      mentalModelCorrection:
        "The incoming contribution must match the element newly included by the boundary movement.",
      mistakeTypes: ["inclusive_expansion_state_not_synchronized"],
      nextAction:
        "Identify the membership difference between the old and new intervals.",
      result: "diagnostic",
    },
  },
  {
...
Which expansion sequence is coherent?`,
    ...
    feedbackModel: {
      decisionSignal:
        "The old exclusive boundary identifies the next element that will enter.",
      mentalModelCorrection:
        "For [left, right), add the element at right before moving the exclusive boundary past it.",
      mistakeTypes: ["half_open_expansion_uses_wrong_incoming_index"],
      nextAction:
        "Compare the old interval [left, right) with the expanded interval [left, right + 1).",
      result: "diagnostic",
    },
  },
  {
    ...
    type: "subgoal_ordering",
    prompt:
      "Under either inclusive or half-open conventions, which sequence correctly shrinks a non-empty window from the left?",
    options: [
      {
    ...
    feedbackModel: {
      ...
      mentalModelCorrection:
        "State mutation must describe the membership change caused by the boundary update.",
      mistakeTypes: ["left_contraction_removes_wrong_element"],
      nextAction:
        "Remove the element before changing the index that identifies it.",
      result: "diagnostic",
    },
  },
  {
...
What is the correct review?`,
    ...
    feedbackModel: {
      decisionSignal:
        "The aggregate contains an element outside the represented interval.",
      mentalModelCorrection:
        "Window-local state must describe exactly the elements currently between the boundaries.",
      mistakeTypes: ["window_state_contains_stale_prefix_element"],
      nextAction:
        "Audit every boundary movement for a matching incoming or outgoing state update.",
      result: "diagnostic",
    },
  },
  {
...
What invariant is broken?`,
    ...
    feedbackModel: {
      decisionSignal:
        "The endpoint convention includes right, but the state update does not.",
      mentalModelCorrection:
        "Boundary semantics and state membership must use the same convention at every observation point.",
      mistakeTypes: ["incoming_right_value_missing_from_window_state"],
      nextAction:
        "Add the incoming endpoint before evaluating window validity.",
      result: "diagnostic",
    },
  },
  {
    ...
    learningStage: "foundations",
    primarySkillAtomId:
      "distinguish_window_boundaries_from_independent_pointers",
    secondarySkillAtomIds: [
...
What is the best correction?`,
    ...
    feedbackModel: {
      decisionSignal:
        "The candidate evaluated by a sliding window is the complete contiguous interval.",
      mentalModelCorrection:
        "Window boundaries jointly define membership; they are not independent selected positions.",
      mistakeTypes: [
        "sliding_window_boundaries_treated_as_independent_candidates",
      ],
      nextAction:
        "List every index whose contribution must be represented by the current state.",
      result: "diagnostic",
    },
  },
  {
...
What is the problem?`,
    ...
    feedbackModel: {
      decisionSignal:
        "The membership updates and length calculation assume different meanings for right.",
      mentalModelCorrection:
        "Choose one interval convention and derive all accesses, updates, lengths, and termination rules from it.",
      mistakeTypes: ["window_boundary_conventions_mixed"],
      nextAction:
        "Document the interval notation and verify every formula against it.",
      result: "diagnostic",
    },
  },
  {
    ...
    prompt:
      "Which invariant most completely supports a sliding-window implementation?",
    options: [
      {
    ...
    feedbackModel: {
      decisionSignal:
        "Correctness requires one coherent definition of membership shared by boundaries, state, length, and updates.",
      mentalModelCorrection:
        "A window is not correct when its pointers, aggregate, and formulas each describe different regions.",
      mistakeTypes: ["window_boundary_state_invariant_incomplete"],
      nextAction:
        "After every expansion or contraction, verify the represented indexes, computed length, and maintained state together.",
      result: "diagnostic",
    },
  },
];

export const keyIdentityAndCompositeKeysQuestions = [
  {
    ...
    feedbackModel: {
      decisionSignal:
        "Map accepts primitive and reference values without coercing all keys into strings.",
      mentalModelCorrection:
        "Primitive equality and object identity use different runtime semantics. Equal-looking objects are not automatically the same key.",
      mistakeTypes: ["javascript_map_key_equality_mismatch"],
      nextAction:
        "Classify the proposed key as a primitive value or a reference value before predicting lookup behavior.",
      result: "diagnostic",
    },
  },
  {
...
state.set(true, "boolean");`,
    ...
    feedbackModel: {
      decisionSignal:
        "Map does not apply loose equality or string coercion when comparing keys.",
      mentalModelCorrection:
        "Numeric, string, and boolean values remain distinct keys even when their textual representations resemble one another.",
      mistakeTypes: ["map_keys_assumed_to_be_string_coerced"],
...
state.set(-0, "negative zero");`,
    ...
    feedbackModel: {
      decisionSignal:
        "JavaScript Map uses SameValueZero rather than === for numeric key comparison.",
      mentalModelCorrection:
        "Under SameValueZero, NaN equals NaN for key matching, and positive and negative zero share one key.",
      mistakeTypes: ["same_value_zero_behavior_missed"],
      nextAction:
        "Account explicitly for NaN and signed zero when numeric edge cases affect key identity.",
      result: "diagnostic",
    },
  },
  {
...
const result = state.get({ id: 7 });`,
    ...
    feedbackModel: {
      ...
      mentalModelCorrection:
        "Equal property contents do not imply identical object keys in JavaScript or TypeScript.",
      mistakeTypes: ["object_keys_assumed_structurally_equal"],
      nextAction:
        "Check whether insertion and lookup reuse the exact same object instance.",
      result: "diagnostic",
    },
  },
  {
...
const result = state.get(key);`,
    ...
    feedbackModel: {
      decisionSignal:
        "The exact reference stored as the key is reused for lookup.",
      mentalModelCorrection:
        "Object keys are valid and stable as long as callers retain and reuse the same identity.",
      mistakeTypes: ["same_reference_lookup_not_recognized"],
      nextAction:
        "Track reference identity separately from the contents stored inside the referenced object.",
      result: "diagnostic",
    },
  },
  {
...
Why is result undefined?`,
    ...
    feedbackModel: {
      decisionSignal:
        "TypeScript tuple syntax changes static typing but not JavaScript's runtime array identity semantics.",
      mentalModelCorrection:
        "A tuple type does not create structural key equality. Fresh tuple literals remain distinct references.",
      mistakeTypes: ["typescript_tuple_assumed_structural_map_key"],
      nextAction:
        "Use a canonical primitive encoding, a nested Map, or a reused tuple reference.",
      result: "diagnostic",
    },
  },
  {
...
Which representation avoids string encoding while preserving both component identities?`,
    ...
    feedbackModel: {
      decisionSignal:
        "Each component already has a valid primitive key type and can be looked up independently.",
      mentalModelCorrection:
        "Nested Maps represent composite keys without flattening component types into an ambiguous string.",
      mistakeTypes: ["unsafe_composite_key_representation"],
      nextAction:
        "Use one Map level per composite-key component when structured lookup is clearer than serialization.",
      result: "diagnostic",
    },
  },
  {
...
Which two different input pairs demonstrate an application-level collision?`,
    ...
    feedbackModel: {
      decisionSignal:
        "The encoding removes information about where the first component ends.",
      mentalModelCorrection:
        "Different composite values can become the exact same primitive key before Map hashing is even involved.",
      mistakeTypes: ["component_boundaries_lost_in_key"],
      nextAction:
        "Use structured serialization, escaping, length prefixes, or nested Maps.",
      result: "diagnostic",
    },
  },
  {
...
The strings themselves may contain "|". Which review is correct?`,
    ...
    feedbackModel: {
      decisionSignal:
        "The delimiter belongs to the supported component alphabet and is not escaped.",
      mentalModelCorrection:
        "A separator is safe only when the encoding makes component boundaries unambiguous for every valid input.",
      mistakeTypes: ["delimiter_collision_not_considered"],
      nextAction:
        "Define escaping, length-prefixing, or structured serialization as part of the key format.",
      result: "diagnostic",
    },
  },
  {
    ...
    type: "single_choice",
    prompt:
      "For which explicitly stated domain is JSON.stringify([first, second]) a straightforward collision-safe key for an ordered pair?",
    options: [
      {
    ...
    feedbackModel: {
      decisionSignal:
        "The safety of a serialized key depends on the exact supported value domain.",
      mentalModelCorrection:
        "JSON serialization is useful for some controlled primitive structures, but it is not a universal structural-key function.",
      mistakeTypes: ["serialized_key_domain_not_specified"],
      nextAction:
        "Document supported types and special values before adopting a serialization format.",
      result: "diagnostic",
    },
  },
  {
    ...
    type: "single_choice",
    prompt:
      'The pairs ["alice", "bob"] and ["bob", "alice"] should represent the same undirected relationship. How should the key be constructed?',
    options: [
      {
    ...
    feedbackModel: {
      decisionSignal:
        "Component order is irrelevant to the application's equivalence relation.",
      mentalModelCorrection:
        "Canonicalization must remove irrelevant permutations before the components are serialized or indexed.",
      mistakeTypes: ["unordered_pair_not_canonicalized"],
      nextAction:
        "Normalize equivalent component orders into one deterministic representation.",
      result: "diagnostic",
    },
  },
  {
...
Why can JSON.stringify(item) be an insufficient canonical key?`,
    ...
    feedbackModel: {
      decisionSignal:
        "The intended equivalence ignores construction history, but raw serialization may preserve property enumeration order.",
      mentalModelCorrection:
        "Structural canonicalization requires a deterministic property order, often recursively for nested values.",
      mistakeTypes: ["raw_object_serialization_assumed_canonical"],
      nextAction:
        "Sort relevant property names and canonicalize nested structures before serialization.",
      result: "diagnostic",
    },
  },
  {
...
Which review is correct?`,
    ...
    feedbackModel: {
      decisionSignal:
        "Ordinary JavaScript object property keys are strings or symbols, while Map preserves arbitrary key values.",
      mentalModelCorrection:
        "TypeScript Record typing does not change JavaScript's runtime property-key coercion.",
      mistakeTypes: ["record_and_map_key_semantics_conflated"],
      nextAction:
        "Choose Map when runtime key type identity must remain observable.",
      result: "diagnostic",
    },
  },
  {
...
Which design matches that requirement?`,
    ...
    feedbackModel: {
      decisionSignal:
        "The required equality is content-based rather than reference-based.",
      mentalModelCorrection:
        "JavaScript Map does not provide structural object equality. That behavior must be implemented through canonicalization, interning, or an explicit structural lookup design.",
      mistakeTypes: ["reference_key_used_for_structural_contract"],
      nextAction:
        "Define a deterministic representation of every equivalence-relevant nested value.",
      result: "diagnostic",
    },
  },
];

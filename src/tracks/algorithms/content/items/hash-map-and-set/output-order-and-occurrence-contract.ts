export const outputOrderAndOccurrenceContractQuestions = [
  {
    ...
    type: "single_choice",
    prompt:
      "A Map provides fast lookup by key. What must be checked separately before using its entries as the final output?",
    options: [
      {
    ...
    feedbackModel: {
      decisionSignal:
        "Efficient access and required presentation order are separate concerns.",
      mentalModelCorrection:
        "A keyed collection can retrieve state by key without arranging results according to a requested ordering contract.",
      mistakeTypes: ["keyed_lookup_assumed_to_satisfy_output_order"],
      nextAction:
        "State the required output order explicitly before choosing how to materialize results.",
      result: "diagnostic",
    },
  },
  {
...
const keys = [...counts.keys()];`,
    ...
    feedbackModel: {
      ...
      mentalModelCorrection:
        "Insertion order is deterministic, but it is not the same as ascending key order.",
      mistakeTypes: ["map_insertion_order_confused_with_sorted_order"],
      nextAction:
        "Sort the materialized keys explicitly when the output contract requires numeric order.",
      result: "diagnostic",
    },
  },
  {
...
[...new Set([4, 2, 4, 3, 2])]`,
    ...
    feedbackModel: {
      decisionSignal:
        "The first insertion establishes a distinct value's iteration position; repeated additions do not create new entries.",
      mentalModelCorrection:
        "Set deduplication naturally preserves first distinct encounter order, not sorted order or every occurrence.",
      mistakeTypes: ["set_iteration_order_misinterpreted"],
...
Which update policy matches the contract?`,
    ...
    feedbackModel: {
      decisionSignal:
        "Later occurrences must not replace the earliest source position.",
      mentalModelCorrection:
        "Duplicate-update policy determines whether a Map represents first occurrence, last occurrence, or another selected occurrence.",
      mistakeTypes: ["first_occurrence_overwritten"],
      nextAction:
        "Define whether an existing key should be preserved or replaced before writing the update.",
      result: "diagnostic",
    },
  },
  {
...
Which Map update is appropriate while scanning left to right?`,
    ...
    feedbackModel: {
      decisionSignal:
        "Each later observation is a newer candidate for the required last position.",
      mentalModelCorrection:
        "Overwriting is correct when the stored meaning is explicitly the most recent index.",
      mistakeTypes: ["last_occurrence_not_updated"],
      nextAction:
        "Maintain the invariant that each stored index equals the latest processed occurrence.",
      result: "diagnostic",
    },
  },
  {
...
["b", "a", "b"].forEach((value, index) => {
  lastIndex.set(value, index);
    ...
    feedbackModel: {
      decisionSignal:
        "Updating an existing Map key changes its associated value but does not change its original insertion position.",
      mentalModelCorrection:
        "A Map can simultaneously preserve first key-insertion order and store the latest metadata for that key.",
      mistakeTypes: ["map_overwrite_assumed_to_reorder_key"],
      nextAction:
        "Track key order and associated values as separate dimensions of Map state.",
      result: "diagnostic",
    },
  },
  {
...
const keys = [...state.keys()];`,
    ...
    feedbackModel: {
      decisionSignal:
        "Deletion removes the key and its existing position from the Map.",
      mentalModelCorrection:
        "Updating an existing key preserves order, but deleting and later re-adding it places it at the end.",
      mistakeTypes: ["map_reinsertion_order_mismatch"],
      nextAction:
        "Distinguish set-on-existing-key from delete-followed-by-new-insertion.",
      result: "diagnostic",
    },
  },
  {
...
Which design is correct?`,
    ...
    feedbackModel: {
      decisionSignal:
        "The required order is key-based rather than encounter-based.",
      mentalModelCorrection:
        "The internal representation may optimize updates while a separate materialization step enforces presentation order.",
      mistakeTypes: ["map_iteration_used_for_sorted_output"],
      nextAction:
        "Apply an explicit comparator to the final entries when sorted output is required.",
      result: "diagnostic",
    },
  },
  {
    ...
    prompt:
      "A function must return each distinct value once, ordered by its first appearance in the input. Which representation naturally supports that contract?",
    options: [
      {
    ...
    feedbackModel: {
      decisionSignal:
        "First distinct insertion order and the required output order are the same.",
      mentalModelCorrection:
        "Insertion-order iteration is useful when the contract explicitly asks for encounter order, but not for unrelated orderings.",
      mistakeTypes: ["encounter_order_contract_not_recognized"],
      nextAction:
        "Match the collection's iteration semantics to the exact requested order.",
      result: "diagnostic",
    },
  },
  {
...
Which approach satisfies the contract?`,
    ...
    feedbackModel: {
      decisionSignal:
        "The ordering field is original-position metadata, not the key itself.",
      mentalModelCorrection:
        "When output order depends on associated metadata, preserve that metadata and order results explicitly by it.",
      mistakeTypes: ["original_index_order_not_materialized"],
      nextAction:
        "Store the position that defines output precedence and use it as the final sort key.",
      result: "diagnostic",
    },
  },
  {
...
What is missing?`,
    ...
    feedbackModel: {
      decisionSignal:
        "The requested order depends on computed bucket metadata and an explicit tie-breaking rule.",
      mentalModelCorrection:
        "Deterministic insertion order does not imply conformance to an arbitrary deterministic output specification.",
      mistakeTypes: ["deterministic_iteration_confused_with_required_order"],
      nextAction:
        "Encode both the primary comparator and every required tie-breaker.",
      result: "diagnostic",
    },
  },
  {
...
Which design best matches the contract?`,
    ...
    feedbackModel: {
      decisionSignal:
        "Each key requires two occurrence fields, and the final ordering depends on one of those fields.",
      mentalModelCorrection:
        "The Map stores lookup metadata; a separate ordering step enforces the returned sequence contract.",
      mistakeTypes: ["state_and_output_order_contract_under_modeled"],
      nextAction:
        "List the metadata needed per key and the field that determines final result order.",
      result: "diagnostic",
    },
  },
];

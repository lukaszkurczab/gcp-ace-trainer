export const chooseMapVsSetStateQuestions = [
  {
    ...
    type: "single_choice",
    prompt:
      'An algorithm only needs to answer: "Have I already seen this value?" It does not need a count, index, or metadata. Which state representation best matches the contract?',
    options: [
      {
    ...
    feedbackModel: {
      decisionSignal:
        "Membership is the only query the algorithm needs to support.",
      mentalModelCorrection:
        "When the state for a key is only present or absent, the key itself carries all required information.",
      mistakeTypes: ["needless_map_for_presence_state"],
      nextAction:
        "List the exact question asked about each key before selecting the collection.",
      result: "diagnostic",
    },
  },
  {
...
What is the best review?`,
    ...
    feedbackModel: {
      decisionSignal:
        "Every stored value in the Map is the same constant boolean.",
      mentalModelCorrection:
        "A Map is useful when its associated value carries information beyond key membership.",
      mistakeTypes: ["constant_boolean_map_value"],
      nextAction:
        "Remove stored values that do not distinguish one key's state from another.",
      result: "diagnostic",
    },
  },
  {
    ...
    type: "single_choice",
    prompt:
      "An algorithm must know how many times each value currently occurs. Which representation matches that requirement?",
    options: [
      {
    ...
    feedbackModel: {
      decisionSignal:
        "Different keys may have different counts, and the algorithm must preserve those values.",
      mentalModelCorrection:
        "A Set can distinguish zero occurrences from at least one occurrence, but not one occurrence from several.",
      mistakeTypes: ["set_used_when_frequency_required"],
      nextAction:
        "Use a key-value structure whenever each key needs its own numeric state.",
      result: "diagnostic",
    },
  },
  {
...
Which state is sufficient?`,
    ...
    feedbackModel: {
      decisionSignal:
        "Removing one occurrence does not necessarily change the key from present to absent.",
      mentalModelCorrection:
        "Presence-only state is insufficient when several active occurrences can share one key.",
      mistakeTypes: ["duplicate_multiplicity_lost_by_set"],
      nextAction:
        "Ask whether one removal always implies that the key is no longer represented.",
      result: "diagnostic",
    },
  },
  {
    ...
    type: "single_choice",
    prompt:
      "A pair-search function must return the original index of a previously seen complement. Which state should it maintain?",
    options: [
      {
    ...
    feedbackModel: {
      decisionSignal:
        "The output requires positional information about the matching key.",
      mentalModelCorrection:
        "A Set can prove that a complement exists but cannot recover the index associated with it.",
      mistakeTypes: ["presence_state_disconnected_from_index_output"],
      nextAction:
        "Store every piece of information that must later be returned or used to construct the result.",
      result: "diagnostic",
    },
  },
  {
    ...
    type: "single_choice",
    prompt:
      "A function must return every original index at which each value appears. Which representation fits the output contract?",
    options: [
      {
    ...
    feedbackModel: {
      decisionSignal:
        "Each key needs a collection of associated positions rather than a presence flag or one scalar.",
      mentalModelCorrection:
        "The Map value should model the complete information required for that key.",
      mistakeTypes: ["map_value_too_weak_for_output_contract"],
      nextAction:
        "Determine whether each key maps to one result item or a group of result items.",
      result: "diagnostic",
    },
  },
  {
    ...
    type: "single_choice",
    prompt:
      "Words must be grouped by a computed signature, with every word having the same signature placed in the same group. Which state is appropriate?",
    options: [
      {
    ...
    feedbackModel: {
      decisionSignal:
        "The algorithm must retain multiple members associated with each grouping key.",
      mentalModelCorrection:
        "Presence of a group is not enough when the output must contain the group's members.",
      mistakeTypes: ["set_used_for_grouping_contract"],
      nextAction:
        "Choose the Map value type from what must accumulate under each key.",
      result: "diagnostic",
    },
  },
  {
    ...
    type: "single_choice",
    prompt:
      "For every category, an algorithm must retain the highest score observed in that category. Which representation best matches the state?",
    options: [
      {
    ...
    feedbackModel: {
      ...
      mentalModelCorrection:
        "Presence state cannot answer a query about the best associated value for a key.",
      mistakeTypes: ["presence_state_used_for_per_key_optimum"],
      nextAction:
        "Model the exact aggregate that must be updated independently for each key.",
      result: "diagnostic",
    },
  },
  {
    ...
    type: "single_choice",
    prompt:
      "For each user ID, an algorithm must retain a status, last-seen timestamp, and retry count. Which state is appropriate?",
    options: [
      {
    ...
    feedbackModel: {
      decisionSignal:
        "Each key owns several associated fields that may differ between keys.",
      mentalModelCorrection:
        "A Map value may be structured metadata when one scalar cannot represent the required per-key state.",
      mistakeTypes: ["set_used_when_key_metadata_required"],
  ...
  },
  {
    ...
    type: "single_choice",
    prompt:
      "A function emits only the first occurrence of each value while scanning left to right. It only needs to know whether a value has already been emitted. Which state is sufficient?",
    options: [
      {
    ...
    feedbackModel: {
      decisionSignal:
        "The decision for each observation depends only on prior membership.",
      mentalModelCorrection:
        "Do not collect counts or positions when the output rule uses only first-seen versus already-seen state.",
      mistakeTypes: ["overmodeled_deduplication_state"],
      nextAction:
        "Store no more information than the emission decision requires.",
      result: "diagnostic",
    },
  },
  {
...
Which state is sufficient for the seen values?`,
    ...
    feedbackModel: {
      decisionSignal:
        "The output is boolean, and the scan only asks whether a complement exists in the processed prefix.",
      mentalModelCorrection:
        "The collection should match the information consumed by the algorithm, not information a different output contract might require.",
      mistakeTypes: ["map_selected_without_index_or_count_need"],
      nextAction:
        "Remove associated values that are never read after insertion.",
      result: "diagnostic",
    },
  },
  {
    ...
    type: "single_choice",
    prompt:
      "A function must return the exact number of occurrences of every distinct value. Which representation is required?",
    options: [
      {
    ...
    feedbackModel: {
      decisionSignal:
        "The returned result contains a separate numeric answer for every key.",
      mentalModelCorrection:
        "Knowing which keys exist does not reveal how often each key occurs.",
      mistakeTypes: ["set_state_disconnected_from_frequency_output"],
      nextAction:
        "Derive the stored value type directly from the value required in the output.",
      result: "diagnostic",
    },
  },
  {
...
Which state selection is correct?`,
    ...
    feedbackModel: {
      decisionSignal:
        "The second contract needs associated provenance information for each key.",
      mentalModelCorrection:
        "The same keys may require different representations when the downstream output changes.",
      mistakeTypes: ["state_choice_not_updated_for_output_contract"],
      nextAction:
        "Ask what information must be recovered after membership has been established.",
      result: "diagnostic",
    },
  },
  {
...
Which representation choice is best justified?`,
    ...
    feedbackModel: {
      decisionSignal:
        "The search condition is similar, but the required result contains different information.",
      mentalModelCorrection:
        "Representation choice follows the strongest query or output the state must support, not the broad problem label.",
      mistakeTypes: [
        "representation_selected_independently_of_output_contract",
      ],
      nextAction:
        'For each key, complete the sentence: "Later I must retrieve..."',
      result: "diagnostic",
    },
  },
];

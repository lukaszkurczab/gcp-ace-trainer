export const recognizeHashStateSignalQuestions = [
  {
    ...
    prompt:
      'While scanning an unsorted array, the algorithm must repeatedly answer: "Has this value appeared earlier?" Which strategy signal is strongest?',
    options: [
      {
        id: "seen_set",
    ...
    feedbackModel: {
      decisionSignal:
        "The query is repeated and is addressed by the current value as a key.",
      mentalModelCorrection:
        "Keyed state is justified when future decisions repeatedly ask for information associated with a particular value.",
      mistakeTypes: ["membership_signal_not_recognized"],
      nextAction:
        'Complete the question the algorithm asks in the form: "What state is stored for this key?"',
      result: "diagnostic",
    },
  },
  {
    ...
    type: "strategy_choice",
    prompt:
      "A function must emit only the first occurrence of each value while preserving encounter order. Which state best supports the decision for each observation?",
    options: [
      {
    ...
    feedbackModel: {
      decisionSignal:
        "Each observation needs a membership answer for its own value.",
      mentalModelCorrection:
        "Stable deduplication requires per-value seen state; one global scalar cannot distinguish which values were emitted.",
      mistakeTypes: ["deduplication_keyed_state_missed"],
      nextAction:
        "Identify the exact value whose prior presence determines whether the current item is retained.",
      result: "diagnostic",
    },
  },
  {
    ...
    type: "single_choice",
    prompt:
      "A stream of updates repeatedly refers to users by userId and must retrieve the current state for that specific user. Which representation is the natural starting point?",
    options: [
      {
        id: "map_by_user_id",
    ...
    feedbackModel: {
      decisionSignal:
        "The algorithm repeatedly retrieves and modifies state addressed by a stable identity.",
      mentalModelCorrection:
        "When updates name a key directly, keyed state avoids treating unrelated entities as one aggregate.",
      mistakeTypes: ["repeated_identity_lookup_signal_missed"],
      nextAction:
        "Identify the field used to address the state that must be retrieved repeatedly.",
      result: "diagnostic",
    },
  },
  {
    ...
    type: "strategy_choice",
    prompt:
      'While scanning values, a pair condition can be rewritten as: "For the current value, has the required partner value appeared earlier?" Which strategy signal is present?',
    options: [
      {
    ...
    feedbackModel: {
      decisionSignal:
        "The current item determines one specific partner key that can be queried directly.",
      mentalModelCorrection:
        "Complement lookup is keyed candidate retrieval, not a property of the complete interval between two positions.",
      mistakeTypes: ["complement_lookup_signal_not_recognized"],
      nextAction:
        "Rewrite the relationship to isolate the prior value required by the current observation.",
      result: "diagnostic",
    },
  },
  {
    ...
    type: "strategy_choice",
    prompt:
      "Items must be divided into groups so that all items with the same category identifier appear in the same bucket. Which strategy fits?",
    options: [
      {
    ...
    feedbackModel: {
      decisionSignal:
        "Every item belongs to a bucket addressed by an equivalence key.",
      mentalModelCorrection:
        "Grouping requires keyed association between each class identifier and the state accumulated for that class.",
      mistakeTypes: ["grouping_hash_signal_missed"],
      nextAction:
        "Identify the value that names an equivalence class and the information accumulated under it.",
      result: "diagnostic",
    },
  },
  {
    ...
    prompt:
      "One collection contains allowed codes. A second, much longer collection must be filtered by repeatedly asking whether each code is allowed. Which high-level design is appropriate?",
    options: [
      {
    ...
    feedbackModel: {
      decisionSignal:
        "Many observations query membership against the same reusable reference collection.",
      mentalModelCorrection:
        "Repeated lookup by value is the hash-state signal; the two collections do not form one contiguous candidate range.",
      mistakeTypes: ["cross_collection_membership_signal_missed"],
      nextAction:
        "Precompute the state that answers the repeated per-item query.",
      result: "diagnostic",
    },
  },
  {
    ...
    type: "strategy_choice",
    prompt:
      "A traversal must avoid processing the same object instance twice. Different objects with identical properties must still be treated as separate instances. Which state matches the contract in JavaScript?",
    options: [
      {
    ...
    feedbackModel: {
      decisionSignal:
        "Membership is defined by JavaScript object identity rather than structural equality.",
      mentalModelCorrection:
        "Set can directly track object references when the contract concerns whether the exact same instance was seen.",
      mistakeTypes: ["identity_membership_signal_missed"],
      nextAction:
        "Determine whether equality means the same reference or merely equal contents.",
      result: "diagnostic",
    },
  },
  {
    ...
    type: "mistake_review",
    prompt:
      "A function only needs the total sum of all numeric values. A candidate proposes storing every value in a Map before calculating the sum. What is the best review?",
    options: [
      {
    ...
    feedbackModel: {
      decisionSignal:
        "The required state is one aggregate over all observations, not information retrieved by individual key.",
      mentalModelCorrection:
        "Hash state is unnecessary when one scalar can fully summarize the processed input for the required result.",
      mistakeTypes: ["hash_state_used_for_scalar_problem"],
      nextAction:
        "Ask whether future decisions retrieve state for a specific key or only update one global aggregate.",
      result: "diagnostic",
    },
  },
  {
    ...
    type: "solution_comparison",
    prompt:
      "An algorithm tracks which lowercase English letters have appeared. The domain is permanently limited to the 26 letters a–z. Which representation is a reasonable alternative to hash state?",
    options: [
      {
    ...
    feedbackModel: {
      decisionSignal:
        "Keys come from a small, fixed, directly indexable domain.",
      mentalModelCorrection:
        "Keyed lookup does not always require a hash table. Bounded domains can often use direct indexed storage.",
      mistakeTypes: ["hash_table_assumed_required_for_all_membership"],
      nextAction:
        "Check whether every possible key can be mapped to a compact fixed index.",
      result: "diagnostic",
    },
  },
  {
    ...
    type: "solution_comparison",
    prompt:
      "An array may be reordered, and the only required result is whether any duplicate value exists. Which statement is correct?",
    options: [
      {
    ...
    feedbackModel: {
      decisionSignal:
        "Reordering is legal, and sorting can place equal values next to one another.",
      mentalModelCorrection:
        "Recognizing a hash-state signal does not mean hashing is the only legal solution.",
      mistakeTypes: ["hashing_treated_as_only_duplicate_strategy"],
      nextAction:
        "Check whether input reordering is permitted and whether ordering exposes the needed relationship.",
      result: "diagnostic",
    },
  },
  {
    ...
    type: "strategy_choice",
    prompt:
      "A task must emit every pair of records satisfying an arbitrary comparison function. No single lookup key can be derived from one record to identify all matching partners. Which observation is most precise?",
    options: [
      {
    ...
    feedbackModel: {
      decisionSignal:
        "The predicate does not expose a stable key that maps one observation to its possible matches.",
      mentalModelCorrection:
        "Hash state helps only when the required relationship can be queried meaningfully by key.",
      mistakeTypes: ["hash_state_forced_without_lookup_key"],
      nextAction:
        "Explain what exact key would retrieve all and only the relevant prior candidates.",
      result: "diagnostic",
    },
  },
  {
    ...
    type: "strategy_choice",
    prompt:
      "A stream contains transactions for many accounts. The algorithm must maintain a separate running total for each accountId. Which signal is present?",
    options: [
      {
    ...
    feedbackModel: {
      decisionSignal:
        "The same operation must maintain an independent aggregate for every identity.",
      mentalModelCorrection:
        "A global accumulator loses which portion of the total belongs to each key.",
      mistakeTypes: ["per_key_aggregation_signal_missed"],
      nextAction:
        "Identify whether the aggregate is global or separately owned by each key.",
      result: "diagnostic",
    },
  },
  {
    ...
    type: "mistake_review",
    prompt:
      "A task requires values to be returned in ascending order. A candidate chooses a Map solely because it offers keyed lookup. What is missing from the reasoning?",
    options: [
      {
    ...
    feedbackModel: {
      decisionSignal:
        "The stated requirement concerns result order rather than lookup by identity.",
      mentalModelCorrection:
        "Hash state and sorting solve different needs and may need to be combined or compared separately.",
      mistakeTypes: ["hash_lookup_confused_with_ordering_strategy"],
      nextAction:
        "Separate the state-access requirement from the final output-order requirement.",
      result: "diagnostic",
    },
  },
  {
...
Which replacement explanation would properly justify keyed hash state?`,
    ...
    feedbackModel: {
      decisionSignal:
        "The representation should be derived from the operations the algorithm must support.",
      mentalModelCorrection:
        "Hash state is justified by repeated key-addressed queries, not by a generic preference for a data structure.",
      mistakeTypes: ["hash_state_justified_only_by_vague_speed_claim"],
      nextAction:
        "Name the exact lookup question and the key used to answer it.",
      result: "diagnostic",
    },
  },
];

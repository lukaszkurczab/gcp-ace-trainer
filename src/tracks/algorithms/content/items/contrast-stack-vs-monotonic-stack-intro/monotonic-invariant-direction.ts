export const monotonicInvariantDirectionQuestions = [
  {
    ...
    secondarySkillAtomIds: ["bottom_to_top_order", "explicit_stack_invariant"],
    type: "single_choice",
    prompt:
      "A stack is described as monotonically increasing from bottom to top. Which sequence of stored values satisfies that invariant?",
    options: [
      {
        id: "one_three_seven",
    ...
    correctOptionId: "one_three_seven",
    feedbackModel: {
      decisionSignal:
        "The direction is defined explicitly from the bottom entry toward the top entry.",
      mentalModelCorrection:
        "An increasing stack requires each higher entry to satisfy the declared increasing relation with the entry below it.",
      mistakeTypes: ["monotonic_direction_mismatch"],
      nextAction:
        "Always state both the comparison relation and whether it is read bottom-to-top or top-to-bottom.",
      result: "diagnostic",
    },
  },
  {
    ...
    secondarySkillAtomIds: ["stack_invariant", "input_order_reasoning"],
    type: "single_choice",
    prompt:
      "The input is [5, 1, 4, 2]. A developer says a monotonic stack cannot be used because the input itself is not monotonic. What is wrong with that reasoning?",
    options: [
      {
        id: "stack_not_input",
    ...
    correctOptionId: "stack_not_input",
    feedbackModel: {
      decisionSignal:
        "The algorithm selectively removes candidates so that the retained unresolved entries satisfy an order.",
      mentalModelCorrection:
        "Monotonic stack describes the state of the auxiliary stack, not a required property of the input sequence.",
      mistakeTypes: ["input_stack_invariant_confusion"],
      nextAction:
        "Describe which stored candidates remain after each input value is processed.",
      result: "diagnostic",
    },
  },
  {
    ...
    type: "single_choice",
    prompt:
      "A stack must remain strictly increasing from bottom to top. Its values are [2, 5, 8], and the current value is 4. What should happen before 4 is pushed?",
    options: [
      {
        id: "pop_eight_five",
    ...
    correctOptionId: "pop_eight_five",
    feedbackModel: {
      decisionSignal:
        "Every value above the insertion point must satisfy the strict increasing relation after the push.",
      mentalModelCorrection:
        "Checking or removing only the first conflicting top is insufficient when the next exposed entry also violates the invariant.",
      mistakeTypes: ["incomplete_invariant_restoration"],
      nextAction:
        "Continue popping while the exposed top conflicts with the value being inserted.",
      result: "diagnostic",
    },
  },
  {
    ...
    type: "single_choice",
    prompt:
      "A stack must remain strictly decreasing from bottom to top. Its values are [9, 6, 3], and the current value is 7. Which entries must be removed before pushing 7?",
    options: [
      {
        id: "pop_three_six",
    ...
    correctOptionId: "pop_three_six",
    feedbackModel: {
      decisionSignal:
        "In a decreasing bottom-to-top stack, every lower entry must be greater than the entry above it.",
      mentalModelCorrection:
        "After removing 3, the exposed 6 still conflicts with pushing 7 above it. Restoration must continue until the full order is valid.",
      mistakeTypes: ["reversed_monotonic_inequality"],
      nextAction:
        "Simulate the stack after the proposed push, not only the first comparison.",
      result: "diagnostic",
    },
  },
  {
    ...
    correctOptionId: "current_resolves_smaller",
    feedbackModel: {
      decisionSignal:
  ...
  },
  {
    ...
    feedbackModel: {
      decisionSignal:
        "The retained candidate offers no future answer that the newer candidate cannot provide at least as well.",
      mentalModelCorrection:
        "Monotonic stacks remove candidates whose continued presence cannot improve any future result under the contract.",
      mistakeTypes: ["dominated_candidate_retained"],
      nextAction:
        "Ask whether the older entry can still win any future comparison or positional tie.",
      result: "diagnostic",
    },
  },
  {
    ...
    secondarySkillAtomIds: ["invariant_precision", "stack_order_orientation"],
    type: "solution_comparison",
    prompt:
      "A code review says only, 'This uses an increasing stack.' Why is that description incomplete?",
    options: [
      {
    ...
    correctOptionId: "direction_and_strictness_missing",
    feedbackModel: {
      decisionSignal:
        "The label increasing is ambiguous without orientation and comparison strictness.",
      mentalModelCorrection:
        "A usable invariant must make it possible to determine whether a concrete stack state is valid.",
      mistakeTypes: ["ambiguous_invariant_description"],
  ...
  },
  {
    ...
    secondarySkillAtomIds: ["while_pop_restoration", "full_stack_invariant"],
    type: "single_choice",
    prompt:
      "A solution uses an if statement to pop at most one conflicting stack top before pushing the current value. When is this insufficient?",
    options: [
      {
    ...
    correctOptionId: "multiple_conflicts",
    feedbackModel: {
      decisionSignal:
        "Removing one entry may expose another entry that violates the same required relation.",
      mentalModelCorrection:
        "Invariant restoration is condition-based, not limited to one mutation per input item.",
      mistakeTypes: ["incomplete_invariant_restoration"],
      nextAction:
        "Use repeated checking until the exposed top is compatible or the stack is empty.",
      result: "diagnostic",
    },
  },
  {
    ...
    secondarySkillAtomIds: ["increasing_stack_trace", "candidate_removal"],
    type: "single_choice",
    prompt:
      "A strictly increasing bottom-to-top stack contains [1, 4, 6, 9]. The current value is 5. What stack remains after restoring the invariant and pushing 5?",
    options: [
      {
        id: "one_four_five",
        text: "[1, 4, 5]",
    ...
    correctOptionId: "one_four_five",
    feedbackModel: {
      decisionSignal:
        "Values 9 and 6 conflict with placing 5 above them, while 4 remains compatible.",
      mentalModelCorrection:
        "Pop exactly the suffix that violates the relation. Entries below the first compatible top remain useful.",
      mistakeTypes: ["monotonic_trace_error"],
  ...
  },
  {
    ...
    type: "single_choice",
    prompt:
      "A stack must be strictly increasing from bottom to top. Which generic condition should remove the current top before pushing currentValue?",
    options: [
      {
    ...
    correctOptionId: "top_greater_or_equal_current",
    feedbackModel: {
      decisionSignal:
        "After the push, the previous top must be strictly smaller than the new top.",
      mentalModelCorrection:
        "Any top that is greater than or equal to the incoming value would violate strict increasing order.",
      mistakeTypes: ["pop_condition_mismatch"],
      nextAction:
        "Derive the pop condition by negating the relation that must hold after insertion.",
      result: "diagnostic",
    },
  },
  {
    ...
    type: "single_choice",
    prompt:
      "A stack must be strictly decreasing from bottom to top. Which condition restores the invariant before pushing currentValue?",
    options: [
      {
    ...
    correctOptionId: "top_less_or_equal_current",
    feedbackModel: {
      decisionSignal:
        "The previous top must remain strictly greater than the newly pushed value.",
      mentalModelCorrection:
        "A top smaller than or equal to currentValue cannot remain directly below it in a strictly decreasing stack.",
      mistakeTypes: ["reversed_monotonic_inequality"],
      nextAction:
        "Write the required post-push relation, then remove every top that violates it.",
      result: "diagnostic",
    },
  },
  {
    ...
    type: "solution_comparison",
    prompt:
      "Which description best captures what entries remaining in an introductory monotonic stack should represent?",
    options: [
      {
    ...
    correctOptionId: "useful_unresolved_candidates",
    feedbackModel: {
      decisionSignal:
        "Stack membership has semantic meaning beyond satisfying a numeric order.",
      mentalModelCorrection:
        "The monotonic invariant compresses the processed prefix into candidates that may still matter.",
      mistakeTypes: ["stack_state_semantics_mismatch"],
      nextAction:
        "Define both the ordering invariant and the unresolved-candidate meaning of each stored entry.",
      result: "diagnostic",
    },
  },
  {
    ...
    type: "solution_comparison",
    prompt:
      "Which explanation gives the strongest definition of a monotonic-stack invariant?",
    options: [
      {
    ...
    correctOptionId: "complete_invariant",
    feedbackModel: {
      decisionSignal:
        "A complete invariant defines order, orientation, strictness, entry meaning, and restoration behavior.",
      mentalModelCorrection:
        "A label such as increasing stack is not sufficient to prove concrete state transitions or candidate elimination.",
      mistakeTypes: ["weak_invariant_justification"],
      nextAction:
        "State the exact order and explain why every retained and removed candidate is treated correctly.",
      result: "diagnostic",
    },
  },
];

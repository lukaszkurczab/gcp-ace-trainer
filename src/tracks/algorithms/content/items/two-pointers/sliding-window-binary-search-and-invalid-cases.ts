...
B. Compare two current positions and prove that one endpoint cannot belong to any remaining answer.`,
    ...
    feedbackModel: {
      decisionSignal:
        "Binary search discards a half through a midpoint predicate; two pointers advance candidate positions monotonically.",
      mentalModelCorrection:
        "A shrinking interval is not enough to identify binary search or two pointers.",
      mistakeTypes: ["binary_search_and_two_pointer_rules_conflated"],
      nextAction:
        "Identify whether progress comes from midpoint halving or coordinated cursor movement.",
      result: "diagnostic",
    },
  },
...
What is the central flaw?`,
    ...
    feedbackModel: {
      decisionSignal:
        "The endpoint values do not bound the values between them in an unsorted input.",
      mentalModelCorrection:
        "Opposite-end sum movement is valid because of sorted monotonicity, not because endpoints are convenient.",
      mistakeTypes: ["opposite_ends_used_on_unsorted_target_sum"],
      nextAction:
        "State the input property that proves the moved endpoint cannot be part of a valid remaining pair.",
      result: "diagnostic",
    },
  },
  {
    ...
    type: "single_choice",
    prompt:
      "What must be true before a two-pointer algorithm may permanently move past a candidate position?",
    options: [
      {
    ...
    feedbackModel: {
      decisionSignal:
        "Pointer movement removes candidates from future consideration.",
      mentalModelCorrection:
        "A failed current comparison is insufficient unless it rules out every remaining relation involving the moved position.",
      mistakeTypes: ["pointer_move_lacks_safe_elimination_proof"],
      nextAction:
        "Describe the complete set of candidates eliminated by the move.",
      result: "diagnostic",
    },
  },
  {
    ...
    prompt:
      "A task tests pairs with an arbitrary function `compatible(a, b)`. The values are not ordered by any property related to that function, and one failed pair reveals nothing about neighboring candidates. What is the best assessment?",
    options: [
      {
    ...
    feedbackModel: {
      decisionSignal:
        "The relation supplies no monotonic consequence from a comparison.",
      mentalModelCorrection:
        "Two pointers cannot safely compress arbitrary pair enumeration without a structural elimination rule.",
      mistakeTypes: ["two_pointers_forced_for_nonmonotonic_relation"],
      nextAction:
        "Explain what a failed comparison proves about adjacent or remaining candidates.",
      result: "diagnostic",
    },
  },
...
What is the best correction?`,
    ...
    feedbackModel: {
      decisionSignal:
        "The interval may represent surviving candidates rather than one evaluated range.",
      mentalModelCorrection:
        "Pattern classification follows the invariant maintained between the boundaries.",
      mistakeTypes: ["candidate_interval_called_window_by_shape"],
      nextAction:
        "State whether the interior is active data or merely unresolved search space.",
      result: "diagnostic",
    },
  },
  {
...
What additional justification is required?`,
    ...
    feedbackModel: {
      decisionSignal:
        "Sortedness helps only when the decision predicate supports directional half elimination.",
      mentalModelCorrection:
        "Binary search is justified by a legal discard rule, not by sorted data alone.",
      mistakeTypes: ["binary_search_selected_from_sortedness_only"],
      nextAction:
        "State what midpoint answer implies about every candidate in one half.",
      result: "diagnostic",
    },
  },
...
Why is that generally unsafe?`,
    ...
    feedbackModel: {
      decisionSignal:
        "A failed relation between two values does not usually invalidate each value independently.",
      mentalModelCorrection:
        "Discard only the endpoint covered by the monotonic elimination argument.",
      mistakeTypes: ["both_endpoints_discarded_without_proof"],
      nextAction:
        "For each moved pointer, separately prove that no remaining answer can use its current position.",
      result: "diagnostic",
    },
  },
...
Which classification is precise?`,
    ...
    feedbackModel: {
      decisionSignal:
        "Interior values are unresolved candidates rather than members of one evaluated range.",
      mentalModelCorrection:
        "Classify the semantic candidate, not the visual presence of two boundaries.",
      mistakeTypes: ["endpoint_pair_code_called_sliding_window"],
      nextAction:
        "List exactly which elements contribute to the current validity test.",
      result: "diagnostic",
    },
  },
  {
...
Which classification is precise?`,
    ...
    feedbackModel: {
      decisionSignal:
        "The maintained state represents every element in one active contiguous interval.",
      mentalModelCorrection:
        "Sliding windows are a specialized coordinated-pointer pattern with range-wide state.",
      mistakeTypes: ["active_range_state_not_recognized_as_window"],
      nextAction:
        "Determine whether adding or removing a boundary element changes aggregate state for the candidate.",
      result: "diagnostic",
    },
  },
  {
...
What is missing?`,
    ...
    feedbackModel: {
      decisionSignal:
        "The unresolved search space is a set of index pairs rather than one ordered index interval.",
      mentalModelCorrection:
        "Binary search needs a monotonic predicate over one chosen search dimension.",
      mistakeTypes: ["pair_space_treated_as_single_binary_search_interval"],
      nextAction:
        "Identify the exact variable being binary-searched and what each predicate result eliminates.",
      result: "diagnostic",
    },
  },
  {
    ...
    type: "strategy_choice",
    prompt:
      "An unsorted stream must be processed in one pass without reordering. For each current value, the algorithm must determine whether its exact complement appeared earlier. Which strategy fits?",
    options: [
      {
        id: "hash_lookup",
    ...
    feedbackModel: {
      decisionSignal:
        "The needed partner is addressed by an exact derived key in prior state.",
      mentalModelCorrection:
        "When sorting is unavailable and prior membership is the required operation, hash lookup is the natural model.",
      mistakeTypes: ["streaming_complement_forced_into_pointer_search"],
      nextAction:
        "Express the required prior observation as a lookup key.",
      result: "diagnostic",
    },
  },
  {
...
What does this reveal?`,
    ...
    feedbackModel: {
      decisionSignal:
        "Both endpoint movements can reduce the difference, but neither endpoint has been independently disproved.",
      mentalModelCorrection:
        "Exact sorted-difference search is commonly modeled with two forward pointers because that representation supplies directional movement rules.",
      mistakeTypes: ["opposite_end_difference_search_lacks_safe_move"],
      nextAction:
        "Choose pointer roles from the monotonic behavior of the expression, not from the presence of sorted data alone.",
      result: "diagnostic",
    },
  },
];

export const pointerRolesBoundariesAndTerminationQuestions = [
  {
    ...
    type: "single_choice",
    prompt:
      "What is the strongest way to explain the role of a pointer variable in an algorithm?",
    options: [
      {
    ...
    feedbackModel: {
      decisionSignal:
        "A pointer is meaningful through the invariant and region associated with it.",
      mentalModelCorrection:
        "Names and movement mechanics do not replace a semantic explanation of the pointer's responsibility.",
      mistakeTypes: ["pointer_role_described_only_syntactically"],
...
What does boundary represent?`,
    ...
    feedbackModel: {
      ...
      mentalModelCorrection:
        "An exclusive boundary is not the last valid element index; it points one position beyond the region.",
      mistakeTypes: ["exclusive_boundary_treated_as_inclusive"],
      nextAction:
        "Translate [start, end) into included indexes before using either boundary.",
      result: "diagnostic",
    },
  },
  {
    ...
    type: "single_choice",
    prompt:
      "In a left-to-right scan, read identifies the next item that has not yet been classified. Which region description is consistent with that role?",
    options: [
      {
    ...
    feedbackModel: {
      decisionSignal:
        "The pointer marks the frontier between completed observations and remaining input.",
      mentalModelCorrection:
        "A read pointer usually advances input progress; it does not automatically describe output progress.",
      mistakeTypes: ["read_pointer_region_misidentified"],
      nextAction:
        "State whether the item at read has already been processed before deciding when to increment it.",
      result: "diagnostic",
    },
  },
  {
    ...
    type: "single_choice",
    prompt:
      "A pointer named write identifies the next position where an output item would be placed. What does [0, write) represent?",
    options: [
      {
    ...
    feedbackModel: {
      decisionSignal:
        "write advances only after one output position has been completed.",
      mentalModelCorrection:
        "The next-write position is an exclusive boundary for finalized output.",
      mistakeTypes: ["write_pointer_prefix_misinterpreted"],
      nextAction:
        "Relate write to produced output count rather than total processed input.",
      result: "diagnostic",
    },
  },
  {
    ...
    type: "single_choice",
    prompt:
      "Two pointers are named slow and fast. What must be known before their algorithmic roles can be classified?",
    options: [
      {
    ...
    feedbackModel: {
      decisionSignal:
        "The same names can describe different traversal invariants in different algorithms.",
      mentalModelCorrection:
        "Pointer semantics come from advancement rules and represented regions, not conventional variable names.",
      mistakeTypes: ["fast_slow_names_assumed_to_define_roles"],
...
What is the main review concern?`,
    ...
    feedbackModel: {
      decisionSignal:
        "The old and new meanings imply different valid ranges, updates, and termination conditions.",
      mentalModelCorrection:
        "A pointer should retain one stable invariant within a phase; role transitions must be explicit and re-established.",
      mistakeTypes: ["pointer_meaning_changes_mid_algorithm"],
      nextAction:
        "Split the logic into named phases or use separate variables with separately stated invariants.",
      result: "diagnostic",
    },
  },
  {
...
Which loop condition safely processes every valid index exactly once?`,
    ...
    feedbackModel: {
      ...
      mentalModelCorrection:
        "An exclusive upper bound permits the last valid item and stops before index length.",
      mistakeTypes: ["forward_scan_bound_off_by_one"],
      nextAction:
        "Write the exact valid index interval before choosing the loop comparison.",
      result: "diagnostic",
    },
  },
  {
...
What happens on the final iteration?`,
    ...
    feedbackModel: {
      decisionSignal:
        "Array length is an exclusive boundary, not a valid element index.",
      mentalModelCorrection:
        "A pointer may legally equal an exclusive boundary only when the code does not index through it.",
      mistakeTypes: ["exclusive_end_indexed_as_element"],
      nextAction:
        "Guard every element access with a condition proving the pointer is inside the valid index range.",
      result: "diagnostic",
    },
  },
  {
    ...
    feedbackModel: {
      decisionSignal:
        "Different contracts treat a one-position remainder differently.",
      mentalModelCorrection:
        "Termination follows from the meaning of the remaining region, not a universal pointer convention.",
      mistakeTypes: ["meeting_and_crossing_rules_generalized_without_contract"],
...
What is the principal bug?`,
    ...
    feedbackModel: {
      decisionSignal:
        "The condition excludes the final valid index even though it remains unprocessed.",
      mentalModelCorrection:
        "A termination condition must become false only after every required candidate has been handled.",
      mistakeTypes: ["pointer_scan_terminates_before_last_item"],
      nextAction:
        "Test the loop condition at the first, last, and one-past-last positions.",
      result: "diagnostic",
    },
  },
  {
...
Why may the outer condition be insufficient for safety?`,
    ...
    feedbackModel: {
      decisionSignal:
        "Repeated advancement can invalidate the bounds that were true at entry to the outer iteration.",
      mentalModelCorrection:
        "Every loop that moves a pointer must independently preserve the conditions required for its next array access.",
      mistakeTypes: ["inner_pointer_loop_uses_stale_bounds"],
      nextAction:
        "Include the relevant pointer relation in the inner loop condition.",
      result: "diagnostic",
    },
  },
  {
...
Which length formulas are correct?`,
    ...
    feedbackModel: {
      decisionSignal:
        "The inclusive convention contains both endpoints, while the half-open convention excludes right.",
      mentalModelCorrection:
        "Length and termination formulas must follow one consistent boundary convention.",
      mistakeTypes: ["inclusive_exclusive_boundary_conventions_mixed"],
      nextAction:
        "Write the first few concrete indexes represented by each interval.",
      result: "diagnostic",
    },
  },
  {
...
What is wrong?`,
    ...
    feedbackModel: {
      decisionSignal:
        "A half-open interval contains exactly the indexes starting at left and ending before right.",
      mentalModelCorrection:
        "Mixing inclusive formulas with exclusive semantics creates off-by-one output and termination errors.",
      mistakeTypes: ["half_open_region_uses_inclusive_length"],
      nextAction:
        "Derive formulas from the explicitly declared interval notation.",
      result: "diagnostic",
    },
  },
  {
    ...
    prompt:
      "Which explanation most completely justifies pointer correctness and termination?",
    options: [
      {
    ...
    feedbackModel: {
      decisionSignal:
        "Correctness requires semantic preservation, bounded access, measurable progress, and an output-aligned stopping condition.",
      mentalModelCorrection:
        "Pointer mechanics are safe only when roles, regions, updates, and termination form one coherent invariant.",
      mistakeTypes: ["pointer_correctness_argument_incomplete"],
      nextAction:
        "For each pointer, state its role, legal range, movement event, preserved region, and terminal state.",
      result: "diagnostic",
    },
  },
];

export const recognizeTwoPointerSignalQuestions = [
  {
    ...
    prompt:
      "A sorted array must be checked for two distinct values whose sum equals a target. Which signal most strongly supports a two-pointer strategy?",
    options: [
      {
    ...
    feedbackModel: {
      decisionSignal:
        "The candidate is a relationship between two selected sorted values, and order provides a safe directional elimination rule.",
      mentalModelCorrection:
        "Two pointers are justified by coordinated candidate elimination, not merely by the presence of a sorted array.",
      mistakeTypes: ["sorted_pair_signal_not_recognized"],
      nextAction:
        "Explain how each comparison proves that one endpoint cannot belong to any remaining solution.",
      result: "diagnostic",
    },
  },
  {
    ...
    type: "strategy_choice",
    prompt:
      "A string must be checked by comparing characters in mirrored positions from the outside toward the center. Which strategy fits?",
    options: [
      {
    ...
    feedbackModel: {
      ...
      mentalModelCorrection:
        "Symmetric comparison uses coordinated endpoints because correctness depends on positional correspondence, not on a range aggregate.",
      mistakeTypes: ["symmetric_comparison_signal_missed"],
      nextAction:
        "Identify the unique counterpart associated with each current position.",
      result: "diagnostic",
    },
  },
  {
    ...
    type: "strategy_choice",
    prompt:
      "An array must be filtered in place so retained values occupy a prefix while preserving encounter order. Which signal supports coordinated pointers?",
    options: [
      {
    ...
    feedbackModel: {
      decisionSignal:
        "Input progress and output progress are separate responsibilities.",
      mentalModelCorrection:
        "Two pointers may coordinate source and destination positions even when no pair of values is compared.",
      mistakeTypes: ["read_write_signal_not_recognized"],
      nextAction:
        "Ask whether one cursor consumes input while another grows a logical output region.",
      result: "diagnostic",
    },
  },
  {
    ...
    type: "strategy_choice",
    prompt:
      "Two individually sorted sequences must be merged into one sorted result. Which representation is the natural starting point?",
    options: [
      {
    ...
    feedbackModel: {
      decisionSignal:
        "Each sorted suffix exposes its smallest remaining candidate at its pointer.",
      mentalModelCorrection:
        "Coordinated pointers are appropriate when progress must be synchronized across multiple ordered sources.",
      mistakeTypes: ["sorted_merge_signal_not_recognized"],
  ...
  },
  {
    ...
    type: "strategy_choice",
    prompt:
      "An in-place pass must place values satisfying a predicate before a returned boundary. Which two-pointer signal is present?",
    options: [
      {
    ...
    feedbackModel: {
      decisionSignal:
        "The algorithm maintains semantic regions whose boundary changes as observations are classified.",
      mentalModelCorrection:
        "Partition pointers are justified by region ownership and placement responsibility.",
      mistakeTypes: ["partition_pointer_signal_missed"],
  ...
  },
  {
    ...
    type: "single_choice",
    prompt:
      "Which property most strongly justifies moving coordinated pointers without revisiting discarded positions?",
    options: [
      {
    ...
    feedbackModel: {
      decisionSignal:
        "A pointer movement permanently removes candidates while preserving every possible solution.",
      mentalModelCorrection:
        "Two-pointer correctness comes from a safe elimination invariant, not from syntax or performance hopes.",
      mistakeTypes: ["two_pointer_signal_based_on_variable_count"],
      nextAction:
        "State which candidates become impossible after each movement.",
      result: "diagnostic",
    },
  },
  {
...
Which mapping is correct?`,
    ...
    feedbackModel: {
      decisionSignal:
        "The first objective depends on selected positions; the second depends on the complete contiguous interior.",
      mentalModelCorrection:
        "Classify the semantic candidate before classifying the pointer pattern.",
      mistakeTypes: ["two_pointers_and_window_conflated"],
      nextAction:
        "List which input elements contribute to evaluating one current candidate.",
      result: "diagnostic",
    },
  },
  {
...
Which mapping is correct?`,
    ...
    feedbackModel: {
      decisionSignal:
        "Binary search discards a fraction through a midpoint predicate; two pointers coordinate moving candidate positions.",
      mentalModelCorrection:
        "Sorted input alone does not determine the strategy. The legal discard rule does.",
      mistakeTypes: ["binary_search_and_two_pointer_signals_conflated"],
      nextAction:
        "Ask whether progress comes from midpoint halving or endpoint/cursor advancement.",
      result: "diagnostic",
    },
  },
  {
    ...
    type: "strategy_choice",
    prompt:
      "A task must examine every pair because its arbitrary pair predicate provides no ordering or monotonic rule for discarding candidates. What is the best assessment?",
    options: [
      {
    ...
    feedbackModel: {
      decisionSignal:
        "No comparison safely eliminates a structured set of untested pairs.",
      mentalModelCorrection:
        "Two pointers do not replace enumeration unless the problem supplies a monotonic progression or region invariant.",
      mistakeTypes: ["two_pointers_forced_without_elimination_rule"],
      nextAction:
        "Explain why a moved pointer position can never be needed again.",
      result: "diagnostic",
    },
  },
  {
...
Which mapping is correct?`,
    ...
    feedbackModel: {
      decisionSignal:
        "One strategy retrieves a partner from stored keyed state; the other eliminates candidates through sorted positional movement.",
      mentalModelCorrection:
        "Similar output goals may support different state and movement models.",
      mistakeTypes: ["hash_and_two_pointer_signals_conflated"],
      nextAction:
        "Ask whether the partner is retrieved by key or reached through ordered pointer movement.",
      result: "diagnostic",
    },
  },
  {
...
What is the best correction?`,
    ...
    feedbackModel: {
      decisionSignal:
        "Pattern identity depends on semantics and coordination rather than variable count.",
      mentalModelCorrection:
        "Two-pointer techniques include several movement shapes, but each needs stable pointer responsibilities.",
      mistakeTypes: ["arbitrary_two_indexes_called_two_pointers"],
      nextAction:
        "Describe how the movement of one pointer changes what the other pointer may still need to consider.",
      result: "diagnostic",
    },
  },
  {
    ...
    type: "single_choice",
    prompt:
      "Which task can legitimately use two pointers that both move left to right?",
    options: [
      {
    ...
    feedbackModel: {
      decisionSignal:
        "Pointers may represent separate monotonic progress frontiers rather than opposite endpoints.",
      mentalModelCorrection:
        "Two-pointer is a broader coordination pattern, not a synonym for inward movement.",
      mistakeTypes: ["two_pointers_defined_as_opposite_direction_only"],
      nextAction:
        "Classify each pointer by responsibility rather than direction alone.",
      result: "diagnostic",
    },
  },
  {
    ...
    type: "mistake_review",
    prompt:
      "A candidate proposes endpoint movement because it worked on several examples, but cannot identify any sorted, symmetric, or region-based invariant. What is the main concern?",
    options: [
      {
    ...
    feedbackModel: {
      decisionSignal:
        "Pointer movement permanently changes the candidate space.",
      mentalModelCorrection:
        "Empirical success does not replace a structural argument for safe elimination or region preservation.",
      mistakeTypes: ["pointer_strategy_selected_by_examples_only"],
      nextAction:
        "Identify the input property that makes every movement irreversible and safe.",
      result: "diagnostic",
    },
  },
  {
...
Which mapping is correct?`,
    ...
    feedbackModel: {
      decisionSignal:
        "The tasks share coordinated pointer movement but assign different semantic roles to those pointers.",
      mentalModelCorrection:
        "Recognize the broad pattern first, then classify the specific roles and invariant.",
      mistakeTypes: ["two_pointer_role_families_conflated"],
      nextAction:
        "For each task, name what each pointer identifies and why it moves.",
      result: "diagnostic",
    },
  },
];

export const mergeAndIntersectionOfSortedSequencesQuestions = [
  {
    ...
    type: "single_choice",
    prompt:
      "In a two-input merge or intersection scan, what should pointers i and j represent?",
    options: [
      {
    ...
    feedbackModel: {
      decisionSignal:
        "All positions before each pointer have already been consumed according to the output contract.",
      mentalModelCorrection:
        "The pointers divide each source into a consumed prefix and an unconsumed suffix.",
      mistakeTypes: ["sorted_source_pointer_semantics_mismatch"],
      nextAction:
        "State what is known about every element before and at each pointer.",
      result: "diagnostic",
    },
  },
  {
    ...
    secondarySkillAtomIds: ["merge_sorted_sequences", "preserve_sorted_output"],
    type: "single_choice",
...
What should happen next?`,
    ...
    feedbackModel: {
      decisionSignal:
        "Sorted suffixes guarantee that no later item in either source can precede the smaller current candidate.",
      mentalModelCorrection:
        "A merge consumes the smaller current item, not both compared items.",
      mistakeTypes: ["merge_advances_both_on_inequality"],
      nextAction:
        "Emit only the source item proven to be next in sorted order.",
      result: "diagnostic",
    },
  },
  {
...
If left[i].key === right[j].key, what should the algorithm do?`,
    ...
    feedbackModel: {
      ...
      mentalModelCorrection:
        "Equal sort keys do not imply duplicate elimination; both source items may still belong in the merged output.",
      mistakeTypes: ["stable_merge_tie_policy_ignored"],
      nextAction:
        "Define which source wins ties and still preserve the other item for a later step.",
      result: "diagnostic",
    },
  },
  {
...
The function then returns immediately. What can be lost?`,
    ...
    feedbackModel: {
      decisionSignal:
        "The loop condition guarantees only that at least one input ended.",
      mentalModelCorrection:
        "A standard merge must append the remaining sorted suffix from the non-exhausted source.",
      mistakeTypes: ["merge_suffix_dropped"],
      nextAction:
        "After the joint loop, consume each source's remaining suffix.",
      result: "diagnostic",
    },
  },
  {
...
What is the result?`,
    ...
    feedbackModel: {
      decisionSignal:
        "At each step, the smaller first unconsumed item is emitted.",
      mentalModelCorrection:
        "The final result includes every item from both inputs, including the final suffix.",
      mistakeTypes: ["sorted_merge_trace_mismatch"],
...
What should happen?`,
    ...
    feedbackModel: {
      decisionSignal:
        "Every later value in b is at least 5, so the current 2 can never find a match.",
      mentalModelCorrection:
        "On inequality, sorted intersection advances only the pointer at the smaller value.",
      mistakeTypes: ["intersection_advances_both_on_inequality"],
      nextAction:
        "Discard only the current value proven unable to match any remaining value.",
      result: "diagnostic",
    },
  },
  {
...
What should happen after emitting one 4?`,
    ...
    feedbackModel: {
      decisionSignal:
        "A multiset match pairs one available occurrence from each input.",
      mentalModelCorrection:
        "Both matched source occurrences must be consumed exactly once.",
      mistakeTypes: ["matched_occurrence_reused"],
      nextAction:
        "Treat a successful equality as consumption from both sources.",
      result: "diagnostic",
    },
  },
  {
...
Which outputs correspond to the two contracts?`,
    ...
    feedbackModel: {
      decisionSignal:
        "Set semantics emit a shared value once; multiset semantics emit it according to shared occurrence availability.",
      mentalModelCorrection:
        "Duplicate-skipping behavior must follow the requested output contract.",
      mistakeTypes: ["set_and_multiset_intersection_conflated"],
      nextAction:
        "Decide whether the result represents shared values or shared occurrences.",
      result: "diagnostic",
    },
  },
  {
...
Why can this be wrong?`,
    ...
    feedbackModel: {
      decisionSignal:
        "Each equal pair of available occurrences may contribute another output item.",
      mentalModelCorrection:
        "Skipping an entire duplicate run implements distinct set semantics unless multiplicity has already been counted separately.",
      mistakeTypes: ["multiset_intersection_accidentally_deduplicated"],
      nextAction:
        "Advance once per matched occurrence under multiset semantics.",
      result: "diagnostic",
    },
  },
  {
...
What can happen?`,
    ...
    feedbackModel: {
      decisionSignal:
        "Equal runs may contain several matched occurrence pairs for one distinct value.",
      mentalModelCorrection:
        "Set intersection needs an explicit policy that emits a shared key once and then moves beyond its duplicate run.",
      mistakeTypes: ["set_intersection_duplicate_output"],
      nextAction:
        "After emitting a distinct match, skip all remaining equal occurrences in both inputs.",
      result: "diagnostic",
    },
  },
  {
    ...
    type: "single_choice",
    prompt:
      "Why may a sorted intersection algorithm stop when either input is exhausted?",
    options: [
      {
    ...
    feedbackModel: {
      decisionSignal:
        "Intersection requires one occurrence or member from each source.",
      mentalModelCorrection:
        "Unlike merge, intersection does not append an unmatched suffix.",
      mistakeTypes: ["merge_suffix_rule_applied_to_intersection"],
      nextAction:
        "Ask whether unmatched source items belong to the requested output relation.",
      result: "diagnostic",
    },
  },
  {
...
Which statement is correct?`,
    ...
    feedbackModel: {
      decisionSignal:
        "Merge preserves all source items; intersection preserves only shared items.",
      mentalModelCorrection:
        "The two operations have different suffix and empty-input contracts.",
      mistakeTypes: ["merge_and_intersection_empty_case_conflated"],
      nextAction:
        "Apply the output relation directly when one source contributes no items.",
      result: "diagnostic",
    },
  },
  {
    ...
    feedbackModel: {
      decisionSignal:
        "The left run represents records that appeared earlier in the original sequence.",
      mentalModelCorrection:
        "Stable merge preserves the original relative order of equal-key records; for consecutive runs, the left run must win cross-run ties.",
      mistakeTypes: ["stable_merge_original_order_not_preserved"],
      nextAction:
        "Tie the equality branch to the original ordering relationship between the two runs.",
      result: "diagnostic",
    },
  },
  {
...
What is the bug?`,
    ...
    feedbackModel: {
      decisionSignal:
        "Only one source item is consumed and emitted per iteration.",
      mentalModelCorrection:
        "A pointer advances exactly when its current source item has been consumed.",
      mistakeTypes: ["merge_unconsumed_item_skipped"],
...
What is the result?`,
    ...
    feedbackModel: {
      decisionSignal:
        "The shared count of 2 is min(2, 3), and 4 appears once in both inputs.",
      mentalModelCorrection:
        "Multiset intersection consumes matched copies from both sorted sources.",
      mistakeTypes: ["multiset_sorted_intersection_trace_mismatch"],
  ...
  },
  {
    ...
    prompt:
      "Which invariant best supports correctness for a merge or intersection scan over two sorted inputs?",
    options: [
      {
    ...
    feedbackModel: {
      decisionSignal:
        "Sorted suffixes make the two current items the only candidates that can be next consumed.",
      mentalModelCorrection:
        "The invariant connects consumed prefixes, unconsumed suffixes, and the exact output relation being implemented.",
      mistakeTypes: ["sorted_two_input_invariant_incomplete"],
      nextAction:
        "State what has been consumed and why the current pointer values are the next relevant candidates.",
      result: "diagnostic",
    },
  },
];

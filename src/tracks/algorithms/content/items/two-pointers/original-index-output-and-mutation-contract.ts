export const originalIndexOutputAndMutationContractQuestions = [
  {
    ...
    type: "single_choice",
    prompt:
      "A pair-search algorithm sorts the input before scanning, but the result must contain original indexes. Which representation preserves the required identity?",
    options: [
      {
    ...
    feedbackModel: {
      decisionSignal:
        "Sorting changes positions, so original identity must travel with each value.",
      mentalModelCorrection:
        "A sorted index describes the transformed representation, not the element's original source position.",
      mistakeTypes: ["original_index_metadata_lost_during_sort"],
      nextAction:
        "Attach every output-relevant identity field before applying a reordering transformation.",
      result: "diagnostic",
    },
  },
  {
...
Why can that result violate the contract?`,
    ...
    feedbackModel: {
      decisionSignal:
        "The pointer positions are coordinates in the sorted array.",
      mentalModelCorrection:
        "Returning pointer positions is correct only when the output contract asks for positions in the transformed representation.",
      mistakeTypes: ["sorted_positions_returned_as_original_indexes"],
      nextAction:
        "Translate each selected sorted record back through its stored original index.",
      result: "diagnostic",
    },
  },
  {
...
Which design satisfies the contract?`,
    ...
    feedbackModel: {
      decisionSignal:
        "JavaScript Array.prototype.sort mutates the array on which it is called.",
      mentalModelCorrection:
        "A correct pointer strategy can still violate the function contract through unintended preprocessing mutation.",
      mistakeTypes: ["non_mutating_contract_violated_by_sort"],
      nextAction:
        "Copy before sorting whenever caller-visible input order must be preserved.",
      result: "diagnostic",
    },
  },
  {
    ...
    prompt:
      "A sorted pair-search function must return only the two matching values, not their original indexes. Which statement is correct?",
    options: [
      {
    ...
    feedbackModel: {
      decisionSignal:
        "The required result contains values rather than source identities.",
      mentalModelCorrection:
        "Preserve only the metadata required by the actual output and tie-breaking contracts.",
      mistakeTypes: ["state_overmodeled_for_value_output"],
      nextAction:
        "List the exact fields that must appear in the returned result.",
      result: "diagnostic",
    },
  },
  {
...
What is the problem?`,
    ...
    feedbackModel: {
      decisionSignal:
        "Sorted encounter order and original-index lexicographic order are different orderings.",
      mentalModelCorrection:
        "Preserving original indexes is necessary but not sufficient; the search must also consider every candidate that could win the specified tie-break.",
      mistakeTypes: ["arbitrary_pair_returned_under_earliest_contract"],
      nextAction:
        "Use a strategy proven to inspect all tie-relevant candidates, then compare them by the required original-index tuple.",
      result: "diagnostic",
    },
  },
  {
...
Which design supports the contract?`,
    ...
    feedbackModel: {
      decisionSignal:
        "Equal comparison keys do not erase record identity or the required relative ordering.",
      mentalModelCorrection:
        "Tie behavior is part of deterministic output correctness, not an implementation detail.",
      mistakeTypes: ["stable_tie_contract_ignored"],
      nextAction:
        "Define how equal-key records are ordered before relying on sorted traversal.",
      result: "diagnostic",
    },
  },
  {
...
What part of the array is the logical output?`,
    ...
    feedbackModel: {
      decisionSignal:
        "write represents the number of produced output items and the next free output position.",
      mentalModelCorrection:
        "In-place compaction often returns a logical length rather than clearing or resizing the physical suffix.",
      mistakeTypes: ["compacted_suffix_treated_as_output"],
      nextAction:
        "Read only the returned logical prefix unless the contract explicitly requires physical resizing.",
      result: "diagnostic",
    },
  },
  {
...
A caller treats the final two values as additional retained output. What is wrong?`,
    ...
    feedbackModel: {
      decisionSignal:
        "The algorithm guarantees only the produced prefix and its length.",
      mentalModelCorrection:
        "Memory beyond the logical output boundary is not automatically a second meaningful region.",
      mistakeTypes: ["stale_suffix_given_output_semantics"],
...
Which return value is correct?`,
    ...
    feedbackModel: {
      decisionSignal:
        "write counts how many output elements have been produced.",
      mentalModelCorrection:
        "Count, last index, logical length, and returned contents are different output contracts.",
      mistakeTypes: ["compaction_return_contract_mismatch"],
      nextAction:
        "Translate the requested output noun directly into the maintained pointer state.",
      result: "diagnostic",
    },
  },
  {
...
Which in-place strategy matches the contract?`,
    ...
    feedbackModel: {
      decisionSignal:
        "The output contract includes stable relative order among retained observations.",
      mentalModelCorrection:
        "An in-place transformation may be correct as a membership filter yet still violate an ordering requirement.",
      mistakeTypes: ["compaction_reorders_retained_values"],
      nextAction:
        "Choose write order from the required output order, not merely from available storage.",
      result: "diagnostic",
    },
  },
  {
...
Which statement is correct?`,
    ...
    feedbackModel: {
      decisionSignal:
        "One output describes identity by position, while the other requires the actual sequence values.",
      mentalModelCorrection:
        "Finding boundaries and materializing contents are separate responsibilities with different output behavior.",
      mistakeTypes: ["range_boundaries_returned_instead_of_copy"],
      nextAction:
        "Check whether the caller needs coordinates, a view, or an independent copied result.",
      result: "diagnostic",
    },
  },
  {
    ...
    prompt:
      "Which invariant most completely supports a pointer transformation that preserves original identity, mutates a compacted prefix, and returns a logical length?",
    options: [
      {
    ...
    feedbackModel: {
      decisionSignal:
        "Correctness combines output contents, ordering, identity metadata, mutation boundaries, and suffix semantics.",
      mentalModelCorrection:
        "Pointer mechanics are correct only when the maintained regions and returned metadata satisfy the complete observable contract.",
      mistakeTypes: ["pointer_output_mutation_invariant_incomplete"],
      nextAction:
        "State what the produced prefix guarantees, what identity fields survive, and what the suffix does not guarantee.",
      result: "diagnostic",
    },
  },
];

...
export function selectAlgorithmSessionPlan(input: SelectAlgorithmSessionItemsInput): AlgorithmSessionSelection {
  ...
  const capacity = catalog.resolveSessionCapacity(input.mode, input.sessionLength, scoped.length);
  if (capacity.kind === "shortfall") throw new Error(`Algorithms blueprint cannot prepare minimum actual length for ${input.mode}; ${capacity.missingItemCount} eligible items are missing.`);
  const actualLength = capacity.kind === "shortened" ? capacity.actualLength : input.sessionLength;
  const items = mode.profile.shortening === "prohibited" ? Object.freeze([...scoped]) : takeDeclaredItems(scoped, actualLength);
...
function selectWeakReview(input: SelectAlgorithmSessionItemsInput, catalog: AlgorithmRuntimeCatalog): AlgorithmSessionSelection { if (!input.reviewSource) throw new Error("Algorithms Weak Area Review requires due_queue or session_misses source."); if (![10, 20].includes(input.sessionLength)) throw new Error("Algorithms Weak Area Review requested length is invalid."); const selection = selectAlgorithmReviewItems({ catalog, reviewedItemRefs: [...(input.attempts ?? []).map((attempt) => attempt.item), ...(input.reviewQueueItems ?? []).map((entry) => entry.sourceItem)], requestedLength: input.sessionLength, source: input.reviewSource === "session_misses" ? { kind: "session_misses", itemRefs: input.reviewItemRefs ?? [] } : { kind: "due_queue", now: input.now ?? "", reviewQueueItems: (input.reviewQueueItems ?? []).filter((entry) => entry.sourceItem.trackId === "coding-interview-dsa-problem-solving") } }); const capacity = catalog.resolveSessionCapacity(input.mode, input.sessionLength, selection.actualLength); if (capacity.kind === "shortfall") throw new Error(`Algorithms Weak Area Review is unavailable: ${capacity.missingItemCount} eligible items are missing.`); const items = selection.items.slice(0, capacity.kind === "shortened" ? capacity.actualLength : input.sessionLength); if (!items.length) throw new Error("Algorithms Weak Area Review is unavailable: no eligible items."); return Object.freeze({ actualLength: items.length, items: Object.freeze(items), requestedLength: input.sessionLength }); }

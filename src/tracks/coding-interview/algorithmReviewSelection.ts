...
export type AlgorithmReviewSelectionSource = Readonly<{ kind: "due_queue"; now: string; reviewQueueItems: readonly ReviewQueueEntry[] }> | Readonly<{ itemRefs: readonly ResolvedContentRef[]; kind: "session_misses" }>;
...
export function selectAlgorithmReviewItems(input: Readonly<{ catalog: AlgorithmRuntimeCatalog; reviewedItemRefs: readonly ResolvedContentRef[]; requestedLength: number; source: AlgorithmReviewSelectionSource }>): AlgorithmReviewSelection {
  ...
  for (const ref of refs) { if (ref.trackId === "coding-interview-dsa-problem-solving" && ref.contentVersion === input.catalog.getContentVersion() && ref.artifactSha256 === input.catalog.getArtifactSha256()) add(input.catalog.getItemById(ref.questionId)); }
  const reviewed = new Set(input.reviewedItemRefs.filter((ref) => ref.trackId === "coding-interview-dsa-problem-solving" && ref.contentVersion === input.catalog.getContentVersion() && ref.artifactSha256 === input.catalog.getArtifactSha256()).map((ref) => ref.questionId));

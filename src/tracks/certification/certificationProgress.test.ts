...
test("Certification progress derives practice, exam, outcome, taxonomy, review, and repeated-mistake signals", () => {
  const progress = buildCloudCertificationProgressViewModel({ attempts: [attempt("correct", "one"), attempt("incorrect", "three", "certification-exam-simulation")], artifactSha256, reviewQueueItems: [review()], now: "2026-01-02T00:00:00.000Z" });
...
test("Certification progress has explicit empty state and no synthetic learning metrics", () => {
  const progress = buildCloudCertificationProgressViewModel({ attempts: [], artifactSha256 });
...
test("Certification progress ignores same-version evidence from another exact content package", () => {
  const foreignArtifactSha256 = "e".repeat(64);
  const foreignAttempt = {
    ...attempt("incorrect", "foreign"),
    item: { ...ref, artifactSha256: foreignArtifactSha256 },
    reviewEvidence: { sourceItem: { ...ref, artifactSha256: foreignArtifactSha256 }, taxonomyOrSkillRefs: [{ axisId: "cloud-domain", nodeId: question.domain }] },
  };
  const foreignReview = { ...review(), sourceItem: { ...ref, artifactSha256: foreignArtifactSha256 } };
  const progress = buildCloudCertificationProgressViewModel({ attempts: [foreignAttempt], artifactSha256, reviewQueueItems: [foreignReview] });
  assert.equal(progress.totalAttempts, 0);

...
export type ClientAvailability = "unconfigured" | "provider_not_composed";
...
export function createApprovedClientRegistry(environment: PublicEnvironment): ApprovedClientRegistry {
  const availability: ClientAvailability = environment.kind === "configured" ? "provider_not_composed" : "unconfigured";
  return Object.freeze(Object.fromEntries(APPROVED_CLIENT_IDS.map((id) => [id, Object.freeze({ availability, id })])) as ApprovedClientRegistry);
}

...
export class MutationCommitFailure extends Error {
  constructor(readonly phase: MutationCommitPhase, readonly durableState: MutationDurableState, readonly cause?: unknown) { super(`Canonical mutation failed during ${phase}${cause instanceof Error ? `: ${cause.message}` : "."}`); this.name = "MutationCommitFailure"; }
}

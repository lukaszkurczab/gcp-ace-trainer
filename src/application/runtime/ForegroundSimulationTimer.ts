...
export type ForegroundSimulationTimerPort = Readonly<{
...
/**
 * A single authoritative timer for a simulation runtime.  It never derives
 ...
 */
export class ForegroundSimulationTimer {
  private segmentStartedAt: number | null = null;

  constructor(
    private state: ForegroundTimerState,
    private readonly port: ForegroundSimulationTimerPort,
  ...
  static restore(input: Readonly<{
    ...
    port: ForegroundSimulationTimerPort;
    monotonicClock: MonotonicClock;
    wallClock: WallClock;
  }>): ForegroundSimulationTimer {
    ...
    return new ForegroundSimulationTimer(input.state, input.port, input.monotonicClock, input.wallClock);
  }

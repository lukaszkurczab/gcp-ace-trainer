...
import { createTrainingAttempt, createTrainingSession, createTrainingSessionResult, completeTrainingSession, createFamilyEnvelope, type ReviewMutationCommand, type ReviewQueueEntry, type TrainingAttempt, type TrainingSession, type TrainingSessionDraft, type TrackFamilyId } from "../../domain";
...
function eligibleEvidence(catalog: CanonicalTrackRuntime, mode: ProductModeConfig, reviews: readonly ReviewQueueEntry[], attempts: readonly TrainingAttempt<unknown>[], now: string): readonly Question[] { const selection = mode.selection; if (selection.kind !== "evidence_conditioned") return catalog.getPool(mode.modeId); const ids = new Set<string>(); if (selection.evidenceSources.includes("due_queue")) reviews.filter((r) => r.dueAt <= now).forEach((r) => ids.add(r.sourceItem.questionId)); if (selection.evidenceSources.includes("committed_session_misses")) attempts.filter((a) => a.result.kind !== "correct").forEach((a) => ids.add(a.item.questionId)); return catalog.getPool(mode.modeId).filter((q) => ids.has(q.questionId)); }

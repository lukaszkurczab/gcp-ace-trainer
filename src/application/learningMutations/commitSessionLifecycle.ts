...
export async function commitSessionAbandonment(session: TrainingSession, createdAt: string, clearExam = false): Promise<void> {
  ...
  await commitMutation(await buildMutationJournal({
    ...
    writes: [
      ...
      ...(clearExam ? [{ kind: "clear_active_exam", sessionId: session.id } as const] : []),
    ],
    createdAt,
  }));
}

...
import { JournalVerificationError, StorageReadError } from "../../storage/errors";
...
import { addReviewQueueItems, getReviewQueueItems, getTrainingAttempts, getTrainingSessions } from "../../storage/repositories";
import { materializeMutation } from "./mutationMaterializer";
import { UnsupportedStoredRecordError } from "../../storage/errors";
import { installMemoryStorage, attempt, journal, review, session } from "../../testing/journalTestSupport";
...
test("rejects conflicting immutable attempt", async () => { installMemoryStorage(); await materializeMutation(journal([{ kind: "put_attempt", record: { ...attempt(), response: { choice: "b" } } }])); await persistMutationJournal(outcome()); await assert.rejects(() => recoverPendingMutation(), (error: unknown) => error instanceof MutationCommitFailure && error.phase === "materialization" && error.durableState === "journal_durable"); });
test("rejects conflicting active journal", async () => { installMemoryStorage(); await persistMutationJournal(outcome()); const commandFingerprint = "1".repeat(64); const conflicting = journal([{ kind: "put_attempt", record: attempt("other") }, { kind: "put_session", record: session("completed") }]); const plan = { operation: conflicting.operation, status: conflicting.status, createdAt: conflicting.createdAt, sessionId: conflicting.sessionId, trackId: conflicting.trackId, artifactSha256: conflicting.artifactSha256, commandIdentity: { version: 1 as const, fingerprint: commandFingerprint }, expectedRevisions: conflicting.expectedRevisions, writes: conflicting.writes } as const; const { createMutationPlanFingerprint } = await import("../../storage/repositories/mutationJournalRepository"); await assert.rejects(() => persistMutationJournal({ journalId: `journal:${commandFingerprint}`, ...plan, planFingerprint: createMutationPlanFingerprint(plan) })) });
...
test("preserves journal after materialization failure", async () => { const storage = installMemoryStorage(); const record = outcome(); await persistMutationJournal(record); storage.setFailurePlan({ kind: "fail_on_key_write", key: STORAGE_KEYS.trainingAttempt("attempt-1") }); await assert.rejects(() => recoverPendingMutation(), (error: unknown) => error instanceof MutationCommitFailure && error.phase === "materialization" && error.durableState === "journal_durable"); assert.notEqual(await getActiveMutationJournal(), null); });
test("preserves journal after verification failure", async () => { const storage = installMemoryStorage(); const record = outcome(); await persistMutationJournal(record); await materializeMutation(record); storage.resetCounters(); storage.setFailurePlan({ kind: "fail_on_read_number", readNumber: 1 }); await assert.rejects(() => recoverPendingMutation(), StorageReadError); storage.setFailurePlan(null); assert.notEqual(await getActiveMutationJournal(), null); });
...
test("rejects an old persisted journal without an exact artifact SHA-256", async () => { installMemoryStorage(); const { artifactSha256: _artifactSha256, ...unpinned } = outcome(); writeCanonicalJson(STORAGE_KEYS.ACTIVE_JOURNAL, unpinned); await assert.rejects(() => recoverPendingMutation(), UnsupportedStoredRecordError); });
...
test("rejects destructive writes that do not belong to the operation", async () => { installMemoryStorage(); await assert.rejects(() => persistMutationJournal(journal([{ kind: "put_attempt", record: attempt() }, { kind: "put_session", record: session() }, { kind: "clear_active_session", sessionId: "session-1" }]))); });
test("rejects attempts and reviews that conflict with the durable session plan", async () => {
  installMemoryStorage();
  const outsideAttempt = { ...attempt(), item: { ...attempt().item, questionId: "outside" }, reviewEvidence: { ...attempt().reviewEvidence, sourceItem: { ...attempt().item, questionId: "outside" } } };
...
test("rejects an immediate submit for a planned but non-current occurrence", async () => {
  ...
  const secondItem = { ...baseSession.itemOrder[0]!.item, questionId: "item-2" };
...
test("rejects contradictory review actions and duplicate terminal item outcomes", async () => {
  installMemoryStorage();
...
test("does not require remote content", async () => { installMemoryStorage(); await persistMutationJournal(outcome()); await recoverPendingMutation(); assert.equal((await getTrainingAttempts()).value[0]?.item.questionId, "item-1"); });

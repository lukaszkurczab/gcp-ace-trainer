...
test("rejects an old persisted journal without an exact package pin", async () => { installMemoryStorage(); const { packagePin: _pin, ...unpinned } = outcome(); writeCanonicalJson(STORAGE_KEYS.ACTIVE_JOURNAL, unpinned); await assert.rejects(() => recoverPendingMutation(), UnsupportedStoredRecordError); });

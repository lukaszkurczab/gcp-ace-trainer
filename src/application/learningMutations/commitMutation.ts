...
export async function commitMutation(record: MutationJournalRecord): Promise<void> { const prepared = await persistMutationJournal(record); await materializeMutation(prepared); await verifyMutation(prepared); await clearMutationJournal(); }

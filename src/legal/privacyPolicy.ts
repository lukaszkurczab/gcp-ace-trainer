...
export function renderPrivacyPolicy(vars: LegalVariables) {
  return Object.freeze({
    en: String.raw`Patternly Privacy Policy

Effective ${vars.privacy.effectiveDate.en}. Version ${vars.documentVersion.en}. This Policy explains how ${vars.privacy.controllerLegalName.en}, ${vars.privacy.controllerBusinessForm.en}, at ${vars.privacy.controllerAddress.en} (the “Controller”) processes personal data in Patternly.

1. Controller and contact

Controller: ${vars.privacy.controllerLegalName.en}, ${vars.privacy.controllerBusinessForm.en}, ${vars.privacy.controllerAddress.en}. Privacy contact: ${vars.privacy.privacyEmail.en}; phone: ${vars.privacy.controllerPhone.en}. Data protection officer or designated privacy contact: ${vars.privacy.dpoContact.en}.
...
Learning progress is retained while the account exists and is removed through the account-deletion process, subject to limited deletion evidence described below. Completed synchronization-operation metadata is retained for ${vars.privacy.completedSyncOperationRetentionDays.en} days after completion.
...
Reports are processed to correct content, respond where contact was requested, protect the service and establish claims. The legal basis for these purposes is the Controller’s legitimate interest under Article 6(1)(f) GDPR in maintaining accurate content, answering a requested contact, protecting the reporting channel and defending legal claims; you may object under Article 21 GDPR. Reports not linked to an account or contact are retained for ${vars.privacy.anonymousReportRetentionDays.en} days; account-linked or contact reports for ${vars.privacy.linkedReportRetentionDays.en} days. The one-way IP rate-limit identifier is kept only for the active limit window of ${vars.privacy.reportRateLimitIdentifierRetentionSeconds.en} seconds. A locally accepted report is removed after the server confirms receipt. An unconfirmed report in the local outbox is automatically removed after ${vars.privacy.localReportOutboxRetentionDays.en} days.
...
Operational logs are retained for ${vars.privacy.operationalLogRetentionDays.en} days and security logs for ${vars.privacy.securityLogRetentionDays.en} days. Patternly redacts credentials and payloads from production logs. Recovery codes copied at your request may remain in the system clipboard; Patternly attempts to clear them after ${vars.privacy.clipboardRecoveryCodeRetentionMinutes.en} minutes where the operating system permits and does not remove content you copied later.

8. Processors, recipients and transfers

Verified active cloud processors and their purposes: ${vars.privacy.activeCloudProcessors.en}. Active independent controllers and recipients: ${vars.privacy.activeIndependentRecipients.en}. Email delivery provider: ${vars.privacy.emailDeliveryProvider.en}. Provider regions: ${vars.privacy.hostingRegions.en}. Safeguards for transfers outside the EEA: ${vars.privacy.internationalTransferSafeguards.en}.

RevenueCat status: ${vars.privacy.revenueCatStatus.en}. Apple may independently retain App Store transaction and subscription records. Deleting a Patternly account does not cancel an App Store subscription or remove records controlled independently by Apple. Patternly reviews its processors, recipients and transfer arrangements whenever providers or their processing locations change.
...
After deletion, Patternly retains a keyed HMAC pseudonym solely to prevent accidental account resurrection for ${vars.privacy.deletionTombstoneRetentionDays.en} days. It remains personal data; it is not anonymous, but no raw provider UID remains. Its legal basis is the Controller’s legitimate interest under Article 6(1)(f) GDPR; you may object to this processing under Article 21 GDPR by contacting ${vars.privacy.privacyEmail.en}. A minimal, unlinkable proof that deletion completed may be retained for up to ${vars.privacy.deletionEvidenceRetentionYears.en} years under Article 6(1)(c) GDPR together with the accountability and rights-obligation framework in Articles 5(2), 12(2), 17 and 24 GDPR. Patternly does not keep a blanket anti-fraud record after deletion: any event-specific anti-fraud retention needs its own documented threat model, necessity assessment and legitimate-interest assessment. Backup and point-in-time-recovery deletion: ${vars.privacy.backupPurgeDisclosure.en}.

10. Your rights

Subject to applicable law, you may request access and the information required by Article 15 GDPR, rectification, erasure, restriction, portability, or object to processing based on legitimate interests. You may withdraw consent without affecting earlier lawful processing. Contact ${vars.privacy.privacyEmail.en}. Requests are normally free of charge. Patternly may verify identity and must respond within one month; where legally permitted, it may extend by up to two further months after notifying you within the first month and explaining why. Any refusal must be reasoned and explain complaint and court options.

Patternly handles privacy requests through intake, identity verification where necessary, assessment, fulfilment or a reasoned refusal, and closure. This also applies where Patternly cannot identify a guest without additional information. You may complain to ${vars.privacy.supervisoryAuthority.en} or another competent authority, and may seek a judicial remedy. Patternly does not make solely automated decisions that produce legal or similarly significant effects.
...
Registration number: ${vars.privacy.registrationNumber.en}. Tax identifier: ${vars.privacy.taxIdentifier.en}. Territories: ${vars.privacy.distributionTerritories.en}.


13. Processing purposes and legal bases

${vars.privacy.processingRegisterDisclosure.en}

14. Detailed retention periods

${vars.privacy.retentionRegisterDisclosure.en}`,
    pl: String.raw`Polityka prywatności Patternly

Obowiązuje od ${vars.privacy.effectiveDate.pl}. Wersja ${vars.documentVersion.pl}. Niniejsza Polityka wyjaśnia, jak ${vars.privacy.controllerLegalName.pl}, ${vars.privacy.controllerBusinessForm.pl}, pod adresem ${vars.privacy.controllerAddress.pl} („Administrator”) przetwarza dane osobowe w Patternly.

1. Administrator i kontakt

Administrator: ${vars.privacy.controllerLegalName.pl}, ${vars.privacy.controllerBusinessForm.pl}, ${vars.privacy.controllerAddress.pl}. Kontakt w sprawach prywatności: ${vars.privacy.privacyEmail.pl}; telefon: ${vars.privacy.controllerPhone.pl}. Inspektor ochrony danych lub wyznaczony kontakt: ${vars.privacy.dpoContact.pl}.
...
Postęp jest przechowywany do usunięcia konta. Metadane zakończonych operacji synchronizacji są przechowywane przez ${vars.privacy.completedSyncOperationRetentionDays.pl} dni po ukończeniu.
...
Raporty służą poprawie treści, odpowiedzi na kontakt, ochronie usługi i ustalaniu roszczeń. Podstawą tych celów jest prawnie uzasadniony interes Administratora z art. 6 ust. 1 lit. f RODO w utrzymaniu poprawności treści, odpowiedzi na żądany kontakt, ochronie kanału zgłoszeń i obronie roszczeń; możesz wnieść sprzeciw na podstawie art. 21 RODO. Raport niepowiązany z kontem ani kontaktem jest przechowywany ${vars.privacy.anonymousReportRetentionDays.pl} dni, a powiązany z kontem lub kontaktem — ${vars.privacy.linkedReportRetentionDays.pl} dni. Jednokierunkowy identyfikator limitu IP jest przechowywany tylko przez aktywne okno limitu wynoszące ${vars.privacy.reportRateLimitIdentifierRetentionSeconds.pl} sekund. Lokalnie zaakceptowany raport jest usuwany po potwierdzeniu odbioru przez serwer. Niepotwierdzony raport w lokalnej kolejce jest automatycznie usuwany po ${vars.privacy.localReportOutboxRetentionDays.pl} dniach.
...
Logi operacyjne są przechowywane ${vars.privacy.operationalLogRetentionDays.pl} dni, a bezpieczeństwa ${vars.privacy.securityLogRetentionDays.pl} dni. Patternly usuwa dane dostępowe i payloady z logów produkcyjnych. Kod odzyskiwania skopiowany na Twoje żądanie może pozostać w schowku systemowym; Patternly próbuje usunąć go po ${vars.privacy.clipboardRecoveryCodeRetentionMinutes.pl} minutach, jeśli system na to pozwala, i nie usuwa treści skopiowanej później przez użytkownika.

8. Podmioty przetwarzające, odbiorcy i transfery

Zweryfikowani aktywni dostawcy chmurowi i cele: ${vars.privacy.activeCloudProcessors.pl}. Aktywni niezależni administratorzy i odbiorcy: ${vars.privacy.activeIndependentRecipients.pl}. Dostawca email: ${vars.privacy.emailDeliveryProvider.pl}. Regiony dostawców: ${vars.privacy.hostingRegions.pl}. Zabezpieczenia transferów poza EOG: ${vars.privacy.internationalTransferSafeguards.pl}.

Status RevenueCat: ${vars.privacy.revenueCatStatus.pl}. Apple może niezależnie zachować historię transakcji i subskrypcji. Usunięcie konta Patternly nie anuluje subskrypcji App Store ani danych kontrolowanych niezależnie przez Apple. Patternly przegląda procesorów, odbiorców i transfery po każdej zmianie dostawcy lub miejsca przetwarzania.
...
Po usunięciu Patternly przechowuje pseudonim w postaci kluczowanego HMAC wyłącznie w celu zapobieżenia przypadkowemu odtworzeniu konta przez ${vars.privacy.deletionTombstoneRetentionDays.pl} dni. Nadal jest to dana osobowa, a nie anonimowa, lecz surowy UID dostawcy nie pozostaje. Podstawą jest prawnie uzasadniony interes Administratora z art. 6 ust. 1 lit. f RODO; wobec tego przetwarzania możesz wnieść sprzeciw na podstawie art. 21 RODO, kontaktując się z ${vars.privacy.privacyEmail.pl}. Minimalny, niepowiązywalny dowód ukończenia usunięcia może być przechowywany maksymalnie przez ${vars.privacy.deletionEvidenceRetentionYears.pl} lata na podstawie art. 6 ust. 1 lit. c RODO wraz z obowiązkami rozliczalności i realizacji praw z art. 5 ust. 2, art. 12 ust. 2, art. 17 i art. 24 RODO. Patternly nie przechowuje po usunięciu ogólnego zapisu antyfraudowego: każde zdarzeniowe przetwarzanie antyfraudowe wymaga własnego modelu zagrożeń, testu konieczności i oceny prawnie uzasadnionego interesu. Usuwanie kopii zapasowych i point-in-time recovery: ${vars.privacy.backupPurgeDisclosure.pl}.

10. Twoje prawa

W zakresie właściwego prawa możesz żądać dostępu wraz z informacjami z art. 15 RODO, sprostowania, usunięcia, ograniczenia, przenoszenia danych lub sprzeciwić się przetwarzaniu opartemu na prawnie uzasadnionym interesie. Możesz wycofać zgodę bez wpływu na wcześniejsze zgodne z prawem przetwarzanie. Napisz na ${vars.privacy.privacyEmail.pl}. Żądania są co do zasady bezpłatne. Patternly może zweryfikować tożsamość i musi odpowiedzieć w ciągu miesiąca; gdy prawo pozwala, może przedłużyć termin maksymalnie o dwa miesiące, informując o tym i powodach w pierwszym miesiącu. Odmowa musi mieć uzasadnienie i wskazywać środki skargi oraz ochrony sądowej.

Patternly obsługuje żądania dotyczące prywatności przez przyjęcie, weryfikację tożsamości, gdy jest potrzebna, ocenę, realizację lub uzasadnioną odmowę oraz zamknięcie sprawy. Dotyczy to również sytuacji, gdy Patternly nie może zidentyfikować gościa bez dodatkowych informacji. Możesz złożyć skargę do ${vars.privacy.supervisoryAuthority.pl} lub innego właściwego organu oraz skorzystać z drogi sądowej. Patternly nie podejmuje wyłącznie zautomatyzowanych decyzji wywołujących skutki prawne lub podobnie istotne.
...
Numer rejestracyjny: ${vars.privacy.registrationNumber.pl}. Identyfikator podatkowy: ${vars.privacy.taxIdentifier.pl}. Terytoria: ${vars.privacy.distributionTerritories.pl}.


13. Rejestr celów i podstaw

${vars.privacy.processingRegisterDisclosure.pl}

14. Szczegółowe okresy retencji

${vars.privacy.retentionRegisterDisclosure.pl}`,
  });
}

export const privacyPolicy = renderPrivacyPolicy(legalVariables);

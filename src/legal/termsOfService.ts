import { legalVariables, type LegalVariables } from "./legalVariables";

export function renderTermsOfService(vars: LegalVariables) {
  return Object.freeze({
    en: String.raw`Patternly Terms of Service

Effective ${vars.terms.effectiveDate.en}. Version ${vars.documentVersion.en}. These Terms form an agreement between you and ${vars.terms.operatorLegalName.en}, ${vars.terms.operatorBusinessForm.en}, at ${vars.terms.operatorRegisteredAddress.en} (the “Operator”) for your use of Patternly.

1. Accepting these Terms

By creating an account or otherwise using account-based Patternly services, you agree to these Terms and acknowledge the separate Privacy Policy. If you do not agree, do not create an account. Guest use that does not require acceptance remains subject to applicable law and the rules needed to operate the app safely.

2. Eligibility

Patternly requires you to be at least ${vars.terms.minimumUserAge.en} to create an account on your own. This is a product rule, not a statement that every person below that age is legally barred from learning. Account-free guest learning may remain available to younger users. Patternly does not provide a parent-or-guardian consent flow, so users below that age cannot create an account. Policy scope: ${vars.terms.minimumUserAgeScope.en}. You must provide accurate account information and keep access credentials secure.

3. The service

Patternly is an independent learning tool for practice in technical interviews and certification-related topics. It is not an official certification provider, examination authority, recruiter, school, or guarantee of employment, examination results, or professional competence.

Features and content may change as the service improves. Material changes that affect an active paid service or your legal rights will be communicated as required by applicable law.

4. Guest use and accounts

Some features can be used as a guest and store data on the device. An account can enable recovery and synchronization of supported data. You are responsible for activity performed through your account unless you promptly report unauthorized access.

The account service agreement is concluded for an indefinite period and continues until you or the Operator terminates it in accordance with these Terms. Deleting your Patternly account terminates the account service, but does not by itself cancel a separate App Store subscription.

You can delete your account through the available in-app deletion flow. The Privacy Policy explains the data affected, retention rules, and limits of deletion.

5. App and content license

The Operator grants you a limited, personal, non-exclusive, non-transferable and revocable right to use the service and its content for your own learning, subject to these Terms and applicable law. Patternly and its original content remain protected by intellectual-property law.

For an app obtained through Apple, the Apple Standard Licensed Application End User License Agreement applies separately to the app license unless a custom EULA is supplied in App Store Connect. These Terms govern the Patternly service and do not replace mandatory Apple terms.

6. Acceptable use

Do not misuse the service, interfere with its security or operation, access another person’s account, automate access outside supported functionality, circumvent access controls, extract or redistribute substantial parts of the content, or use Patternly in violation of law or third-party rights.

You may quote limited portions where permitted by law, but you may not republish the question bank or sell access to Patternly content without written permission.


7. Premium and subscriptions

Patternly Premium is a monthly, automatically renewing digital service sold through the Apple App Store in Poland and the other European Union countries where the offer is available. The offer is ${vars.terms.premiumProductName.en} (product identifier ${vars.terms.premiumProductIdentifier.en}). It includes: ${vars.terms.premiumServiceScope.en}. Its billing period is ${vars.terms.premiumBillingPeriod.en}. There is no trial period. The initial total price including applicable taxes is ${vars.terms.premiumPriceIncludingTaxes.en}; the renewal price including applicable taxes is ${vars.terms.premiumRenewalPriceIncludingTaxes.en} for each ${vars.terms.premiumBillingPeriod.en}. Before you order, the purchase screen shows these current prices, the renewal period, the Premium scope, the payment method and timing, when the service starts, and the payment obligation.

The merchant-of-record arrangement is: ${vars.terms.merchantOfRecord.en}. Apple provides the payment and subscription-management channel, but this does not remove the Operator’s responsibility for Patternly’s conformity or your statutory claims against the responsible trader.

A subscription renews automatically only on the terms clearly displayed before purchase. You can stop future renewals through your Apple account settings. Deleting the app or your Patternly account does not cancel an App Store subscription. A material price increase is never accepted merely by continuing to use Patternly; it requires the legally applicable affirmative-consent, re-purchase, or renewal mechanism.

Apple may process transaction refunds through its platform. You may still direct complaints about Patternly’s service, conformity, or statutory consumer remedies to the Operator.

Premium access starts immediately after purchase only after your separate, express request to begin the service before the 14-day withdrawal period ends. No box may be preselected. Withdrawing after service has begun may require proportionate payment for the service supplied until withdrawal, where mandatory law permits it. Premium is not offered as a separate one-time supply of digital content, so Patternly does not ask you to acknowledge losing the withdrawal right on that basis. Otherwise statutory withdrawal rights remain. Submitting a complaint is not limited to 14 days.



8. Availability and changes

The Operator aims to keep Patternly available but does not promise uninterrupted or error-free access. Maintenance, security incidents, provider outages, device limitations, or legal requirements may interrupt features. Offline and synchronized data may not always have identical availability.

If a paid feature is materially discontinued, the Operator will provide any remedy required by applicable consumer law and the purchase platform’s rules.

9. Reports and communications

If you submit a content report or support message, you confirm that it is lawful and does not knowingly include material you have no right to share. You keep rights in your message and allow the Operator to use it only as needed to review the report, support you, protect the service, and meet legal obligations. The Privacy Policy explains associated data processing.

10. Suspension and termination

You may stop using Patternly and delete your account at any time, subject to separate cancellation of any platform subscription. The Operator may proportionately restrict access only for a material breach, a verified security threat, unlawful use, non-payment, or a binding legal requirement.

Except where immediate action is necessary to prevent harm or comply with law, the Operator will explain the reason, give reasonable prior notice and an opportunity to remedy the breach. You may appeal through ${vars.terms.complaintEmail.en}. Access will be restored when the ground ends. Suspension does not remove statutory conformity, refund, data-return, complaint, or cancellation rights and does not by itself cancel future Apple renewals.

11. Responsibility and statutory rights

Patternly provides learning support, not professional, employment, examination, legal, financial, or other regulated advice. You remain responsible for decisions made using the service and for checking requirements with the relevant official authority.

The Operator does not generally exclude or cap responsibility owed under mandatory law. Nothing in these Terms restricts rights concerning conformity of digital services, required updates, price reduction, withdrawal, termination, refund, complaint handling, personal injury, intent, gross negligence, or any other liability that cannot lawfully be limited.

12. Third-party services

Patternly may rely on Apple, authentication, hosting, notification, and billing providers. Their own terms may govern your relationship with them. References to certification providers and technology owners identify learning topics and do not imply sponsorship or endorsement.

13. Governing law and disputes

These Terms are governed by ${vars.terms.governingLaw.en}. The courts described as ${vars.terms.competentCourts.en} have jurisdiction, without depriving you of mandatory consumer protections or the right to bring a claim before a court available under applicable law in your place of residence.

Before filing a formal claim, you may contact the Operator so the issue can be reviewed. This does not limit statutory complaint, regulatory, alternative-dispute-resolution, or court rights.

14. Changes to the service, price, and Terms

The Operator may change the digital service without extra cost only for a valid reason stated here: legal compliance, security, interoperability, correcting defects, or improving functions without reducing the contracted paid-service scope. A negatively affecting change will be explained on a durable medium at least 30 days in advance, unless an immediate change is strictly required by law or an urgent security threat.

Where mandatory law applies, you may terminate within 30 days after receiving notice or after the change, whichever is later, unless the Operator keeps the unchanged conforming version available without additional cost. Material price changes require the applicable affirmative-consent, re-purchase, or renewal process; continued use is not silent consent.

A new Terms version records its effective date and changes. Renewed consent will be collected when legally required. An immutable copy of the version accepted by you will remain available on a durable medium.

15. Contract formation and durable record

Before account creation or purchase, Patternly must make these Terms and all required pre-contract information freely available in a form you can save. The contract is concluded only after an unambiguous action accepting the identified version. A paid order control must state that placing the order creates a payment obligation.

The Operator must provide confirmation on a durable medium and retain an auditable record of the Terms version, offer, price, renewal conditions, timestamp, locale, and consents associated with the transaction.

16. Technical requirements

Technical and compatibility requirements: ${vars.terms.technicalRequirements.en}. Internet access is required for account authentication, synchronization, purchases, restoring purchases, and communications with the Operator. Supported offline features may remain limited to data already available on the device.

Support and update commitment: ${vars.terms.supportCommitment.en}. Required security and conformity updates will be supplied for the period required by the contract and mandatory law.

17. Consumer withdrawal

If you are an eligible consumer, you generally have 14 days to withdraw from a distance contract without giving a reason. Send an unambiguous statement to ${vars.terms.withdrawalEmail.en} or the Operator’s postal address before the deadline. You may state: ‘I hereby withdraw from the contract for [service], ordered on [date], name, address, date and signature if sent on paper.’ This model is optional.

18. Digital-service conformity

Patternly must conform to its description, agreed paid-service scope, functionality, compatibility, accessibility, continuity, security, and updates that you can reasonably expect under the contract and mandatory law. If it does not, notify ${vars.terms.complaintEmail.en}. You may require conformity within a reasonable time and without significant inconvenience.

Where statutory conditions are met, you may seek a proportionate price reduction or terminate and obtain the applicable refund. These remedies are against the responsible trader and are not replaced by Apple’s transaction tools.

19. Complaints

Submit a complaint to ${vars.terms.complaintEmail.en} or the Operator’s postal address. Identify the account or transaction where possible, describe the issue and requested remedy, and avoid sending passwords. The Operator will confirm receipt on a durable medium and answer within 14 days from receipt unless a shorter mandatory period applies. The 14-day response period is not a deadline for submitting a complaint.

20. Data after termination

Where applicable law gives you a right to retrieve non-personal content you supplied or created, request it through ${vars.terms.complaintEmail.en}. The Operator provides the applicable content free of charge, without hindrance, within a reasonable time and in a commonly used machine-readable format, subject to statutory exceptions.

21. Out-of-court dispute resolution

The Operator’s position on participation in consumer ADR is: ${vars.terms.adrPosition.en}. Relevant entity or information point: ${vars.terms.adrEntity.en}. This section does not refer to the discontinued EU ODR platform and does not restrict your right to contact a consumer authority or court.

22. Operator and contact

Operator: ${vars.terms.operatorLegalName.en}, ${vars.terms.operatorBusinessForm.en}. Address: ${vars.terms.operatorRegisteredAddress.en}. Email: ${vars.terms.operatorEmail.en}. Phone: ${vars.terms.operatorPhone.en}.

Patternly is offered in: ${vars.terms.distributionTerritories.en}. Registration number: ${vars.terms.operatorRegistrationNumber.en}. Tax identifier: ${vars.terms.operatorTaxIdentifier.en}.`,
    pl: String.raw`Warunki korzystania z Patternly

Obowiązują od ${vars.terms.effectiveDate.pl}. Wersja ${vars.documentVersion.pl}. Niniejsze Warunki stanowią umowę między Tobą a ${vars.terms.operatorLegalName.pl}, ${vars.terms.operatorBusinessForm.pl}, pod adresem ${vars.terms.operatorRegisteredAddress.pl} („Operator”), dotyczącą korzystania z Patternly.

1. Akceptacja Warunków

Tworząc konto lub korzystając z usług Patternly powiązanych z kontem, akceptujesz niniejsze Warunki i przyjmujesz do wiadomości odrębną Politykę prywatności. Jeśli ich nie akceptujesz, nie twórz konta. Korzystanie jako gość bez wymaganej akceptacji nadal podlega właściwemu prawu i zasadom niezbędnym do bezpiecznego działania aplikacji.

2. Uprawnienie do korzystania

Patternly wymaga ukończenia co najmniej ${vars.terms.minimumUserAge.pl}, aby samodzielnie utworzyć konto. Jest to zasada produktu, a nie stwierdzenie, że prawo bezwzględnie zakazuje młodszym osobom nauki. Nauka w trybie gościa bez konta może pozostać dostępna dla młodszych użytkowników. Patternly nie udostępnia procesu zgody rodzica lub opiekuna, dlatego osoba poniżej tego wieku nie może utworzyć konta. Zakres zasady: ${vars.terms.minimumUserAgeScope.pl}. Podawaj prawidłowe dane konta i chroń dane dostępowe.

3. Usługa

Patternly jest niezależnym narzędziem do nauki i ćwiczeń związanych z rozmowami technicznymi oraz tematami certyfikacyjnymi. Nie jest oficjalnym dostawcą certyfikacji, organem egzaminacyjnym, rekruterem ani szkołą i nie gwarantuje zatrudnienia, wyniku egzaminu ani kompetencji zawodowych.

Funkcje i treści mogą się zmieniać wraz z rozwojem usługi. Istotne zmiany wpływające na aktywną usługę płatną lub Twoje prawa zostaną zakomunikowane w zakresie wymaganym przez właściwe prawo.

4. Gość i konto

Z części funkcji można korzystać jako gość, zapisując dane na urządzeniu. Konto może umożliwiać odzyskiwanie i synchronizację obsługiwanych danych. Odpowiadasz za działania wykonane przez Twoje konto, chyba że niezwłocznie zgłosisz nieuprawniony dostęp.

Umowa dotycząca usługi konta jest zawierana na czas nieoznaczony i trwa do jej rozwiązania przez Ciebie albo Operatora zgodnie z niniejszymi Warunkami. Usunięcie konta Patternly rozwiązuje umowę dotyczącą konta, ale samo w sobie nie anuluje odrębnej subskrypcji App Store.

Możesz usunąć konto przez dostępny przepływ usuwania w aplikacji. Polityka prywatności opisuje zakres danych, okresy przechowywania i ograniczenia usuwania.

5. Licencja na aplikację i treści

Operator udziela Ci ograniczonego, osobistego, niewyłącznego, nieprzenoszalnego i odwołalnego prawa do korzystania z usługi i jej treści na własne potrzeby edukacyjne, zgodnie z Warunkami i prawem. Patternly i jego oryginalne treści podlegają ochronie własności intelektualnej.

Dla aplikacji pobranej przez Apple standardowa umowa licencyjna Apple dotycząca aplikacji obowiązuje odrębnie, chyba że w App Store Connect udostępniono własną EULA. Niniejsze Warunki regulują usługę Patternly i nie zastępują obowiązkowych warunków Apple.

6. Dozwolone korzystanie

Nie nadużywaj usługi, nie zakłócaj jej bezpieczeństwa ani działania, nie uzyskuj dostępu do cudzego konta, nie automatyzuj dostępu poza obsługiwanymi funkcjami, nie obchodź zabezpieczeń, nie pozyskuj ani nie rozpowszechniaj istotnych części treści oraz nie używaj Patternly niezgodnie z prawem lub prawami osób trzecich.

Możesz cytować ograniczone fragmenty, jeżeli pozwala na to prawo, ale bez pisemnej zgody nie możesz publikować ponownie banku pytań ani sprzedawać dostępu do treści Patternly.


7. Premium i subskrypcje

Patternly Premium jest miesięczną, automatycznie odnawianą usługą cyfrową sprzedawaną przez Apple App Store w Polsce i pozostałych krajach Unii Europejskiej, w których oferta jest dostępna. Ofertą jest ${vars.terms.premiumProductName.pl} (identyfikator produktu ${vars.terms.premiumProductIdentifier.pl}). Obejmuje: ${vars.terms.premiumServiceScope.pl}. Okres rozliczeniowy: ${vars.terms.premiumBillingPeriod.pl}. Nie ma okresu próbnego. Początkowa łączna cena z podatkami wynosi ${vars.terms.premiumPriceIncludingTaxes.pl}; cena odnowienia z podatkami wynosi ${vars.terms.premiumRenewalPriceIncludingTaxes.pl} za każdy ${vars.terms.premiumBillingPeriod.pl}. Przed zamówieniem ekran zakupu pokazuje te aktualne ceny, okres odnowienia, zakres Premium, sposób i moment płatności, moment rozpoczęcia świadczenia oraz obowiązek zapłaty.

Model merchant of record: ${vars.terms.merchantOfRecord.pl}. Apple zapewnia kanał płatności i zarządzania subskrypcją, ale nie znosi to odpowiedzialności Operatora za zgodność Patternly ani Twoich ustawowych roszczeń wobec właściwego przedsiębiorcy.

Subskrypcja odnawia się automatycznie wyłącznie na warunkach jasno przedstawionych przed zakupem. Przyszłe odnowienia możesz zatrzymać w ustawieniach konta Apple. Usunięcie aplikacji lub konta Patternly nie anuluje subskrypcji. Istotna podwyżka ceny nie może zostać zaakceptowana przez samo dalsze korzystanie; wymaga właściwego mechanizmu wyraźnej zgody, ponownego zakupu albo odnowienia.

Apple może obsługiwać zwroty transakcji przez platformę. Reklamacje dotyczące usługi Patternly, jej zgodności i ustawowych środków możesz nadal kierować do Operatora.

Dostęp Premium rozpoczyna się natychmiast po zakupie wyłącznie na Twoje odrębne, wyraźne żądanie rozpoczęcia usługi przed końcem 14-dniowego terminu na odstąpienie. Checkbox nie może być zaznaczony domyślnie. W razie odstąpienia po rozpoczęciu świadczenia może być należna proporcjonalna zapłata za usługę spełnioną do chwili odstąpienia, jeżeli pozwalają na to bezwzględnie obowiązujące przepisy. Premium nie jest oferowane jako odrębna, jednorazowa dostawa treści cyfrowej, dlatego Patternly nie wymaga potwierdzenia utraty prawa odstąpienia na tej podstawie. W pozostałym zakresie prawa ustawowe pozostają. Reklamacji nie trzeba składać w ciągu 14 dni.



8. Dostępność i zmiany

Operator dąży do utrzymania dostępności Patternly, ale nie gwarantuje nieprzerwanego ani bezbłędnego działania. Konserwacja, incydenty bezpieczeństwa, awarie dostawców, ograniczenia urządzenia lub wymogi prawne mogą czasowo ograniczyć funkcje. Dane offline i zsynchronizowane mogą nie być zawsze dostępne w identycznym zakresie.

Jeżeli płatna funkcja zostanie istotnie wycofana, Operator zapewni rozwiązanie wymagane przez właściwe prawo konsumenckie i zasady platformy zakupu.

9. Zgłoszenia i komunikacja

Wysyłając zgłoszenie treści lub wiadomość do pomocy, potwierdzasz, że jest zgodna z prawem i nie zawiera świadomie materiałów, których nie masz prawa udostępnić. Zachowujesz prawa do wiadomości i zezwalasz Operatorowi użyć jej wyłącznie w zakresie potrzebnym do rozpatrzenia zgłoszenia, udzielenia pomocy, ochrony usługi i wykonania obowiązków prawnych. Powiązane przetwarzanie danych opisuje Polityka prywatności.

10. Ograniczenie i zakończenie dostępu

Możesz przestać korzystać z Patternly i usunąć konto, pamiętając o osobnym anulowaniu subskrypcji. Operator może proporcjonalnie ograniczyć dostęp wyłącznie z powodu istotnego naruszenia, potwierdzonego zagrożenia bezpieczeństwa, bezprawnego użycia, braku płatności lub wiążącego wymogu prawnego.

Poza sytuacją wymagającą natychmiastowej reakcji dla zapobieżenia szkodzie lub wykonania prawa Operator wyjaśni powód, uprzedzi z rozsądnym wyprzedzeniem i umożliwi usunięcie naruszenia. Odwołanie można wysłać na ${vars.terms.complaintEmail.pl}. Dostęp zostanie przywrócony po ustaniu podstawy. Zawieszenie nie usuwa ustawowych praw do zgodności, zwrotu, odzyskania danych, reklamacji ani anulowania i samo nie zatrzymuje odnowień Apple.

11. Odpowiedzialność i prawa ustawowe

Patternly wspiera naukę, ale nie świadczy porad zawodowych, rekrutacyjnych, egzaminacyjnych, prawnych, finansowych ani innych usług regulowanych. Odpowiadasz za decyzje podjęte na podstawie usługi i sprawdzenie wymagań u właściwego oficjalnego podmiotu.

Operator nie wyłącza ani ogólnie nie ogranicza odpowiedzialności wynikającej z bezwzględnie obowiązującego prawa. Warunki nie ograniczają praw dotyczących zgodności usługi cyfrowej, aktualizacji, obniżenia ceny, odstąpienia, rozwiązania, zwrotu, reklamacji, szkody na osobie, winy umyślnej, rażącego niedbalstwa ani innych podstaw niewyłączalnych.

12. Usługi podmiotów trzecich

Patternly może korzystać z Apple oraz dostawców uwierzytelniania, hostingu, powiadomień i rozliczeń. Relację z nimi mogą regulować ich własne warunki. Odwołania do dostawców certyfikacji i właścicieli technologii identyfikują temat nauki i nie oznaczają sponsorowania ani rekomendacji.

13. Prawo właściwe i spory

Warunki podlegają prawu: ${vars.terms.governingLaw.pl}. Właściwe sądy: ${vars.terms.competentCourts.pl}, bez pozbawiania Cię bezwzględnej ochrony konsumenckiej ani prawa do wniesienia sprawy przed sąd dostępny zgodnie z właściwym prawem w miejscu Twojego zamieszkania.

Przed formalnym zgłoszeniem roszczenia możesz skontaktować się z Operatorem, aby umożliwić rozpatrzenie sprawy. Nie ogranicza to ustawowego prawa do reklamacji, regulatora, alternatywnego rozstrzygania sporów ani sądu.

14. Zmiany usługi, ceny i Warunków

Operator może zmienić usługę cyfrową bez dodatkowego kosztu wyłącznie z ważnego powodu wskazanego tutaj: zgodność z prawem, bezpieczeństwo, interoperacyjność, usunięcie błędów albo ulepszenie bez ograniczenia umówionego zakresu usługi płatnej. Zmiana wpływająca negatywnie zostanie wyjaśniona na trwałym nośniku co najmniej 30 dni wcześniej, chyba że natychmiastowej zmiany bezwzględnie wymaga prawo lub pilne zagrożenie bezpieczeństwa.

Gdy wymaga tego prawo, możesz rozwiązać umowę w ciągu 30 dni od informacji lub zmiany — zależnie od tego, co nastąpi później — chyba że Operator pozostawi bez dodatkowego kosztu niezmienioną, zgodną wersję. Istotna zmiana ceny wymaga właściwego mechanizmu wyraźnej zgody, ponownego zakupu albo odnowienia; dalsze korzystanie nie jest milczącą zgodą.

Nowa wersja Warunków zapisuje datę obowiązywania i zmiany. Ponowna zgoda zostanie zebrana, gdy wymaga jej prawo. Niezmienna kopia zaakceptowanej wersji pozostanie dostępna na trwałym nośniku.

15. Zawarcie umowy i trwały zapis

Przed utworzeniem konta lub zakupem Patternly musi bezpłatnie udostępnić Warunki i wszystkie wymagane informacje przedumowne w formie możliwej do zapisania. Umowa zostaje zawarta dopiero przez jednoznaczne działanie akceptujące wskazaną wersję. Kontrolka płatnego zamówienia musi wskazywać, że zamówienie powoduje obowiązek zapłaty.

Operator musi przekazać potwierdzenie na trwałym nośniku i zachować audytowalny zapis wersji Warunków, oferty, ceny, odnowienia, czasu, języka i zgód związanych z transakcją.

16. Wymagania techniczne

Wymagania techniczne i kompatybilność: ${vars.terms.technicalRequirements.pl}. Internet jest wymagany do uwierzytelniania konta, synchronizacji, zakupów, odtwarzania zakupów i kontaktu z Operatorem. Obsługiwane funkcje offline mogą ograniczać się do danych dostępnych już na urządzeniu.

Zobowiązanie wsparcia i aktualizacji: ${vars.terms.supportCommitment.pl}. Aktualizacje bezpieczeństwa i zgodności będą dostarczane przez okres wymagany umową i bezwzględnie obowiązującym prawem.

17. Odstąpienie konsumenta

Jeśli jesteś uprawnionym konsumentem, co do zasady masz 14 dni na odstąpienie od umowy zawartej na odległość bez podania przyczyny. Przed terminem wyślij jednoznaczne oświadczenie na ${vars.terms.withdrawalEmail.pl} lub adres pocztowy Operatora. Możesz użyć treści: „Niniejszym odstępuję od umowy dotyczącej [usługi], zamówionej [data], imię i nazwisko, adres, data i podpis w przypadku wersji papierowej”. Wzór nie jest obowiązkowy.

18. Zgodność usługi cyfrowej

Patternly musi być zgodne z opisem, umówionym zakresem usługi płatnej, funkcjonalnością, kompatybilnością, dostępnością, ciągłością, bezpieczeństwem i aktualizacjami, których można rozsądnie oczekiwać na podstawie umowy i prawa. Brak zgodności zgłoś na ${vars.terms.complaintEmail.pl}. Możesz żądać doprowadzenia do zgodności w rozsądnym czasie i bez nadmiernych niedogodności.

Po spełnieniu ustawowych przesłanek możesz żądać proporcjonalnego obniżenia ceny albo rozwiązać umowę i otrzymać należny zwrot. Środki przysługują wobec właściwego przedsiębiorcy i nie są zastępowane przez narzędzia transakcyjne Apple.

19. Reklamacje

Reklamację wyślij na ${vars.terms.complaintEmail.pl} lub adres pocztowy Operatora. W miarę możliwości wskaż konto lub transakcję, opisz problem i żądane rozwiązanie; nie wysyłaj hasła. Operator potwierdzi odbiór na trwałym nośniku i odpowie w ciągu 14 dni od otrzymania, chyba że obowiązuje krótszy termin. Termin odpowiedzi 14 dni nie jest terminem na złożenie reklamacji.

20. Dane po zakończeniu umowy

Jeżeli właściwe prawo daje Ci prawo odzyskania nieosobowych treści dostarczonych lub utworzonych przez Ciebie, złóż wniosek przez ${vars.terms.complaintEmail.pl}. Operator udostępnia właściwe treści bezpłatnie, bez przeszkód, w rozsądnym czasie i powszechnie używanym formacie do odczytu maszynowego, z uwzględnieniem wyjątków ustawowych.

21. Pozasądowe rozstrzyganie sporów

Stanowisko Operatora wobec udziału w konsumenckim ADR: ${vars.terms.adrPosition.pl}. Właściwy podmiot lub punkt informacyjny: ${vars.terms.adrEntity.pl}. Sekcja nie odsyła do zamkniętej unijnej platformy ODR i nie ogranicza prawa kontaktu z organem konsumenckim ani sądem.

22. Operator i kontakt

Operator: ${vars.terms.operatorLegalName.pl}, ${vars.terms.operatorBusinessForm.pl}. Adres: ${vars.terms.operatorRegisteredAddress.pl}. E-mail: ${vars.terms.operatorEmail.pl}. Telefon: ${vars.terms.operatorPhone.pl}.

Patternly jest oferowane na terytoriach: ${vars.terms.distributionTerritories.pl}. Numer rejestrowy: ${vars.terms.operatorRegistrationNumber.pl}. Identyfikator podatkowy: ${vars.terms.operatorTaxIdentifier.pl}.`,
  });
}

export const termsOfService = renderTermsOfService(legalVariables);

...
function verifiedProfile(value: Record<string, unknown>, modeStructures: unknown, manifest: Record<string, unknown>, sourceModes: readonly string[], familyId: ContentPackageFamilyId, packageItemIds: readonly string[]) {
  ...
  const modes: VerifiedPackageMode[] = value.modes.map((mode) => {
    ...
    return Object.freeze({ modeId: mode.modeId, blueprintModeId: mode.blueprintModeId, availability: mode.availability, requestedLengths: Object.freeze([...mode.requestedLengths]), minimumActualLength: mode.requestedLengths[0]!, defaultRequestedLength: mode.defaultRequestedLength });
  });
  ...
  const configurations = structures.map((structure) => {
    ...
    return Object.freeze({ configurationId: structure.configurationId, configurationVersion: structure.configurationVersion, modeId: structure.modeId, blueprintModeId: structure.blueprintModeId, availability: structure.availability, requestedLengths: Object.freeze([...structure.requestedLengths]), minimumActualLength: structure.requestedLengths[0]!, defaultRequestedLength: structure.defaultRequestedLength, reinsertPolicy: structure.reinsertPolicy, ...(structure.feedbackOptions === undefined ? {} : { feedbackOptions: Object.freeze([...structure.feedbackOptions]) }), selection: cloneFreeze(structure.selection) });
  });
  let completionRule;
...
function orderedPositiveNumbers(value: unknown): value is readonly number[] { return Array.isArray(value) && value.length > 0 && value.every(positive) && value.every((entry, index) => index === 0 || entry > value[index - 1]!); }

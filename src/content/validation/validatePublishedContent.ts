...
function validateFeedbackDocument(value: unknown, feedbackAssets: ReadonlySet<string>): void {
  ...
  for (const [index, value] of blocks.entries()) {
    const block = record(value, `Algorithms feedback block ${index}`); const type = text(block.type, `Algorithms feedback block ${index} type`);
    if (type === "paragraph") { exact(block, ["type", "text"], "Algorithms paragraph feedback block"); text(block.text, "Algorithms paragraph feedback text"); continue; }
    if (type === "heading") { exact(block, ["type", "level", "text"], "Algorithms heading feedback block"); if (block.level !== 2 && block.level !== 3) throw new ContentValidationError("Algorithms feedback heading level is invalid."); text(block.text, "Algorithms heading feedback text"); continue; }
    if (type === "bullet_list" || type === "ordered_list") { exact(block, ["type", "items"], "Algorithms list feedback block"); const items = values(block.items, "Algorithms feedback list items"); if (!items.length) throw new ContentValidationError("Algorithms feedback list must not be empty."); items.forEach((item) => text(item, "Algorithms feedback list item")); continue; }
    if (type === "code") { exact(block, ["type", "language", "code"], "Algorithms code feedback block"); if (!(ALGORITHM_FEEDBACK_CODE_LANGUAGES as readonly string[]).includes(text(block.language, "Algorithms feedback code language"))) throw new ContentValidationError("Algorithms feedback code language is unsupported."); text(block.code, "Algorithms feedback code"); continue; }
    if (type === "image") { exact(block, ["type", "assetId", "alt"], "Algorithms image feedback block"); const assetId = text(block.assetId, "Algorithms feedback image assetId"); if (!/^[a-z0-9][a-z0-9/_-]*$/.test(assetId) || !feedbackAssets.has(assetId)) throw new ContentValidationError("Algorithms feedback image must reference a declared local asset."); text(block.alt, "Algorithms feedback image alt"); continue; }
    if (type === "callout") { exact(block, ["type", "kind", "text", "title"].filter((key) => block[key] !== undefined), "Algorithms callout feedback block"); if (!(ALGORITHM_FEEDBACK_CALLOUT_KINDS as readonly string[]).includes(text(block.kind, "Algorithms feedback callout kind"))) throw new ContentValidationError("Algorithms feedback callout kind is unsupported."); text(block.text, "Algorithms feedback callout text"); if (block.title !== undefined) text(block.title, "Algorithms feedback callout title"); continue; }
    throw new ContentValidationError("Algorithms feedback block type is unsupported.");
  }
}
function validateItemFeedback(value: unknown, feedbackAssets: ReadonlySet<string>): void { const feedback = record(value, "Algorithms item feedback"); exact(feedback, ["reason", "details", "wrongOptionExplanationsByOptionId", "omittedCorrectExplanationsByOptionId"].filter((key) => feedback[key] !== undefined), "Algorithms item feedback"); text(feedback.reason, "Algorithms feedback reason"); validateFeedbackDocument(feedback.details, feedbackAssets); for (const explanations of [feedback.wrongOptionExplanationsByOptionId, feedback.omittedCorrectExplanationsByOptionId]) if (explanations !== undefined) for (const [id, explanation] of Object.entries(record(explanations, "Algorithms feedback explanations"))) { text(id, "Algorithms feedback option id"); text(explanation, "Algorithms feedback explanation"); } }
...
export function validateCertificationBank(value: unknown, manifest: PublishedTrackManifest): PublishedCertificationBank {
  ...
  validateCertificationMixedPractice(bank.mixedPractice, ids);
  validateCertificationQuickReview(bank.quickReview);
...
function validateCertificationMixedPractice(value: unknown, knownItemIds: ReadonlySet<string>): void {
  const mixed = record(value, "Certification Mixed Practice");
  exact(mixed, ["blueprintId", "blueprintVersion", "modeId", "requestedLengths", "shortening", "selectionScope", "itemIds"], "Certification Mixed Practice");
  text(mixed.blueprintId, "Certification Mixed Practice blueprint ID");
  text(mixed.blueprintVersion, "Certification Mixed Practice blueprint version");
  if (mixed.modeId !== "certification-mixed-practice" || mixed.shortening !== "allowed_within_interleaved_blueprint" || mixed.selectionScope !== "unique_interleaved_blueprint") throw new ContentValidationError("Certification Mixed Practice contract is invalid.");
  const lengths = values(mixed.requestedLengths, "Certification Mixed Practice requested lengths");
  if (lengths.length !== 3 || lengths.some((length, index) => length !== [10, 20, 40][index])) throw new ContentValidationError("Certification Mixed Practice must expose exactly 10, 20, and 40 item lengths.");
  const itemIds = stringValues(mixed.itemIds, "Certification Mixed Practice item IDs");
  if (itemIds.length < 10 || itemIds.some((itemId) => !knownItemIds.has(itemId))) throw new ContentValidationError("Certification Mixed Practice must name unique installed interleaved items.");
}

function validateCertificationQuickReview(value: unknown): void {
  const review = record(value, "Certification Quick Review");
  exact(review, ["blueprintId", "blueprintVersion", "modeId", "maximumLength", "shortening", "selectionScope", "persistentResolutionPolicy"], "Certification Quick Review");
  text(review.blueprintId, "Certification Quick Review blueprint ID");
  text(review.blueprintVersion, "Certification Quick Review blueprint version");
  if (review.modeId !== "certification-quick-review" || review.maximumLength !== 10 || review.shortening !== "allowed_within_eligible_review_evidence" || review.selectionScope !== "eligible_due_review_evidence" || review.persistentResolutionPolicy !== "two_consecutive_due_review_successes") throw new ContentValidationError("Certification Quick Review contract is invalid.");
}

...
test("the first Home visit replaces repeated empty metrics with one honest next-step state", () => {
  ...
  assert.match(homeTab, /isFirstUse && !showGoalInvitation/);
...
test("goal invitation stays optional for guest and healthy account, reloads after Goal, and persists dismissal per track", () => {
  const homeTab = readFileSync("src/features/home/tabs/HomeTab.tsx", "utf8");
  ...
  assert.match(source, /account\.state\.kind === "guest" \|\| \(account\.state\.kind === "authenticated"/);
  assert.match(source, /account\.state\.accountData\.status === "synced"/);
  assert.match(source, /&& homeActiveSession === null && data\.goal === null && !data\.goalOnboardingDismissed/);
  ...
  assert.match(homeTab, /goalInvitationAudience === "account" \? "No goal is set for this track" : "Set a goal for this track"/);
  assert.match(homeTab, /"Choose when and why you want to practise \{\{trackName\}\}\."/);
  ...
  assert.match(homeTab, /We couldn't hide this goal invitation\. Try again\. Your goal data was not changed\./);

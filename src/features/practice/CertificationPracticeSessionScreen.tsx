...
export function CertificationPracticeSessionScreen({ navigation, route }: Props) {
  ...
  return <PracticeSessionSurface
    ...
    retryLabel={exitFailure === "retry_abandon" || exitFailure === "retry_checkpoint" ? "Retry end session" : exitFailure === "recover_abandon" ? "Recover end session" : exitFailure === "recover_operation" ? "Recover timer checkpoint" : completionFailure ? completionFailure.kind === "retry_completion" ? "Finish session" : completionFailure.kind === "recover_completion" ? "Recover completion" : completionFailure.kind === "retry_final_checkpoint" ? "Retry final checkpoint" : "Recover final checkpoint" : canRecover ? "Recover session" : undefined}
    ...
    timer={{ accessibilityLabel: `${t("Active foreground time")} ${formatPracticeElapsedTime(projection.elapsedForegroundMs)}`, label: `${t("Active time")} ${formatPracticeElapsedTime(projection.elapsedForegroundMs)}` }}
  />;
}

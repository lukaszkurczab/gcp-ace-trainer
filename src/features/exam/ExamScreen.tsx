...
export function ExamScreen({ navigation }: Props) {
  ...
  useEffect(() => { let active = true; void (async () => { try { await refresh(); } catch (cause) { try { await startCertificationExam(); if (active) await refresh(); } catch (startCause) { if (active) setError(startCause instanceof Error ? startCause.message : cause instanceof Error ? cause.message : "Exam is unavailable."); } } })(); return () => { active = false; }; }, []);
  useEffect(() => { const interval = setInterval(() => { void refresh().catch((cause) => setError(cause instanceof Error ? cause.message : "Exam refresh failed.")); }, 1_000); return () => clearInterval(interval); }, [projection?.session.id]);
  ...
  const finish = async () => { try { navigation.replace(ROUTES.RESULT, { sessionId: await finalizeCertificationExam() }); } catch (cause) { setError(cause instanceof Error ? cause.message : "Exam finalization failed."); } };

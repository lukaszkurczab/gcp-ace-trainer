  ...
  const projection: SimulationSurfaceProjection = result?.score
    ? {
        ...
        completion: {
          ...
          ...(review ? {} : { reviewAction: { label: "Review session", onPress: () => navigation.navigate(ROUTES.ALGORITHMS_INTERVIEW_SIMULATION_REVIEW, { sessionId: result.sessionId }), variant: "secondary" as const } }),
        },
        actions: { primary: { label: "Back to practice", onPress: () => navigation.navigate(ROUTES.PRACTICE_HUB) } },
      }
    : {
        ...
        actions: { primary: { label: "Try again", onPress: () => { void load(); } }, secondary: { label: "Back to practice", onPress: () => navigation.navigate(ROUTES.PRACTICE_HUB), variant: "secondary" } },
      };

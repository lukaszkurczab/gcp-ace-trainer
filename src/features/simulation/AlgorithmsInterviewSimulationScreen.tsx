...
function noop() {}
function confirmationSurface(projection: AlgorithmsSimulationProjection, state: "finish_confirmation" | "leave_confirmation" | "abandon_confirmation", title: string, description: string, cancel: () => void, confirm: () => void): SimulationSurfaceProjection { return { ...base(projection), state, question: question(projection, responseFromProjection(projection)), confirmation: { title, description, secondary: { id: "cancel-confirmation", label: "Keep working", onPress: cancel, variant: "secondary" }, primary: { id: `confirm:${state}`, label: state === "abandon_confirmation" ? "Abandon simulation" : state === "finish_confirmation" ? "Finish simulation" : "Leave and resume later", onPress: confirm, ...(state === "abandon_confirmation" ? { variant: "destructive" as const } : {}) } } }; }
